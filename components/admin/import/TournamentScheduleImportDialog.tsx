...
export default function TournamentScheduleImportDialog({ open, tournamentId, orgSlug, onClose, onPreviewed, onCommitted }: Props) {
  ...
  async function runPreview() {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/tournaments/${encodeURIComponent(tournamentId)}/schedule/import/preview${query}`,
        { method: 'POST', credentials: 'same-origin', body: form },
      );
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error ?? 'Schedule import preview failed.');
      setPreview(data.preview as ImportPreview);
      await onPreviewed?.();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Schedule import preview failed.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function runCommit() {
    ...
    try {
      const res = await fetch(
        `/api/admin/tournaments/${encodeURIComponent(tournamentId)}/schedule/import/commit${query}`,
        {
          method: 'POST',
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error ?? 'Schedule import apply failed.');
      setCommitResult(data.result as ImportCommitResult);
      await onCommitted?.();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Schedule import apply failed.');
    } finally {
      setCommitting(false);
    }
  }
      ...
      <div className={`modal ${styles.modal}`} onClick={event => event.stopPropagation()}>
        <div className="modal-header">
          <div className={styles.titleBlock}>
            <div className={styles.titleRow}>
              ...
              <h3>Add or Update Schedule</h3>
            </div>
            <p>Download a schedule template, edit it in Excel or Google Sheets, then preview and apply add/update rows. Missing games are not deleted.</p>
          </div>
          <button type="button" className="btn btn-ghost btn-data" onClick={closeAndReset} aria-label="Close schedule import dialog">
            <X size={16} />
          </button>
        </div>
        ...
        <div className={styles.body}>
            ...
            <div className={styles.templateRows}>
              <div className={styles.templateRow}>
                <div className={styles.templateCopy}>
                  ...
                  <span>Best for previewing bulk edits. Includes existing game IDs, team IDs, venue IDs, and facility IDs.</span>
                </div>
              ...
              <div className={styles.templateRow}>
                <div className={styles.templateCopy}>
                  ...
                  <span>Best for previewing new scheduled games. Fill in division, team, date/time, and venue fields.</span>
                </div>
                <div className={styles.templateActions}>
          ...
          {preview && (
            <section className={styles.previewPanel}>
              ...
              <p className={styles.modeNote}>Add/update only. Games missing from this file stay unchanged.</p>
        ...
        <div className="modal-footer">
          <span className={styles.footerNote}>
            {commitResult ? 'Schedule data was updated.' : 'Preview first, then apply add/update changes.'}
          </span>

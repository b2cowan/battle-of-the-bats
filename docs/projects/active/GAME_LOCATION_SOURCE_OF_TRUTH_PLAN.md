...
### Phase 4 — House league gets the same field model (RULED R3 — the only phase with a migration)
- [x] **DECIDED (R3, 2026-08-08): house league uses the same venue + surface model as tournaments.** Zero prod rows (§0.6) means this is a schema decision made before there is data to migrate — that window closes the moment a customer schedules a league game, so this phase should not drift to the end
- [ ] Migration: add venue + surface references to `league_games` and `league_practices` (**first task** — nothing else in this phase starts before it)
- [ ] **Same unit of work:** update `docs/agents/db/DATA_DICTIONARY.md` + run `npm run refresh:snapshots` (dev + prod). `npm run check:dictionary` fails the build otherwise
- [ ] Reuse the Phase 1 helper and the Phase 2 derived-string formatter unmodified — if either needs changing to fit league games, Phase 1 got the shape wrong
- [ ] House league authoring picks a field the same way tournaments do; clash detection and the `venue_unchecked` finding apply from day one
- [ ] **Coach events** (`rep_team_events.field_number`, `rep_tryout_sessions`): recommend **explicitly out of scope** — 115 prod rows across 7 strings, frequently off-site, honestly text. Record the decision so it is a ruling, not an omission (prompt §4.5)
- [ ] `basic_coach_team_events`: follows the coach-events ruling
- [ ] Any of this that needs a table/column ⇒ **migration + `docs/agents/db/DATA_DICTIONARY.md` + `npm run refresh:snapshots` in the same unit of work**

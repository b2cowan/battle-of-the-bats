...
#### Phase A1 — Announcement push event + fan tournament-wide fan-out (core ask)
- [ ] Add an `announcement` (working name; consider `tournament_message`) notification event type to `NotificationEventType` and `NOTIFICATION_EVENT_LABELS` (`lib/notification-labels.ts`); default push ON in `PUSH_DEFAULT_ON_EVENTS`.
- [ ] Add a tournament-wide fan fan-out `notifyFansForAnnouncement()` in `lib/fan-notify.ts` (analogous to `notifyFansForGame()`), reading `fan_push_subscriptions` for **all** subscribers of the tournament (not scoped to one team). Reuse the existing web-push sender + 410-dead-subscription cleanup.
- [ ] Notify **coaches** on the same event via `notify()` (accepted-team coaches for the tournament), respecting their existing global/per-tournament preferences.
- [ ] Wire the `save` action in `app/api/admin/communications/route.ts` so that a posted announcement can optionally trigger the fan + coach push (guarded by a "notify" flag on the request, not automatic — see Open Question 4).
- [ ] Push payload deep-links to the tournament's `/news` (or the pinned Schedule banner) so a tap lands on the message.
- [ ] Plan-gate the fan push per Open Question 3 (default assumption: reuse `fan_score_alerts` tier for now; confirm with `/billing` + `/strategy`).

#### Phase A2 — Broaden the fan opt-in (tournament-level + categories)
- [ ] Allow a **tournament-level** opt-in (follow the tournament, not just a team) so a fan can receive tournament messages without following a specific team. Extend the fan subscription model to allow a `team_id`-null (tournament-wide) row, or add a subscription "scope"/category. **Migration required** (new column(s) on `fan_push_subscriptions`, e.g. nullable `team_id` already exists as a key — confirm whether a `categories`/`scope` column is needed).
- [ ] Add fan-facing category preferences (small set): **Score alerts**, **Schedule changes**, **Tournament messages** — default all ON. Surface in `FollowAlertsToggle` / the My-Team dock and an "app notifications" entry point that works even without following a team.
- [ ] Respect categories in both `notifyFansForGame()` (score) and `notifyFansForAnnouncement()` (messages/schedule).

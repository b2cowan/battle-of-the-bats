...
> **Created:** 2026-06-28 · **Status:** ✅ **SHIPPED — LIVE ON PROD since 2026-07-22** (status trued up 2026-07-28; header previously said "Planning"). The directory shipped as part of the Unified Home bundle: live at `/discover`, server-rendered + SEO metadata (`app/(consumer)/discover/page.tsx`), tournament-keyed via `lib/directory.ts`, sitemap + robots shipped, Event Settings listing toggle live. **Model evolution vs. this brief:** listing flipped from opt-in-default-OFF to **discoverable-by-default (opt-out)** per owner decision 2026-07-22 (mig 197; existing rows untouched, public-status gate still ANDs). Detailed phase-checkbox reconcile + archive move = pending follow-up pass. · **Plan:** `docs/projects/active/PUBLIC_TOURNAMENT_DISCOVERY_DIRECTORY_PLAN.md`

...
**Status:** BUILT — dev-only, unpushed (2026-06-24). Decisions confirmed (all 7 at the recommended set: flexible multi-division rooms · auto-maintained membership · undeletable "All coaches" room · division-scoped V1, deferring hand-add/cross-tournament/coach-created). **No database migration** (the engine was built for sub-rooms). typecheck clean; lint clean (2 idiomatic data-fetch-in-effect warnings, matching existing chat code); dictionary/snapshot/org-context guards green. **Next:** adversarial `/review` on the room-CRUD + membership-routing write paths + the multi-room UI, then browser verification, then ship behind the gate.

**What was built (dev):**
- **Resolver** (`lib/chat-resolvers.ts`): `resolveTournamentChatParticipants` now takes a division SET (`null` = all; non-empty = those divisions; empty = nobody — a mis-scoped room never widens to "all").
- **Room model** (`lib/chat-service.ts`): `roomDivisionIds()` scope source of truth; `listTournamentChatRooms`, `getTournamentRoomById`, `createTournamentDivisionRoom`, `renameChatRoom`, `deleteTournamentChatRoom` (All-coaches protected), `listTournamentChatRoomSummaries`, `getTournamentDivisionsForChat`; sync + roster derive scope from the room; coach self-heal extended to EVERY matching room; coach room list carries a tournament `contextLabel`.
- **API**: `…/chat/rooms` (GET list+divisions / POST create), `…/chat/rooms/[roomId]` (GET roster / PATCH rename / DELETE), `…/chat/moderate` extended with `roomId`. The old single-room `…/chat` GET was removed (subsumed). **Delete is empty-only**: `deleteTournamentChatRoom` rejects the All-coaches room AND any room with messages (close it instead) — history is never destroyed.
- **UI** (owner-directed iteration): a **two-dock messaging layout** framing a full-width conversation. **Left "Rooms" dock** = pure switcher (room list + New room), toggled by a "Rooms" button in the header; collapsible side column on desktop, full-screen slide-over on mobile (WhatsApp-style). **Right "Manage room" dock** = everything about the open room (members + mute + invites + rename/close/delete), toggled by the "Members" button (mirror of the old single-dock pattern). Room list sorts All-coaches first, then by most-recent message. The default room shows as **"All coaches"** in the admin (the tournament is named in the nav). Rename only for division rooms; delete only for empty division rooms. `NewRoomDialog` composer = division multi-select with team counts. Coach sidebar shows the tournament context label for division rooms. (The earlier top pill-strip was replaced by the dock layout per owner UX review.)

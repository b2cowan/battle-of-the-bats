...
**WHERE WE ARE (updated 2026-06-18):** Steps 1–3 DONE + browser-verified. **Step 4 (accepted portal) DONE + committed** — payment instructions now surfaced in the coach portal (How-to-pay panel), the redundant status block repurposed into a "Payment" detail section, a persistent accepted "What's next" strip (pay → roster → schedule), a quiet `CoachEmptyState` tier for no-action empties, an honest "Tournament starts in" countdown, and Event Settings copy noting coaches see instructions in-portal. **Step 5 (notification emails) DONE + committed** — a dev-only email preview gallery at `/dev/email-preview`; announcement emails fixed (branded + correct recipient); bulk accept/reject/payment emails fixed (real organizer contact + real tournament/division names — J5-056/057); the acceptance email now states the amount owed (J5-063); and a **Premium Coaches Portal welcome email** added on checkout provisioning (was missing). **Step 6 largely done out of order** (Fees/Announcements/footer/logo → [COACH_PORTAL_GROWTH_PLAN.md](COACH_PORTAL_GROWTH_PLAN.md)). **NOW AT Step 7 (Upgrade to Premium) — PLUMBING BUILT 2026-06-18, ready to WALK on dev:** the Premium `team` checkout gate is **OPEN on the dev database** (`plan_gating.team='live'`; prod-guarded flip helper `scripts/dev-open-team-gate.mjs`); Stripe **TEST** is configured on dev (owner-confirmed); the in-portal upgrade CTAs (`ScopeShelf` footer on all 4 free-team pages + the afterglow `ScopeCeilingInterest` widget) now route to the **real `/coaches/start` checkout when the gate is open** (dev), else express-interest — gate read server-side, per-environment DB, **fails closed, prod-safe (a deploy can't flip it)**. **PROD stays express-interest until prod's gate is deliberately opened at launch.** All committed to local `dev`; **owner must push `dev` to deploy** before walking on `dev.fieldlogichq.ca`. Coach = `b2cowan@outlook.com`; accepted tournament record `/coaches/tournaments/81b249be-dfb1-4460-9239-d7c6bd8aad57`. **Open threads for Step 7/8:** (a) two more upgrade CTAs (coach hub + team-workspaces list) already link to `/coaches/start` but keep a static "Express interest" label — optional consistency follow-up; (b) the in-portal upgrade routes to `/coaches/start` (a "create new team" signup) — decide whether it should carry the existing basic-team context for upgrade-in-place; (c) Gap B: the express-interest form sends no confirmation email (prod-facing); (d) **Step 8** = walk the full Premium Coaches Portal at `/{orgSlug}/coaches/*` (reachable in dev via platform-admin Dev Tools "Coaches Portal Premium" seed → `/dev-standalone-team/coaches`, or now via the real checkout).

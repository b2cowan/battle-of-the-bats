...
**WHERE WE ARE (handoff point, 2026-06-15):** Steps 1–2 DONE on dev. Coach registers → lands straight in portal with first-run welcome banner (working). Currently at the **pending portal (Step 3)** — `toronto blue jays5` registered into U13 of Battle of the Bats 2026, pending. Next: review the pending portal, then have `owner@dev.local` accept it (Step 4). **Two fresh observations logged below (welcome/hero redundancy; ambiguous "Decision" date).** Test account `b2cowan@outlook.com` is LIVE on dev with this registration (NOT wiped — owner is mid-review).

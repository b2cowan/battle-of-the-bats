...
**Remaining Phase 6:**
- [ ] **Auto-scheduler double-elim dependency graph** — the auto-scheduler can slot a dependent game (e.g. Grand Final) before its feeder games, producing impossible/overlapping times; it should topologically order playoff games by `Winner/Loser` references before assigning slots.
...
- [ ] **Double-elim / consolation for split-pool (no-crossover) mode** — currently single-elim per pool.
- [ ] **Seeding sources**: seed-by-number captured at registration; seed by prior-event results.

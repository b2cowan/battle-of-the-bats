...
### 2A.4 — Printable manifest ✅ BUILT · Schedule projection ⛔ BLOCKED (concurrent work)
- [x] **Printable candidate sheet** — a "Print sheet" action on the check-in screen builds a branded PDF (Bib · [Player] · Age · In · Notes; **bib-only columns when blind**, blank In/Notes for pen) via the existing `downloadPDF` (FieldLogicHQ-branded default; org-branded settings = later polish). Lint 0-err, typecheck clean. `getRepTryoutSessionsForTeam` already exists in the data layer for the projection.
- [ ] **Schedule projection — DEFERRED:** rendering tryout sessions as read-only "Tryout" chips on the coach schedule REQUIRES editing `app/[orgSlug]/coaches/teams/[teamId]/schedule/page.tsx`, which currently has **another agent's uncommitted concurrent changes** in the working tree. Editing it would entangle this work with theirs (can't be committed separately). **Resolve before building:** wait until the foreign schedule work is committed, then add the projection (small diff: union `getRepTryoutSessionsForTeam` into the schedule render as a distinct read-only chip), OR owner OKs building on top. Surface = the schedule view only; no new data/API needed (the read function exists).

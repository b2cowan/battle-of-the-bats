...
## 5. Stage 2 · The block — in full

**RULED 2026-09-15 (owner, in chat: "I agree with your proposals" — D1–D11 all Build as drawn, as revised on the tab that day) · drawn at true size on the hub's "2 · The block" tab · NOT BUILT · kickoff prompt: [COACH_PRACTICES_STAGE2_BUILD_PROMPT.md](COACH_PRACTICES_STAGE2_BUILD_PROMPT.md)**

### 5.0 The rulings (owner, 2026-09-15)

| | Decision (as drawn) | Ruling |
|---|---|---|
| D1 | A block opens to four things — title · the clock row · *What you're doing* · *What you're watching for* — plus its Players line (reading **Whole team** until names are chosen); Coaching points · Staff · Equipment (while no stations) · Stations are doors at the foot; a field with content is always shown. | Build as drawn |
| D2 | Description and Goal stay two fields (renamed by D3); no notes-area merge; no shape change. | Build as drawn |
| D3 | One vocabulary at both levels: *What you're doing · What you're watching for · Coaching points*; the sheet's block prefix "Goal:" → "Watch for:"; stored keys untouched. | Build as drawn |
| D4 | The open block repeats nothing of the start; the clock row shows "ends 7:15 p.m." (the rest block: "runs to 8:30 p.m."; a template: nothing). The gutter's size is not re-opened. | Build as drawn |
| D5 | Quick lengths 5 · 10 · 15 · 20 · 30 as chips before the minutes; *Rest of practice* the last chip, ABSENT when another block holds it; un-pressing Rest returns 15. | Build as drawn |
| D6 | The shut row: title (+ Rotation tag) · first line (through a sole station) · "3 stations" when it has them · **Whole team** or "6 players" · nothing else; no points count. | Build as drawn |
| D7 | A block with exactly one station and no words of its own asks for no teaching; it opens onto the station's read-only text and provenance; title and minutes stay the block's; D20 untouched; the sole station's card head is stage 3's. | Build as drawn |
| D8 | 390 and 641–768: no new arrangement — the clock row and doors wrap; the 44px floor. | Build as drawn |
| D9 | Escape closes every sheet on the page (the five, on the shared dialog floor); never an open block. | Build as drawn |
| D10 | Coaching points as ONE field, one point per line (stored as today — a capped list). | Build as drawn |
| D11 | Kit lives at exactly ONE level — the activity's, the same level as people (the block while it has no stations; each station once it has them; kit MOVES when a station arrives, never vanishes); the practice's list at the top is the bag — the union plus extras, derived items removed where they came from. | Build as drawn |

**Two calls made without a decision stand:** the doors line at the block's foot in the ghost row's quiet voice; the title placeholder "What are we doing?" stays.

**As proposed (2026-09-14):** One block's anatomy as the row stage 1 made open in place: what a block asks for, in what
...
|---|---|---|---|
| **D1** | **Always shown vs on demand.** Which fields a block opens with, and which wait behind a door. | **Always: title · the clock row · What you're doing · What you're watching for · Players (reading "Whole team" until names are chosen — owner, 2026-09-15; clearing every name returns it; never a number — names are what the sheet and the field screen print, and "how many are here" is attendance's question, answered on the field screen). On demand: + Coaching points · + Staff · + Equipment (D11, while the block has no stations) · + Stations.** A field with content is always shown. The Players line is absent when the block has stations (people at one level — already the code's rule) and on a template; the teaching fields and the Coaching points door are absent when the block has EXACTLY ONE station and no words of its own (D7). Alternative on the table: *Watching for* on demand too — one line cheaper on every block, at the cost of the line the field screen exists for being one tap further from the keyboard. | The block body's render order and the doors line (new CSS, no model change); the "emptiness discards nothing" rule (kept — a door opened is a field once it holds something); the layout gate's blind spot inside an open block (an interaction hook in `scripts/layout-screens.mjs`, owed since stage 1's review). |
...
| **D6** | **The shut row's line.** Today: title · first line · "Who: everyone / N players" or "N stations" · "Coaching points: N" · the Rotation tag. | **Title (+ Rotation tag) · first line · "3 stations" when it has them · "Whole team" or "6 players" (the block's own Players line read back — owner, 2026-09-15) · nothing else.** Drop "Coaching points: N" (§133 ruling); "Who: everyone" was an inference and is replaced by the block's own word. The first line resolves through a sole station so a drill-placed row reads its drill. Untitled stays "Block 2" in the quiet italic. | `rowMeta` in `BlockCard`; the row's accessible name (its content behind the sr-only "Open" — unchanged idiom); the help's "how many coaching points it carries" sentence. |
...
| **D10** *(added with the frames, 2026-09-15)* | **Coaching points as one field — one point per line — instead of a numbered row per point.** | **One field.** Drawn at true size the doors alone take the written warm-up from 691px to ~600 — on a block that already holds points and players only Staff and Stations leave, and the numbered rows (an input, a number, a remove button and an add link per point — a form inside the form; 59 buttons on the walk’s three-block page) are most of what remains. The field screen and the sheet already print the points as a list; a coach types one per line. Stored exactly as today (a list, capped at eight, each line capped). The same field on a station is stage 3’s. | `CoachingPoints` on the block becomes a textarea split on newlines at patch time (`MAX_COACHING_POINTS` / `MAX_SHORT_TEXT_LEN` hold on save — the sanitiser is unchanged); the station and the drill library keep their rows until stage 3 (two shapes for one idea for one stage — flagged). |
| **D11** *(added 2026-09-15, the owner's question; tightened the same day on "three layers?")* | **Equipment: kit lives at exactly ONE level — the activity's, the same level as people — and the practice's list at the top is the bag.** | **Two ideas, not three layers.** Two questions about kit: *what does this activity need* and *what do I bring tonight*. A block is one activity (no stations) or a circuit (stations), so the activity's kit lives at whichever level the activity lives, never both: **on the block while it has no stations; on each station once it has them** — the block's line goes when a station exists, exactly as `playerIds` does (people at one level, 2026-08-01). The seam: kit typed on a block that then gains a station never vanishes (today the block's players silently do — `sanitizeBlock` deletes them) — it moves onto the first station when that station is written, or up into the bag when the first station is a drill. A written block had nowhere to say "ladders" except the top, and nothing adds the two up, so the sheet's head and its stations can already disagree. The practice's line then reads the UNION — what every block and station needs, plus what the coach adds at the top that belongs to no block (first-aid kit, water); what rose from below is removed below, never at the top — the rule the practice's tags already follow (`derivedCategories`). Quantities stay out (the vocabulary is names — "2 ladders" lives in the block's sentence). | `PracticePlanBlock.equipmentTagIds` (plan JSON — no migration; legacy `equipment` names not needed on a block), kept only while `stations` is empty — `sanitizeBlock` MOVES it (first written station, else the plan's `equipmentTagIds`) rather than deleting it; `restrictTagIds`, `collectPracticePlanTagIds` and `repointPracticePlanTags` add the block level; `resolvePracticePlanTagNames` resolves it; the About summary derives the union (memoised on blocks' equipment ids); `planToTemplateShape` keeps it (shape); `handlePrint` adds "Equipment: …" to the block line; the field screen shows a block's kit under its words (stage 5's to place). A pre-existing top-level list stays exactly as typed — nothing is moved down. |
...
### 5.9 The frames — DRAWN 2026-09-15 on the hub's "2 · The block" tab (before/after, true size, clickable markers, rulings wired)

**Measured on the drawings at true size** (the build re-measures on the real screen): the written warm-up open **691 → 579px** with its Players and Equipment lines (507 without equipment; 598 with the doors alone — D10 accounts for the rest); a block just added **~600 → 411px** (Players reading Whole team); the drill-placed block **1,225 → 922px** (its station card drawn as today — stage 3’s); the phone’s warm-up **889 → 742px** (the clock row takes two lines there); the tablet block 600px with the 44px floor; a template’s block 439px. The honest reading: **the doors win big on a new block and a drill-placed block, and modestly on a written one** — which is why D10 was added.

**Measured on the BUILD (Playwright on the UAT fixture, 2026-09-15 — the real screen, the fixture's words):** the probe practice's written warm-up open **682px at 1440** (the frame drew 579 — the fixture's block also holds a Staff line and its six longer names wrap to a second row of chips; without those it is ~614) · **698 at 768** · **810 at 390**; the rest-of-practice game block **541** at 1440; the drill-placed block **1,033** at 1440 (drawn 922 — the fixture's station card carries two hint lines) and **1,327** at 390; the template's warm-up **503** (drawn 439). ⚠ Two facts on the phone that are stage 1's, stated for the walk (H3): the open block is **240px wide** (the frame's facsimile drew ~290 — the sheet keeps its own 1rem padding inside the shell's at 640), and the title field beside the three 44px head buttons is ~90px.

The frames as named before they were drawn:
...
### 5.10 Build record (2026-09-15)

- **Step zero was already done:** stage 1 had been committed as `6cff92e8` the night before; the working copy held only the stage 2
  rulings (this section, the hub tab, the brief) and a peer's Development stage 2 files. Built in one session, D1–D11 as drawn;
  no migration, no version bump, no new route. Dev server: no restart taken — hot reload served every edit, including the two
  shared modules (`lib/types.ts`, `lib/rep-practice-plan.ts`); the restart rule still applies before the owner's browser walk.
- **What was built, by decision:** `BlockCard`'s open body in the ruled order — the clock row (`QUICK_MINUTES` chips on the
  `ppSuggestChip` recipe via `composes`, `aria-pressed`; the minutes input; the Rest chip ABSENT while another block holds it;
  "ends …" / "runs to …" from `clock.endLabel` — no second arithmetic) · the two teaching fields under the station's labels ·
  the held sections (points as `CoachingPointsField`, one textarea split on newlines at patch time; Staff; Players reading
  **Whole team** with a quiet "Choose players…" link; the block's Equipment picker while it has no stations; the rotation; the
  stations section behind its door) · the doors line at the foot in the ghost row's idiom. `openDoors` lives in the EDITOR
  (one open block, one set), reset wherever `openId` is set and seeded from `blockHolds` so a field the coach empties stays
  on screen until the block closes; `teaching` is a latch in that set, not a door the line offers. The shut row reads its
  first line through a sole station (`resolveStationTeaching`), "N stations" only at two or more, Whole team / "N players".
  `blockAsksForTeaching` beside `blockRotates`. The five sheets on `useDialogFloor` (the roster picker's and the start-from
  picker's inline `modalOverlay`s took a panel ref; the three components mount the hook while open; Save to my drills and Save
  as template are busy-gated). The sheet's block line prints `Watch for:` and `Equipment: …`; its head prints the bag.
- **D11 as built — one pure pass, two callers.** `settleBlockKit(plan)` in `lib/rep-practice-plan.ts` moves a block's kit the
  moment it has stations (onto a written first station, that station's own ids first; up into the plan's list when the first
  station is a drill), idempotent, the same object back when nothing moves. The sanitiser runs it LAST — after `restrictTagIds`
  — and the editor runs it on every change to the blocks (`setBlocks`), so the coach sees the kit move with no reload. The
  bag is `practiceKitBag` (plan ∪ blocks ∪ stations as names, each once; `fromBlocks` for the About fold's "from the blocks
  below" line); `restrictTagIds` / `repointPracticePlanTags` / `collectPracticePlanTagIds` reach the block level; the
  dictionary's plan shape names the key and invariant 5 states the move. The block's players are still DELETED at the same
  moment — recorded beside the move, sent to stage 3 as its D8.
- **/simplify (four lenses) — applied:** one `toTagMap` for the two id→name walks (the bag builds it once); `unionIds` retired
  for `strList`; `blockHolds` is the one "what does the block hold" read by the door seed and the show flags; each branch of
  `BlockCard` computes only what it renders; ONE tap-floor rule on `.ppTlOpen` for every field in an open block (the title in
  the head sat at 35px until the gate first opened a block). Skipped: the print path's second walk (once per Print click).
- **/review (high-risk; correctness · security/tenancy · regression · concurrency/autosave · business rules + a11y; 9 → 7 → 3
  confirmed and FIXED, 1 refuted, 4 advisory):** kit typed on a block VISIBLY VANISHED when a station arrived (the browser
  never moved it, only the server did, and the save's response is not applied) — the one defect D11 was written to prevent,
  raised by three lenses → `settleBlockKit` in the editor too; the kit union was capped BEFORE the library check (a stale id
  could take a live one's slot) → the pass runs after it, tested; the door seed tested points by length where the predicate
  tests by content → one test. Refuted: the teaching latch keeping empty fields mid-session (deliberate). Advisory, recorded:
  templates store kit ids unvalidated at every level (pre-existing, mig 266); the response-not-applied architecture; two
  other runners skip the sweep's new gesture; the promote dialog's busy flag has unguarded close buttons.
- **The shared floor's autoFocus bug, found on "Save as template…":** `useDialogFloor` recorded the restore target inside its
  effect, AFTER React's autoFocus had moved focus into the dialog — Escape then "returned" focus to a detached input. Every
  floor with an autoFocus field had it. Fixed in the floor with a two-deep focus history from one document `focusin` listener
  (render-time ref reads are refused by the hooks lint; layout effects run after autoFocus).
- **The gate learned to open a block:** `scripts/layout-screens.mjs` entries may carry `interact(page)`; the plan page and the
  template editor open their first block before measuring (the §4.5 blind spot, closed). First run found the title input at
  35px on phone and tablet — fixed as above. Baseline: no new rows, none to prune (the two screens' four rows are the shared
  masthead "?"). The fixture's Probe drill was bare (no words, no kit) so a drill-placed row could show nothing — the seed now
  gives it the frame's words once (an existing bare row is updated; a written one is left alone).
- **/docs:** the Practice plans article's block sub-topic reads as the block is built (a short list: the clock row, the two
  questions, Players, the doors), the "people live in one place" note gained its equipment twin, the About paragraph explains
  the bag, the staff/equipment section names the block's kit; every new phrase is searchable. No new section, no anchor rename.
- **Gates at hand-off:** `verify:changed` green (4,010 unit, spelling, tokens, purity, dead selectors, dictionary, demos) ·
  `typecheck` clean · `check:layout --only=coach-practice-plan,coach-development-template` at 361/390/768/1440 with the block
  OPEN — no new findings · a 36-step Playwright probe on the Oct 27 fixture practice (restored afterwards; two equipment tags
  seeded and deleted) · the practice vocabulary guard (`tests/unit/practice-vocabulary-guard.test.ts`).
- **Calls made without a ruling, each a one-line reversal:** the "+ Stations" door opens the section (its "+ Add a station"
  then opens the sheet — one tap more than the other doors; opening the sheet directly would unmount the link Escape must
  return focus to); the read-only viewer keeps disabled fields as today with no doors and no chips (the read-only face as text
  is §5.2's narrative, not a ruling); "N stations" is absent at one station (frame note 18: with one station the station is
  the block).
- **Stated for the walk:** the block is 240px wide on the phone and the title field ~90px (stage 1's sheet and head — H3);
  a drill-placed row needs a drill with words (the fixture's has them now).

### 5.11 Three owner revisions on the build (2026-09-15, during the §190 walk)

The owner opened the written warm-up and the roster picker on the warm theme and asked three things. Two were build misses
against the drawing; one was a ruling reversed on sight. All three built the same hour, no migration, no new API.

1. **The open block was all one tone — a build miss against §5.9's frames.** The drawing put every field WHITE on the block's
   cream, the names as white pills, the "6 players" button solid, and the pressed minute chip filled with ink. The build reused
   the portal-wide field ground, which on the warm theme *is* the paper tone — so the fields sat paper-on-paper with a hairline
   between them and the card, the minute chips came from the dashed *suggestion* family (a pressed chip only firmed its border),
   and the names were cream boxes. Built as drawn: one rule lifts every field inside an open block to the card tone (the dark
   theme lifts to its own card tone the same way); `.ppChip` is a white pill; `.ppPickBtn` is solid; `.ppQuickChip` is its own
   recipe (white pill, solid edge, ink fill when pressed) and no longer composes `ppSuggestChip` — the 768 touch floor names it
   beside the suggest family. Verified in the browser: card `#F8F4ED`, fields `#FFF`, pills `#FFF`/999px, pressed chip
   `#241E15` on white.
2. **The way back to "Whole team" was twelve taps.** D1/note 28 said "clearing every name returns it to Whole team" and the
   walk's B4 said "untick both, Done" — true, and the only way; with a whole roster ticked, twelve unticks. Built: the roster
   picker's footer carries **Whole team** beside Done on a BLOCK's picker while names are chosen — one press clears every name
   and closes the picker on the answer. Absent on a station's or a group's picker (no "whole team" to return to) and absent
   while the block already reads Whole team. Deliberately NOT built: collapsing "all twelve ticked" into Whole team — a named
   list of twelve stays those twelve when a thirteenth joins, which is the difference the word carries; the button makes the
   choice one tap instead.
3. **The picker no longer prints a player's focus areas under their name (ruling reversed: D27, 2026-08).** "First-step
   quickness off the bag" under Devon read as a stray note — on the warm theme the chip's edge mapped to white-on-white, and
   one line under twelve names has no label to say what it is. The owner's call: choosing who is at a block is a roster
   question; the rail under the plan ("What everyone's working on") is where focus areas are read. The chip, its data and the
   Development grant that gates it are untouched everywhere else (the rail, the printed sheet).

Gates after the three: `verify:changed` green · `typecheck` clean · `check:layout --only=coach-practice-plan,coach-development-template`
with the block open at 361/390/768/1440 — no new findings · a Playwright probe on the probe practice at 1440 and 390 reading the
computed tones and the picker's footer (`Whole team · Done`, zero focus chips). Walk steps A2 and B4 revised on the hub; the
stage-2 tab carries the three under "Owner revisions on the build".
...
| | Decision (as drawn) | Ruling |
|---|---|---|
| D1 | A block with exactly ONE station flattens into the block — no "Stations" head, no numbered card, no second trash. The drill's provenance line and read-only words sit where the block's own words would (*Edit just for this practice* · *Swap drill* on that line); who runs it, who's at it and tonight's note become the block's own lines (a door each until set); the kit is the block's. A second station un-flattens it back into columns; title and minutes never detach the drill. | Build as drawn |
| D2 | Stations as columns under their block on the desktop (name · who runs it · tonight's note · the first line of what happens there), rows on a phone; "+ Add a station" the last column. Three columns ≈ 230px, four ≈ 170px on the 816px sheet; five and more wrap to a second row of columns rather than shrink under the tap floor. | Build as drawn |
| D3 | The rotation is ONE line between the block's words and the columns: "Groups rotate" as a pressed chip (un-press it and stations run separately, each with its own Players line), *every N min* typed once, the rounds it makes computed and stated honestly ("3 rounds of 15 = 45 min"; "45 does not divide by 20 — 2 rounds and 5 min over", never tidied), and the draw. The stacked rotation panel goes; the group list and the grid move under the columns. | Build as drawn |
| **D4** | *Drawn 15 Sep as:* a station column opens by widening in place, the others folding to their heads. *Revised same day, after the owner's read of the built draw:* **a station opens as a MODAL**, with a footer stepper ("‹ 1 of 3 stations · Close control ›") carrying "what else is running tonight" instead of the layout — the same pattern the fundraiser drive drawer already uses. See §6.4 for the reasoning. | **Build as REVISED, not as first drawn** |
| D5 | The draw is ONE control on the strip — "3 groups · from who replied · Draw"; the count, the pool (who replied yes, or the whole roster when attendance is unknown, as today) and the button in one place; press again to re-draw; groups appear under the grid with an *edit* into the same Choose players picker. Deliberately dumb — a shuffle and a deal, never sorted by ability. | Build as drawn |
| D6 | The grid's columns are the station columns — rounds as rows with their clock, stations as columns in the same order and width as the cards above, each cell the group there then. A station's "Starts with" is the first row, so the column does not repeat it. | Build as drawn |
| D7 | Coaching points on a station — and in the drill library's editor — become ONE field, one per line (stage 2's D10, finished). Stored exactly as today. | Build as drawn |
| D8 | People move the way kit does when a station arrives — never silently deleted (recorded by stage 2, §5.11 kit-move item; ruled here for people). The block's players move onto the first station (a drill station holds people too); turning on rotating makes them the first draw's pool. Nothing typed vanishes. | Build as drawn |

**Not this stage (unchanged):** the drill library's rows and the docked panel (stage 4); drag (S.4 — the owner's call at stage 4; arrows until then); the run screen's station view and the printed grid's drawing (stage 5).

### 6.1 What the walk saw, and what the code says

Measured on the fixture's three-station circuit (Skills circuit — Warm-up · Skills circuit · Small-sided game, three stations rotating every 15 min): the open block is **4,083px tall on the desktop and 4,819px on a phone** today — the rotation's controls (every-N-minutes, the draw as three separate controls, the group list with pickers) stacked above three full station cards stacked above the rounds×groups grid, every station showing all nine of its fields whether or not it has content, and the "who starts here" answer written three times (the station's own "Starts with" line, the grid's own first row, and the group list). A station card is the block's whole anatomy again, one level down — three of its nine fields (what you're doing, watching for, coaching points) are the block's own questions asked a second time. The grid is the one thing on the page a shared document genuinely cannot do (rounds × stations with the clock on each round, never tidied) and it sits a screen and a half below the block's own title. With exactly one station, the run screen already reads the station AS the block (`resolveStationTeaching`); the editor does not yet, and stage 2 stopped short of ruling it so this stage could.

### 6.2 What a coach sees and does differently (the proposal)

The circuit's title, clock row and two teaching lines stay exactly as stage 2 built them. Under them: one line says whether the block rotates, how often, how many rounds that makes, and holds the draw. Under that, the stations stand side by side as named columns — at a glance, who is running what and what tonight's note says for each, without opening anything. Tapping a station (D4, revised) opens it as a dialog over the page, full width, with the same fields the block itself asks for; a footer lets a coach step to the next or previous station without closing anything, the same "‹ 1 of 2 · Bottle drive ›" control already used on a fundraiser drive. Under the columns, the rotation grid uses those same columns as its own — reading down one column is that station's whole evening; reading across a row is where every group stands at a given time. Groups are listed and edited under the grid, the one place people live in a rotating block.
...
- **D6** rides: the grid's orientation (today groups are columns and rounds are rows with station names in the cells) — `computeRotation` untouched, the same cells, turned.
- **D7** rides: the station's and the drill library editor's points control; no model change.
- **D8** rides: the sanitiser's one remaining silent delete becomes a move (the same pure pass D11 gave kit at stage 2); the help system's "you'll see the names move" sentence becomes true for people too, not just kit.

### 6.4 The D4 revision — why the drawn frame was not what got ruled

Drawn on 15 September, D4 read the same way every other "open in place" moment on this sheet reads: a station column widens where it sits, its siblings folding to quiet heads beside it. Reviewing the built draw, the owner asked why a station couldn't just open as a modal instead, given how much of the card's height was going unused beside two collapsed columns. The frame's own justification — "a row opens in place so a coach can see where they are in the sequence" — was checked against the actual claim and did not survive it: that reasoning is true of a BLOCK, which really does run before and after its neighbours in time, and false of a STATION, which merely shares a clock with the stations beside it and has no other relationship to them. Once the one reason for keeping the layout in-line was gone, the modal was strictly better for a card this deep (nine fields): no dead columns, no inline scroll competing with the page's own scroll, full width to work in. The one thing a modal owes back is "what else is happening tonight," which the sheet was answering through the layout — the fix is a footer stepper doing the same job the fundraiser drive drawer's own "‹ 1 of 2 drives · Bottle drive ›" control already does, so this is a second application of an existing pattern, not a new one. **Ruled: build the modal (with the stepper), not the widening column** — the hub's "3 · Stations and the rotation" tab carries both frames, the superseded inline draw and the modal that replaces it, so the record shows what changed and why.

### 6.5 Verification (this stage, once built)

`npm run verify:changed` · `npm run typecheck` · the unit suite (the people-move rule mirrors D11's kit-move pattern; a new test for the modal's stepper wrap-around at both ends of the station list) · `npm run check:layout -- --only=coach-practice-plan,coach-development-template` **with an open-station interaction step added to the sweep** (a modal is a new overlay surface `check:layout` has not measured on this page before) at 1440 · 768 · 390 · `check:demos`. The modal joins whatever shares `useDialogFloor` today (stage 2, D9) — confirm Escape and focus-return behave the same way a station modal as every other sheet on this page.

### 6.6 The frames — DRAWN 2026-09-15, REVISED 2026-09-15 (D4), on the hub's "3 · Stations and the rotation" tab
...
**DRAWN 2026-09-16 on the hub's "4 · The library" tab (fifteen frames at true size, notes 54–79 and 86–90, decisions L1–L9 with a paste-back) · RULED 2026-09-16 (§7.0) · BUILT ON DEV 2026-09-16 (§7.8) · COMMITTED `34bb88f5` 2026-09-16 · the walk is the hub's "QA walk · 4" tab, ledger §195. The two stage-4 prompts (planning, build) were deleted with the commit.** The stage's scope is the ladder's
...
## 8. Stage 5 · Paper & the field — DRAWN 2026-09-17 on the hub ("5 · Paper & the field" tab) · RULED 2026-09-17 (P1–P9 all as drawn, §8.0) · BUILT ON DEV 2026-09-17 (§8.7 the build record; ledger §201; the hub's "QA walk · 5" tab) · walk owed · COMMITTED `fe2d8244` 2026-09-17

Walk stations 7 (the printed sheet) and 8 (the field). Drawn on the hub as the tab after "QA walk · 4": fourteen
frames (notes 104–134), the befores as the live screen and the rendered sheet at true size, the afters in the
portal's materials and the sheet's ink; asks `P1`…`P9` (the letter keeps their state and ids apart from every
earlier tab; `dLabel` passes `P` through; the paste-back is `#buildSummaryStage5`). Nothing here changes what a
practice stores and nothing writes at the field.

### 8.0 The rulings (owner, 2026-09-17, pasted from the artifact)

| | Decision | Ruling |
|---|---|---|
| P1 | The paper's grid turns — stations across, one row per round with its clock, the group in each cell (stacked when two share), a "Sitting out" column only when a round has one, the sideways fallback kept — and a heading wraps rather than overrunning. | Build as drawn |
| P2 | One renderer: the jsPDF sheet is the print of the screen's document; close now the grid, each station as a labelled block, "Whole team", "Arrive 17:45" → "Arrive 5:45 p.m."; paper's own rights kept (the stacked clock, atomic blocks, the band and true page totals, the focus section by grant, a shut block's words, no doors or pills). | Build as drawn |
| P3 | One window (±3h, at least one block), every door — the Schedule slide-over reads the hub's condition from a minute clock; outside the window the run screen reads "Planned for Tue, Sep 15 · 5:01 p.m." / "… · the clock starts 2:21 p.m." in the clock's place; the words, Back and Next stay. | Build as drawn |
| P4 | A lime Run practice first in the plan page's toolbar while the window is open — the page's one lime once blocks exist — and the toolbar as today outside it. | Build as drawn |
| P5 | "Whole team" on the field for a block that names nobody and for one that names every player on the roster; chips for a few. The paper prints "Whole team" where it printed nothing and a coach's list as written. The plan page unchanged. | Build as drawn |
| P6 | A plain block's staff · players line on the field under its words ("UAT Coach · Whole team"); a rotating block's own staff on the same line under the rows. | Build as drawn |
...
**This stage:** the printed sheet (`lib/export/pdf.ts` · `buildPracticeRunSheetDoc` and what `handlePrint` hands
it); the run screen (`practice/[eventId]/run/page.tsx`) including its face outside the run window — before and
after; the doors (hub card and rows, Overview card, the Schedule slide-over) and whether the plan page earns one
back; the words for "everyone" on the field and on paper; who runs a block, on the field; the rotation panel's
restyle under a hand-arranged grid (§6.11 handed it here); a now-marker on the sheet during the window.
**Stage 6's, untouched:** "How it went" (renders once the practice has *started* — the start time passing,
`practiceStarted`; nothing finer); the record's face for a past practice (the May practice still opens today's
editor with its ghost row); the closed-season shelf and its reader.

### 8.2 The inputs, answered from the code and the screens before drawing

| # | Input | What the code does · what the screen showed (17 Sep, UAT fixture) | Answer |
|---|---|---|---|
| 1 | The paper's grid turns (ruled 16 Sep) | `handlePrint` assembles `groupNames` across and `rounds[].stations` in the cells; `drawRotation` draws that; the screen has read stations across since D6 (`rotationByStation`). Rendered and read back: the cell "Footwork ladder" (21.9mm at 8pt) already touches "Finishing" in its 22.3mm column. Turned, station names become HEADINGS — and `gridFontFor` measures a heading's widest *word* while `drawGrid` prints the whole heading unwrapped, so "Footwork ladder" passes the rule and overruns by 2mm. Measured with the renderer's Helvetica (`proseW` 155.7mm, 133.7 after the 22mm round column): six stations at 8pt on the fixture; ~7 columns at 7pt when the widest word is 9 letters; 6 at 11; 5 at 13. | P1 — drawn as ruled, plus one thing the ruling could not know: headings wrap (two lines at the fitted size; the head row grows); the fit rule stays per-word. Sentences unchanged. |
| 2 | One renderer or two | The sheet is a second renderer of the same document, held by the rendered check (`check:pdf`, 23 documents, every page read back; the run sheet's fixture is typed in the OLD orientation). Print CSS would lose the identity band, true "Page X of Y", the atomic-block rule as a rule, the phone's download path, and the rendered gate. | P2 — keep the one jsPDF renderer as the print of the screen's document; the agreement is a list (order and words) the harness holds. Close now: the grid, the stations' lines, "Whole team", **"Arrive 17:45" → "Arrive 5:45 p.m."** (`event.arrivalTime` printed raw where the Schedule prints it through `fmtClock`; the clock rule has no paper carve-out; one line, `formatTime`). Paper's own rights kept: the stacked clock, atomic blocks, the band and footer, the focus section by grant, a shut block's words, no doors. |
| 3 | The door, once | Hub card + rows and the Overview read `practicePlanState` (≥1 block, `isInRunWindow` ±3h) from `useMinuteClock`. The Schedule slide-over (`schedule/page.tsx` ~2734) offers "Run practice →" whenever `selectedEvent.practicePlan` is truthy — any date, and a goal-only plan — under a hub-row comment claiming it uses the same condition. Live: the next practice, five days out, offers the door; its run screen reads "122:36:04 · LEFT OF 20 MIN"; the probe practice two days on reads "+44:33:48 · OVER BY"; May's "+3019:45:43". | P3 — one window, every door: the slide-over reads the hub's state from a minute clock; outside the window the run page swaps the clock block for the fact — "Planned for Tue, Sep 15 · 5:01 p.m." / "Planned for Tue, Sep 22 · 5:21 p.m. · the clock starts 2:21 p.m." Not "Ran" (D4: planned, never done). Words, Back and Next stay. The two empties stay. |
| 3b | The plan page's door | Removed at stage 1 because the builder offered it at any date; the page already reads `nowMs` per minute (for `started`); the toolbar exists once blocks exist; the ghost row's "+ Add a block" is quiet after the first block, so the page has no lime during a practice. | P4 — yes, inside the window only: a lime "Run practice" first in the toolbar (the page's one lime once blocks exist). The alternative (no door, two taps from the hub card) is drawn as the unchanged toolbar. Close call. |
| 4 | "Everyone" | A Whole-team block has an empty `playerIds`: the sheet prints the staff alone and the run screen prints no chips. All twelve ticked: twelve names on paper, twelve chips on the field. The plan page keeps "12 players" apart from "Whole team" by the owner's 15 Sep revision. | P5 — the word is "Whole team" (the plan page's, the picker's, the help's; not the walk's "Everyone" — S.5). The field prints it for an empty list and for a list equal to the active roster; chips for a subset. The paper prints it where it printed nothing; a coach's list prints as written. Not attendance-aware. |
| 5 | Who runs this block | The run page reads `station.staff` only (§5.7); the paper prints `block.staff · players`. | P6 — the paper's line on the field, under the words: "UAT Coach · Whole team". Not the eyebrow (the round's slot). |
| 6 | The rotation panel | A round renders one `ppRunWhere` card per GROUP (six), then a Stations list of one row per station (six) — the door to "my station". 1,319px at 390, Rotate 300px below the fold. Due: `dueMoves` joins every move into ONE amber sentence ("E → Close control · F → Footwork ladder · … · D sits out"). Found on the way: `.ppRunStationMeta` is an inline span with a dead top margin — "Close controlSam Assistant". | P7 — ONE list keyed by station (`rotationByStation`, the board's own re-key): letter(s) · station · who runs it · View; the row is the door; "nobody" for an empty station; a sitting-out group one line under the list; due: the amber head and the arriving letter per row. Six rows, not twelve (drawn 792px vs 1,319; due 913). The meta becomes a block. |
| 7 | The stations on paper | One prose run per station joined by " · ", wrapping mid-item ("Run by Sam / Assistant"). | P8 — a labelled block per station (name · Run by; Setup · Equipment · Tonight as lines; points as a list — the modal's order); never columns (52mm at three across; a list that changes shape with its count). Cost measured: the six-station fixture runs page 2 ~18mm past its foot, so the game moves whole to page 3; the seeded three fit on two. Alternative named: points on one line. |
| 8 | The now-marker | The page reads a minute clock and `computeBlockClocks` already; a marker is one word in the gutter. It is the *plan's* now — the field is anchored by taps, the sheet by the plan; a late start makes them disagree by the minutes behind. | P9 — this stage, minimal: "now · 5:37" in the running block's gutter with the spine's dot filled; only inside the window and while a block's planned window holds the clock; opens, scrolls, writes nothing; gone when the planned practice ends. Alternative: the sheet as it is (stage 6 or never). Close call. |
| 9 | The helper's face | `_PracticeStationView` reads the station as stages 2–3 left it (teaching falls through to the block; points as one list; kit as tag names; Running it; Coming to you with the clock); a helper gets the sentence. One label drifts: "Note for tonight" on the field vs "Just for tonight" (the editor's field) and "Tonight:" (the column, the paper). | Stated, not restyled; the label takes the editor's. |
| 10 | The help and the demo | `#premium-practice-plans` "Then run it" says Run practice is NOT on the plan page; `#premium-practice-run`'s second paragraph says "You'll find it on the plan itself, and on the practice in your Schedule". The coach demo's tour stops on neither screen (checked `lib/sandbox-chrome.ts`). | Both paragraphs change together with P3/P4; the rotation subtopic and the print sentence change with P7/P8; no tour anchor moves. |

**Kept, confirmed on the code:** "This practice has no start time" (`steps.length === 0`); a goal-only plan has no
toolbar and nothing to print (the gate is blocks); an unfinished rotation prints its sentences without a grid (the
harness's `unfinished-rotation` case); the date and block clocks are in the org's zone and on the product's clock —
only the arrival time is not; the atomic-block rule (the fixture's circuit moves whole to page 2, leaving page 1 to
the warm-up).
...
|---|---|---|---|
| P1 | The grid turns: stations across, the clock down the side, groups in the cells (stacked when two share), a "Sitting out" column only when a round has one, the sideways fallback kept — and a heading wraps rather than overrunning. | Ruled in words 16 Sep; drawn as ruled — confirm on sight (frames 111 · 113 · 114). | `PracticeSheetRotation` → `stationNames` · `rounds[].groups` (stacked) · `rounds[].out`; `handlePrint` assembles from `rotationByStation`; `drawGrid` learns a two-line heading and a stacked cell; `measureBlock` both heights; the harness fixture turns. |
| P2 | One renderer: the jsPDF sheet is the print of the screen's document; the agreement is a list; close the grid, the stations' lines, "Whole team", the arrival clock; keep paper's own rights. | A (recommended). B, print CSS: refused for the reasons in 8.2 row 2; not drawn. | `formatTime(event.arrivalTime)` in `handlePrint`; the harness fixture types a raw "17:45"; the renderer's header names the agreement. |
| P3 | One window, every door; outside it the run screen reads "Planned for …" in the clock's place. | Yes, as drawn (124 · 125 · 127 · 128). | The slide-over reads `practicePlanState` from `useMinuteClock` (its game-day pill may join the clock); the run page reads `isInRunWindow`; the hub row's comment corrected; the help's two paragraphs agree. |
| P4 | A lime Run practice first in the plan page's toolbar, inside the window only. | Yes (131); the alternative (130) is today's toolbar. Close call. | `practicePlanState === 'run'` on the page; the one-lime rule (probe: no second lime once blocks exist); the help's "Then run it". |
| P5 | "Whole team" on the field (empty list, or every active player) and on paper (where nothing printed); chips/names for a subset; the plan page unchanged. | Yes, as drawn (112 · 122). | The run page compares `playerIds` to the active roster; `handlePrint`'s players field; help. |
| P6 | A plain block's staff · players line on the field, under the words. | Yes, as drawn (122). | `block.staff` on the run page. |
| P7 | One list keyed by station, each row the door; "nobody"; a sitting-out line; the due state per row. | Yes, as drawn (117–120). Not chosen: rows by group with View folded in — an empty station would have no door. | `rotationByStation` on the run page; `chooseStation` on the row; `dueMoves` per row; `.ppRunStationMeta` a block; `practice-run-layout.spec.ts` learns the row; help. |
| P8 | A station on paper is a labelled block, never a prose run, never columns. | Yes, as drawn (110); the page cost stated. | `PracticeSheetBlock.stations`; the renderer draws and measures it; the harness fixture carries three stations with every field. |
| P9 | "now · 5:37" in the running block's gutter during the window, by the plan. | Yes, this stage, minimal (132 · 133). Close call; the alternative is the unchanged sheet. | The editor's gutter reads `nowMs` against `computeBlockClocks`; window-gated; no autosave, no open block, no scroll. |

### 8.4 The frames, measured (true size, `[data-measure]` + a live `[data-hpx]` tag)

Paper (816 = letter at 96dpi): page 1's head 424 · page 2 after 1,127 (against a 1,056 page — P8's cost) · the
grid's three shapes 833. The field at 390: a round 792 (1,319 today) · due 913 · the plain block 478 · outside the
window 489 · 489. The doors: the Schedule section 126 · 126 · the toolbar 38. The now-marker: the docked pair's
sheet 664 at 1440 · 497 at 768. Befores: the run screen live at 390 (stops 1 and 4, the open state at +44h, the
May practice, the next practice at −122h); the rendered sheet's two pages (`probe-sheet.pdf`, read back with pdf.js).

### 8.5 Verification (for the build session, after the rulings)

- **The rendered check proves the turn** — `npm run check:pdf --only=coach-practice-run-sheet` reads the turned
  grid's headings back as text (stations across, a wrapped heading on two lines, a stacked cell, a "Sitting out"
  column in the hand-arranged case) and "Arrive 5:45 p.m." from a fixture typed "17:45"; then the full run at the
  pre-commit hook.
- A Playwright probe on the fixture (plan restored after): the run screen at 390 on the circuit — six rows, the
  row opens the station, "that's you"; the due state's rows (a hand-arranged round via the groups room, then
  forgotten); "Whole team" on the game block; the staff line; the probe practice out of window → "Planned for";
  the next practice → "the clock starts"; the Schedule slide-over with and without the door; the plan page's
  toolbar with the lime inside the window and without it outside (freeze the clock LAST — `page.clock.install`
  rotates the stored session); the gutter's "now" at 1440 and 768.
- `check:layout --only=coach-practice-run,coach-practice-plan,coach-schedule` at 361/390/768/1440;
  `practice-run-layout.spec.ts` (rows ≥44, buttons ≥56); `verify:changed`; `typecheck` (the export types change).
- `/docs` on both practice articles; `/review`; the demo's tour-anchor check runs as always (no stop moves).

### 8.6 Not this stage

"How it went" and when it shows; the record's face for a past practice; the closed-season shelf and reader (all
stage 6); a wake lock (an explicit fast-follow); attendance on the field beyond the fold; anything written at the
field; print CSS (refused in P2); the library (stage 4, done).

### 8.7 Build record (2026-09-17) — BUILT ON DEV, walk owed (§201), COMMITTED `fe2d8244` 2026-09-17 on the owner's word

Built as ruled, P1–P9 as drawn; **no migration, no new route, nothing written at the field.** One visible difference
from the frames, flagged before the build: the now-marker reads **"now" over "5:37 p.m."** on two gutter lines
rather than the frame's one-line "now · 5:37" — the clock ruling (settled, gated, no carve-out) puts the period on
every time a coach reads, and "now · 5:37 p.m." is wider than the 88px gutter. A one-line change if the owner wants
the bare clock.

**What was built, where.**
- **The paper (P1 · P2 · P5 · P8)** — `lib/export/pdf.ts`: `PracticeSheetRotation` turned (`stationNames` ·
  `rounds[].groups` stacked · `rounds[].out`); `PracticeSheetStation` (name · runBy · words · facts as label/value
  pairs · points) and `PracticeSheetBlock.stations`; `gridFit` decides the size AND the wrap together (per-word fit,
  then `splitTextToSize` to the column, at most `RUN_GRID_HEAD_LINES` = 2 — a third line sends the grid sideways);
  `layoutGrid` is the ONE layout both `drawGrid` and `measureBlock` read (the head row grows by `gridLineH(size)`
  per extra heading line, a row by the deepest stacked cell — the 2026-08-24 overflow was a second copy of this
  arithmetic); `rotationGrid` adds the "Sitting out" column only when some round has a group out, and the sideways
  fallback prints "Station" as the row head with a "Sitting out" ROW; `stationLines` is the one line-writer both
  `stationsHeight` and `drawStations` read (name bold with " · Run by …" muted on the same line, then indented
  labelled lines — the label in the muted ink, the value in the prose ink — then "– …" points; a bare station is one
  bold line); the renderer's header names the agreement with the screen and paper's own rights. `handlePrint`
  assembles the grid from `rotationByStation` (by station id, never a cell's position), each station as a block,
  "Whole team" for a block that names nobody (`blockOwnPeople`), and **`Arrive ${formatStoredClock(arrivalTime)}`**
  — the stored "17:45" printed raw since the sheet existed; `formatStoredClock` (the Schedule's shape guard, lifted
  into `lib/utils.ts`) now also reads the Overview's call-time chip and the game console's "Arrive" chip, which
  carried the same drift.
- **The doors (P3 · P4)** — the Schedule slide-over offers "Run practice →" on `practicePlanState(selectedEvent,
  nowMs) === 'run'` from `useMinuteClock` (the game-day pill joined the same clock — the 2026-08-12 gap closed on
  this screen; its windows are computed once per season load and compared per minute); the plan page's toolbar
  gains a lime `Run practice` first while `practicePlanState === 'run'` (`data-testid="run-practice"`); the hub row's
  comment tells the truth now.
- **The field (P3 · P5 · P6 · P7)** — `run/page.tsx`: outside `isInRunWindow` the clock block is the fact ("Planned
  for" · "Tue, Sep 15 · 5:01 p.m." · the window beneath, or "The clock starts 2:21 p.m." from `runWindowOpensAt`);
  ONE list over `rotationByStation(grid, block.stations)` — `renderStationRow` (one row, two faces: `data-face`
  rotation/station; the letter(s) via `shortGroupLabel`, "nobody" muted, `data-count`), each row `chooseStation`;
  the due state keeps the amber head and reads the NEXT round's row (`turned.rows[stepRound]`) per station; a
  sitting-out group is one line under the list; a plain block's `.ppRunMeta` reads `blockStaff · Whole team | chips`
  (`namesWholeTeam` — a SET comparison against the active roster the route returns); a rotating block's own staff
  under the rows. `_PracticeStationView`: "Note for tonight" → "Just for tonight".
- **The now-marker (P9)** — `BlockClock.endMs` beside `endLabel`; `plannedBlockAt(clocks, nowMs)` (index — ids are
  not unique); the editor takes `nowMs` from the page and its gutter reads a third line on the block whose planned
  window holds the clock, only inside the run window; the spine's dot is the ROW's `::before` on the timeline's one
  `--pp-gutter-w` property (the drag gap reads it too — it had hardcoded 5.5rem and sat 12px off on a phone).
- **Lib** — `soleStationOf` and `blockOwnPeople` moved to `lib/rep-practice-plan.ts` as the one home for "the
  station IS the block" and "whose people are these" (the editor imports the former); `formatClockMs` exported
  (the clock walk's own face) for the now line and the fact; `formatStoredClock` in `lib/utils.ts`.

**Proved.** `check:pdf` renders the run sheet and FIVE edge cases (oversize block, unfinished rotation, no focus
grant, **hand-arranged** — a shared cell, an empty station, "Sitting out" — and **long-station-names** — eight
stations, sideways) under both looks: every heading whole on the page (R3 now lists the station names, "Sitting
out", "Station" and the round heads), nothing off paper or over the footer; read back with pdf.js: the head row on
one baseline (113.9mm), "Close control" / "Footwork ladder" wrapped onto a second head line, the rounds with their
clocks down the side, "—" for empty cells, the shared cell stacked with its row grown. The contract test's recording
fake records `x` now and proves the ORIENTATION (station names on the head baseline in the coach's order, group
names in the station's column on the round's line, no "Group" head), the wrap (second line in its own column, the
first round below it), the stack and the "Sitting out" column, the sideways fallback, the labelled station block in
order, "Whole team" as written — 125/125. The printed sheet from the plan page's own Print button, read back: "Arrive
5:45 p.m.", the turned grid, labelled stations, two pages on the three-station seed. Unit: `namesWholeTeam` (set,
never count), `plannedBlockAt` (boundaries; a block with no end never holds the clock), the per-station rows and the
hand-arranged shapes, `runWindowOpensAt` (opens exactly when `isInRunWindow` first says yes), `formatStoredClock`;
the vocabulary guard holds "Arrive" through the formatter, "Planned for", never "Ran", "nobody", the sitting-out
line, "Just for tonight". 4,151/4,151. A Playwright probe on the re-seeded fixture: the Schedule's door present on
the day and absent five days out (section 118px); the toolbar lime first and the page's ONLY lime (37px); the gutter's
"now" at 1440 and 768 (dot on the spine at 1440/768/390 = 83/95/87px); the run screen at 390 — three rows (61px
each), the row opens the station, "that's you", the plain block's "UAT Coach · " + six chips and "Whole team" on the
game (448px); the due state after a hand-arranged round 3 via the API (snapshot → arrange → probe → restore): "nobody
/ A B / nobody", "C sits round 3 out" (722px); May's "Planned for · Thu, May 14 · 7:00 p.m." (438px) and next week's
"The clock starts 1:39 p.m." (459px); no lime and no marker on next week's plan. `check:layout --only=` the run
screen, the plan page and the docked pair at 361/390/768/1440 — no new findings (the dot's first cut spilled 7px past
the gutter and the sweep caught it). `verify:changed` green; `typecheck` 0; focused lint 0 errors.

**/simplify** (four lenses; 14 applied): the station re-key returns the stations themselves; the outside-window
strings built once per event, not per second; one lib home for the sole-station and own-people rules; the two
hand-rolled clock formats read the lib's; the gutter width one property on the timeline; the Schedule's game windows
once per load; the wrap helper shared with the sideways fallback; dead attributes, inline styles, a duplicate class,
the caller-side fact filter, the Schedule's alias gone; the guard's call-shape assertions dropped. Skipped, said so:
a shared label map for the station fields (the paper prints "Tonight" as drawn); "Run by" as a labelled line (P8
puts it beside the name).

**/review** (high-risk funnel, four lenses — correctness · blast radius · clocks/state · gate integrity + privacy; **31 found → 19 after dedup → 17 confirmed, 12 fixed in code, 5 accepted with a reason, 0 refuted**). Fixed: **the run window's trailing edge** — it was ±3h of the START alone, so a four-hour practice lost its doors and its field clock an hour before it finished; the one rule now runs to three hours after the END (or the start, with no end), every caller passes the end, three tests pin it; the station view kept the raw counter outside the window (a remembered station opened straight onto "+44:33:48") and the amber due state could fire over the fact — outside the window there is no clock anywhere on the screen now; an UNNAMED station in a rotating block lost its door (the list read the named stations only) — every station is a row, its column found by id; the renderer measured a block's notes at 8pt and drew them at 9pt (the August under-measure class, pre-existing) — measured at the draw size; the grid's CELLS were never width-fitted — the coach's group names now join the fit and wrap to the column, the row growing; a heading joined by a non-breaking space passed the per-word test and would have been chopped by the wrapper — both split on the same space; a round with fewer cells than stations slid the sitters into a station's column — padded by index; the paper said "Whole team" for a block naming only players who had since left, and printed nothing for a sole-station block naming nobody where the field says "Whole team"; the vocabulary guard's function bound swallowed the page's whole render; the lifted clock guard rejected "HH:mm:ss"; the layout spec's clock probe gained a time dependency (it skips with the reason outside the window). **Gates hardened:** the wrap test proves the head row GREW (against a control) and no heading reaches its neighbour; the stack test proves the row grew; a second footer-floor sweep walks a six-high stack + wrapped heading + "Sitting out" down the page; **a new rule R7 in `check:pdf` fails any two runs that print over each other** (clean on all 23 documents; the hand-arranged fixture carries a three-high stack under a round for it) and two new mutations — a frozen stacked row, an unwrapped unfitted heading — go RED by name (13/13). Accepted: the sheet's now-marker and the field disagree at the one instant a block with no length shares its start with the next (a block with no window cannot hold the clock); a sole-station block's staff reads on the meta line and on its one row; "The clock starts 11:30 p.m." names no day for a practice before 3 a.m.; the minute clock's up-to-59s lag (the hook's); the slide-over's event snapshot not re-syncing while open (pre-existing).

**Measured against §8.4** (the seed has THREE stations, the frames drew six): the run screen's rotating stop 592
(drawn 792 for six rows; 1,319 before), due 722 (913), the plain stop 448 (478), the fact face 438/459 (489), the
Schedule section 118 (126), the toolbar 37 (38).

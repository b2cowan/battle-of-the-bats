...
> **Status:** PLANNING — created 2026-07-04. Planning-only; no code, no migrations. **Owner decisions #1, #2, #3, #9 ACCEPTED 2026-07-04** (see §7); #4–#8, #10, #11 still open. Awaiting the remaining sign-offs before an implementation session begins.
...
| **2** ✅ | **Standings aggressiveness + gating** — **ACCEPTED 2026-07-04: Conditional tile + view.** | Always-on tile · Conditional · Off | **Conditional tile + view; Premium; also mirror a lighter link on Basic tournament portal** | More surfacing = the biggest wow; but a team that only scrimmages has no standings — gate on a standings-bearing context (an active/complete tournament pool or a league division with peer teams) or it shows an empty/confusing tile. |
...
| **4** | **History Overview preview tile** | Yes · No | **Yes** | A last-season preview makes resonant data proactive; costs one tile + a small history-summary read. |
| **5** | **Tryouts hide trigger** | Always show · Hide unless run · Explore-only | **Hide unless a tryout signal exists** | Cleaner nav for the majority; needs a reliable "runs tryouts" signal (event presence or per-team flag). |
| **6** | **Tournaments hide trigger** | Always show · Hide until first registration | **Hide until ≥1 external registration** | Removes a dead-end for rep-only coaches; keep rediscoverable so it's not "lost." |
| **7** | **Settings split** | Keep mixed · Light split | **Light split:** Settings = config (division, org link, lineup rules); surface "Start next season" from Season/Overview at the boundary (also keep in Settings) | Cleaner mental models; more than one surface touches "start next season" (acceptable — it's a moment, not a page). |
| **8** | **"Accounting" → "Money" rename + Overview surfacing** | Keep name · Rename + surface | **Rename + surface overdue on Overview** | Plain language for non-tech coaches; requires copy + a help-doc touch. |
...
### Phase 2 — Standings (the quick-win wow; new route, no migration)
- New coach-side standings route over the existing engine (`getStandings`/`computeTournamentStandings`), scoped to the team's current tournament pool and/or league division; **Overview rank tile** + a **Standings view**.
- **Coach gains:** "where do we stand?" answered in-portal for the first time. Highest wow-per-effort.

### Phase 3 — Navigation rebuild (Premium; IA-only, no migration)

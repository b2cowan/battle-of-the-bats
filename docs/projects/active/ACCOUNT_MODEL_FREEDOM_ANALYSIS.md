...
- **[✅ CLOSED 2026-07-24 by `c36ac94c`, confirmed on `origin/master` — verified 2026-07-28. The finding below was accurate when written; do not act on it.]** ~~`/api/league/create` has no one-org guard at all~~ — it's blocked only by the `LEAGUE_STARTER_BETA` flag being off in production (it is *on* in dev). When League Starter launches, this is an open second-org door. *(`app/api/league/create/route.ts:96-133`)*
...
4. **[✅ DONE 2026-07-24 — `c36ac94c`, in production. Residual: league-create still lacks the *pending-invitation* check org-create has.]** ~~`/api/league/create` has no one-org guard~~ — close it before `LEAGUE_STARTER_BETA` is ever flipped on in production. *(High)*

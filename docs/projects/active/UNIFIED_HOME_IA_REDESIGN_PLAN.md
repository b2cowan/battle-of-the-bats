...
| Organization | only as a secondary match field on tournament rows | new `searchOrgsForDirectory()` + **new `organizations.list_in_directory` boolean** (opt-out, default true for active non-canceled orgs; migration + Data Dictionary + snapshots in the same unit of work). Derived-from-tournaments proxy rejected — it would make League/Club orgs with no one-off tournaments unfindable |
...
- **Phase 2 — Search expansion:** org `list_in_directory` migration (+ dictionary/snapshots), `searchOrgsForDirectory`, `searchTeamsForDirectory`, unified search API, Teams chip.

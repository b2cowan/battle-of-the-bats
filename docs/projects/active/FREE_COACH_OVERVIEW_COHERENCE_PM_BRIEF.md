# PM brief — Free coach Overview coherence

**Status:** review complete, awaiting a decision. Nothing built.
**Plan:** `FREE_COACH_OVERVIEW_COHERENCE_PLAN.md` · **Mockups:** `claude.ai/code/artifact/8efbb388-a58c-40b9-8377-62b36f140bde`
...
free coach's Overview had the same problem. It doesn't — but it has one of its own.
...
times on one screen** — as a row, again as a card right below it, and again as a number in the
strip — plus a fourth time in the header bar that's always on screen. Nearly half the first
...
then five boxes reading zero, zero, none, $0.00, zero — and the card that would actually help them
...
- **The Overview names one tournament — the one that matters right now** — with the link to its
...
The free portal is the acquisition funnel — every public tournament registration creates one. A page
...
- **The full tournament list leaves the Overview.** A coach with several entries has one extra tap
  to see them all. Judged worth it — the list has a permanent home in the tab row, and the Overview
...
**Medium — small.** One pass, mostly ordering, layout and wording. No database change, no new
...
3. A brand-new coach sees "Let's set up your team" without scrolling (today: 82–186 px below the
   fold depending on the phone).
4. The tournament block names the event being played whenever there is one.
5. No change to what the coach can do or reach — this is about what they see first.

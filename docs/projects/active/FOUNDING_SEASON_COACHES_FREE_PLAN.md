...
## Phase 2 build log (2026-07-20)

Phase 2 BUILT + reviewed (standard tier, 2 lenses: correctness + blast-radius). P7/P12 via physical move of app/start → app/(consumer)/start (URLs unchanged; tab bar mounts in dark skin since /start ∉ CONSUMER_SHELL_PREFIXES; SiteChrome suppresses the marketing Navbar via explicit /start match — deliberately NOT added to the prefix list, which also drives warm-theme/footer classification). P8 Footer STATIC_ROOTS += four for-* segments. P13 signed-out heads-up screen at /start/tournament. P9 account-only 409 → account_exists inline notice (verified: the only 409 on that path). P10 signed-out "Run a tournament" → pinned quiet note (mirrors signed-in door). P11 comments corrected. Review fixes: start.module.css .page dropped min-height:100vh + vertical centering (auth.module.css shell pattern — trailing links no longer hide behind the tab bar); route-list guard comments added. Accepted: added auth/context lookups on /start for signed-in users (matches the accepted /auth//account pattern); one stale UAT-notes artifact re old chrome. Post-move gotcha for the record: three form components imported the auth stylesheet by stale relative paths — tsc can't catch CSS module paths; caught by route smoke-check (all /start* + controls 200).

## Post-launch: warm restyle of /start (S1-1, ratified 2026-07-20)

Design round S1 (artifact `c6f4c1e1-32b6-43fb-b17a-e9fae1e59ad0` rev 2 = binding spec; logged in memory/design_decisions.md): the /start family goes WARM (R1-4 tokens: paper/white 14px cards/ink/olive) **after Phase 3 ships**. Riders shipped immediately with Phase 2: compact three-column cards at every width (mobile full-width arrow variant removed from start.module.css) and League/Club cards removed from the chooser (League re-adds itself when `LEAGUE_STARTER_BETA` is on — see the `LEAGUE_OPTION` block in `app/(consumer)/start/page.tsx`; Club returns via its own decision). Restyle scope at build: start.module.css warm rewrite + tournament heads-up + team/league/club sub-pages; structure/copy/routes unchanged; admin untouched.

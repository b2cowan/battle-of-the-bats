...
**Status:** **Phases 1 + 2 BUILT 2026-06-09 · Phase 4 BUILT 2026-06-10** (on branch `feat/free-tier-coaches`; **mig 122 applied to dev AND prod 2026-06-10** — owner-approved; extension + 2 cron jobs verified live on prod, anon execute denied) · **Phase 3 (feedback widget + triage + Mechanism-A requestId plumbing) BUILT 2026-06-10 — see §15** · **Phase 5 ❌ CANCELLED 2026-06-10** (in-bell alerts not needed; route-coverage sweep moved to OBSERVABILITY_ROUTE_INSTRUMENTATION_PLAN.md; Sentry stays an additive non-planned option)
...
- **Dev vs prod separation:** every row carries `env`. Belt-and-suspenders — data physically lives in separate Supabase projects (dev `fieldlogichq-dev` vs prod `qcttcboqysynwcdyghil`) **and** is tagged by `env`. **Update 2026-06-26:** the dashboard's Production/Dev toggle was removed — the separate-projects boundary IS the environment boundary, so each deployment's dashboard shows only its own DB's rows. The `env` column is still written (and shown on the issue detail) but is no longer a dashboard filter.
...
- ✅ ~~Env toggle (production default)~~ **(removed 2026-06-26 — DB is the env boundary)** + 24h/7d/30d window + "last rollup" freshness chip (degrades gracefully pre-Phase-4). Shared `MetricCard` extracted to `components/platform-admin/`. Static checks + smoke verified; 14-agent adversarial review folded in.
...
### Phase 3 — In-app feedback (end users can submit bugs/features) — *1.5–2 days* ✅ BUILT 2026-06-10
...
### Phase 5 — ❌ CANCELLED 2026-06-10 (owner: "won't be needed")
The deferred/optional Phase 5 is **closed — not being built**. Its three parts are resolved as follows:
- **In-bell platform-admin alerts** (`admin_notifications` + `platform_admin_push_subscriptions` tables + `PlatformNotificationBell` org-less variant) — **cancelled.** Email-to-`ADMIN_EMAIL` critical alerting (Phase 4) is sufficient; an org-less in-app bell is not worth the new tables + push plumbing. Reopen only if email alerting proves inadequate.
- **Route-coverage sweep** — **NOT cancelled; moved out** to its own plan, [OBSERVABILITY_ROUTE_INSTRUMENTATION_PLAN.md](OBSERVABILITY_ROUTE_INSTRUMENTATION_PLAN.md) (Mechanism A shipped with Phase 3; Mechanisms B + C are the active continuation). This is the real "next" for observability.
- **Optional Sentry swap** — remains an **always-available additive option** (the design is a deliberate clean subset, so a Sentry capture SDK can bolt in behind `lib/observability/capture.ts` without a rewrite) but is **not a planned phase**. Decide only if source-mapped client stacks / session replay ever become a must-have.
...
## 15. Phase 3 detailed build plan — ✅ BUILT 2026-06-10 (owner-approved; Mechanism A folded in)
...
> ### Phase 3 build log (2026-06-10)
> **Built & verified** on `feat/free-tier-coaches`: the in-app `FeedbackWidget` (portaled modal: type pills Bug/Feature/Feedback + route-defaulted category + title + required body, success state) + `FeedbackLauncher`, mounted on AdminSidebar footer, admin bottom-nav "More", `CoachPortalShell` rail, and the scorekeeper/check-in headers. `POST /api/feedback` (all personas; anonymous accepted; per-user 1/hr + anon IP throttle + global 30/min backstop; redact before write; `supabaseAdmin` insert; **awaited** admin-notify + fire-and-forget submitter confirmation via `lib/email.ts`). Platform-admin `feedback/page.tsx` triage (type/category/status badges, org link, filters, CSV/XLSX export, "View related issue" deep-link) + `[id]/status` route (`manage_product`, `writePlatformAuditLog`) + `Feedback`/MessageSquare nav item. **Owner decision #1 (Mechanism A) folded in this build** — the requestId is no longer 2-route best-effort: `proxy.ts` stamps `x-request-id` on every `/api/*` request+response (cheap fast-path; `/api/admin/*` keeps the full proxy), `withObservability` adopts it, and a client `window.fetch` wrapper stashes it for the widget. **Verified:** `npm run typecheck` exit 0 · `eslint` 0 errors · 34 unit tests pass (10 new feedback + 24 observability) · change footprint = Phase-3 files only.
> **Adversarial review (10-agent: 4 dimensions × find + adversarial-verify) — 4 findings folded:** (1) **[high]** `instrumentation.ts onRequestError` stored `error_events.request_id = null` for the global-capture path (no ALS context) → the deep-link would never match → now reads `x-request-id` off the request and threads it via a new `captureError({ requestId })` opt (`opts.requestId ?? reqCtx?.requestId ?? null`); (2) **[med]** CSV formula-injection in the feedback export (attacker-controlled body/title) → `neutralizeFormula` prefixes a `'` for leading `= + - @ \t \r` (XLSX was a false positive — ExcelJS stores strings as strings; guarded anyway); (3) **[med]** the `window.fetch` wrapper stacked over itself on Fast-Refresh → the "already wrapped" flag now rides on `window.fetch` itself, idempotent across HMR; (4) **[med]** the fixed-position modal was clipped by the bottom-nav's `backdrop-filter` containing block → `createPortal(document.body)` + the bottom-nav now owns the widget state at the nav root (survives the dropdown's outside-click close). **Accepted (not changed):** anonymous XFF-spoof rate-limit (bounded by the global 30/min backstop — the same documented trade-off as `/api/client/error-capture`); multi-org attribution uses first membership; deep-link dangles after the 30-day event purge (graceful no-op).
...
- **Coverage (as built — Mechanism A folded in):** broad, not 2-route best-effort. `proxy.ts` stamps `x-request-id` on **every `/api/*`** request+response, `withObservability` adopts it, and — the adversarial-review fix — **`instrumentation.ts onRequestError` reads it off the request and threads it to `captureError`**, so `error_events.request_id` carries the proxy id for the global-capture path (uncaught throws + RSC errors), where almost all real errors land. The widget still works when no id is present, and the triage deep-link degrades gracefully — `error_events` purge at 30 days, so an old item's link can dangle; resolve `context.requestId → error_events.request_id → group_id` **at read time** and no-op the link when the event row is gone. (Per-route metrics coverage — Mechanism B of [OBSERVABILITY_ROUTE_INSTRUMENTATION_PLAN.md](OBSERVABILITY_ROUTE_INSTRUMENTATION_PLAN.md) — remains the separate tranche sweep.)

> STALE HEADER (corrected 2026-08-01): Fixes 1-2 WERE BUILT on dev 2026-07-31 (code-verified: canViewPlayerDocuments in lib/coach-capabilities.ts) and Fix 3 = migration 212 applied DEV-ONLY (must reach prod at release). The "planned, awaiting go-ahead" status below is stale. Owner QA tracked in active/OWNER_QA_LEDGER.md.

# Player documents & guardian PII â€” access plan
...
medical consent form. That removes the incident-response pressure and â€” importantly â€”
removes the mid-season-access-reduction problem entirely (see Q3).

### Q2 â€” Other doors to the same files

**Door A â€” file bytes: CLOSED.** Live prod `storage.buckets.rep-team-documents` is
...
URLs are genuinely the only read route. âœ…

**Door B â€” org-admin route: correctly gated, no change needed.**
...
to owner/admin. It never consults coach capabilities, which is right â€” org admins are a
different authority. âœ…

**Door C â€” PostgREST metadata bypass: OPEN, and wider than the documents defect.**
...
- `anon` and `authenticated` both hold the default `SELECT` grant (confirmed via
  `has_table_privilege`) â€” the known prod-only posture in `reference_supabase_rls_grants.md`.
- Each table carries two permissive SELECT policies: *"coaches can read assigned team â€¦"*
  (`team_id IN (SELECT team_id FROM rep_team_coaches WHERE user_id = auth.uid())`) and
  *"org members can read â€¦"* (any `organization_members` row for the org).
...
`medical_notes`, `emergency_contact_*`, `notes`, `admin_notes` â€” every column
`redactRosterPlayer` nulls â€” plus every player-document row. **The entire `rosterPii`
...
dev's `authenticated` role lacks the SELECT grant. Dev structurally cannot reproduce this â€”
the posture must be read from prod. Prod impersonation was not run to completion because
there are zero rows to read.

Nothing in the app reads these tables with a browser/anon client â€” every caller goes
...
**Door D â€” exports/bulk: none.** `roster/bulk/*` is import-only (CSV in). No roster or
document export path exists.

### Q3 â€” This removes access some live assistants have today
...
### Q4 â€” Head coaches and org admins unaffected
...
**Requiring both `documents !== 'off'` AND `rosterPii` â€” as approved. No new capability.**
...
### Fix 1 â€” gate player-attached documents behind guardian-PII

`lib/coach-capabilities.ts` â€” two new predicates beside the existing pair:

```ts
/** Per-player signed forms (waiver/medical) â€” the file itself may hold exactly the guardian
...
| File | Change |
|---|---|
| `â€¦/roster/[playerId]/documents/route.ts` GET | `canViewDocuments` â†’ `canViewPlayerDocuments` |
| `â€¦/roster/[playerId]/documents/route.ts` POST | `canManageDocuments` â†’ `canManagePlayerDocuments` |
| `â€¦/documents/[docId]/route.ts` GET (signed URL) | `canViewDocuments` â†’ `canViewPlayerDocuments` |
| `â€¦/documents/[docId]/route.ts` DELETE | `canManageDocuments` â†’ `canManagePlayerDocuments` |
| `â€¦/roster/[playerId]/page.tsx` | wrap the section in the view gate; pass the manage gate |

`â€¦/documents/templates/route.ts` (team blank forms) is deliberately **untouched**.

### Fix 2 â€” make the confirmation promise true

`components/coaches/CoachStaffPanel.tsx`:

- Move the `documents` segment from `SENSITIVE_SEGMENTS` â†’ `EVERYDAY_SEGMENTS`. Once
...
follows automatically â€” no second edit, no second chance to lie.

Result: all four remaining Sensitive items prompt. The sentence becomes true, unchanged.

### Fix 3 (NEW â€” needs owner go-ahead) â€” close the PostgREST door

Migration dropping the four dead SELECT policies, leaving RLS enabled with no policies â€”
...
Post-apply proof (prod): `set local role authenticated` + a coach's `sub` â†’ `count = 0`.
Dictionary + snapshots refresh in the same unit of work.

**Without fix 3, fix 1 does not actually close the leak** â€” it only closes the UI door.
...
source-object permission check â€” unproven either way, and unexploitable today (zero
...
- Unit coverage for the two new predicates across head/assistant Ã— grant combinations
- `/review` at the **high-risk tier** (auth/capability gating, children's medical records)
- `/docs` â€” Coaching staff and Documents guides describe the access model

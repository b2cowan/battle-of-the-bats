...
3. **Extend `operator` scope** (C38/C39/C47): add dirs `components/accounting`, `components/billing`, `components/feedback`, `components/platform-admin`, `app/tryout-score`; add files (NOT whole dir — `register.module.css` is public per C24): `components/rep-teams/{TryoutDayCard,TryoutFlowHeader,TryoutCheckIn}.module.css`, `components/notifications/{notifications,notifications-page,EnablePushBanner}.module.css`. ⚠ `TryoutAcceptDrawer.module.css` was in this list and is **deleted** (2026-08-26 — the tryout accept drawer is gone, accepting is one tap). `CODEBASE_CLEANUP_ANALYSIS.md` still names it as audit evidence from when it existed; do not re-add it here from that record.

...
No migration (the DB's replace function never decided the method rule — only the pure module did). Verification and the walk: ledger §187. B10–B12 (numbered after the artifact's B8 = the sheet and B9 = attempts) landed on the same uncommitted build; the help's "Changing a definition later" note, its "method not recorded" sentence and "same method throughout" example are owed to the `/docs` pass with the rest of stage 1.

...
| 43 | Player profile (edit UX) | Med | Save button lived at the bottom of a long form — hidden when editing fields near the top; unclear how/whether edits save | Built — **viewport-pinned** save bar (fixed to the window bottom, offset for sidebar / above mobile nav, +spacer) showing "Unsaved changes · Discard · Save changes" with inline "Saved" confirmation. Plus a reusable **UnsavedChangesGuard** that warns on leave for BOTH tab-close/refresh (native) AND in-app nav (sidebar/bottom-nav/link clicks intercepted → "Leave / Stay" modal). Decision: explicit save, NOT autosave (form holds medical/PII) |
| 44 | Schedule | High | Couldn't edit a scheduled event's details (date/time/location/opponent/name/notes) — the editor was add-only; opening an event showed read-only details | Built — "Edit details" action in the event slide-over reuses the event form to update the occurrence (backend PATCH already supported it). Recurrence not re-editable (edits the single occurrence) |
| 45 | Coach modals (edit UX) | Med | Modal Save button sat below a long scroll (e.g. scheduling a game); closing/navigating from a modal dropped unsaved input | Built — **sticky modal footer** (shared across all coach modals → Save always visible while the body scrolls) + **UnsavedChangesGuard** on the Add/Edit Event and Add Player modals (nav warning + confirm-before-discard on close/backdrop). Remaining modals (accounting, announcements) get the sticky footer free; nav-guard sweep there = follow-up |
| 46 | Schedule (event slide-over) | Med | Event detail was one long stack (details → attendance → lineup → score), burying Edit/Cancel/Delete at the very bottom below the lineup | Built — Edit/Cancel/Delete moved to a **top action row** (always visible under the details); Attendance / Lineup / Result now behind **tabs** (only the relevant ones show per event type), so the panel is short. Each event opens on Attendance |
| 47 | Coaches portal (all) | Med | Two competing primary-button styles — coach screens used a blue local primary while Add Player / profile-save / Add Event were lime; inconsistent | Built — standardized the coach portal primary button to **lime** (app-wide convention) in one place; all ~35 primary/save buttons across 13 coach screens now match |
| 48 | Schedule (lineup tab) | High | Lineup tool was a tedious 35-dropdown grid; no fairness checks, no auto-build, no reusable starting points, no field-ready printout | Built — **Coach Lineup Builder** Phases 1–4 (dev): mistake/fair-play checks, drag batting order, one-click auto-generate (best-of-N), **dugout-wall + batting-order PDFs** (pen-fillable, "@/vs", optional notes), and **named templates** ("Gold medal game"; mig 159, ⚠prod-pending). Each /review-passed. See COACH_LINEUP_BUILDER_PLAN.md |
| 49 | Schedule (event create + detail) | High | Eval 2026-06-28 (UX+design, verified): all 6 event types share one generic form + one over-stacked detail panel; redundant required Name; easy-to-skip Home/Away; multi-day tournaments stored as a single time + shown on one day; orphan-able tournament game slots; missing arrival/field/uniform fields; season W-L-T silently league-only | Decided → **Tier 1 plan written** (per-type grouped forms, slim detail panel, tournament date-range/spanning; Tier-2 fast-follow adds game-day fields). Owner decisions captured. See SCHEDULE_EVENT_UX_PLAN.md + _PM_BRIEF.md |

## Mobile findings (deferred to a dedicated mobile pass — owner decision 2026-06-28)

These are cross-cutting (shared bottom nav / modal container), so we finish the desktop
walkthrough first, then fix them together. Logged as spotted:

- **Bottom nav shows "My Teams" for a single-team coach** — mirror the desktop fix (hide the hub entry / switcher with ≤1 team).
- **Modals don't fill on mobile** — odd margins around the modal; part of the modal (incl. the Save bar) is **clipped behind the fixed bottom nav**. Modal container needs mobile sizing + bottom-nav clearance.
- **Slide-over per-section Save (attendance/lineup) hidden behind the bottom nav** on mobile — same nav-clearance issue.
- (add more as we walk each page on mobile)

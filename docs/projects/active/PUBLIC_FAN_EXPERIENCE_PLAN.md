...
- **Alerts + offline shell — BUILT** (typecheck + focused lint clean; ⚠ on-device verification pending — see below):
...
> **Phase D — alerts + offline shell COMPLETE (pending on-device verification).** The four heavier-PWA decisions taken (owner sign-off): offline shell = fallback **+ last-known scores**; iOS alerts = explainer **+ post-install nudge**; branded iOS icon = **generated 180×180**.
>
> ⚠ **Must be verified on real hardware (cannot be self-tested):** (1) iPhone + Android — install the PWA, then enable alerts and confirm the toggle/nudge flow; (2) finalization org — submit then finalize a score and confirm a "Final:" push arrives (and a forfeit does NOT send a score push); (3) tap a notification → focuses the app + shows the branded logo; (4) revoke notification permission → toggle flips back to "Get score alerts"; (5) install → airplane mode → open a previously-visited tournament page (last scores) and a never-visited one (branded offline page); confirm admin/coaches pages are NEVER served offline; (6) iPhone Plus event → Add to Home Screen shows the tournament logo, free event shows the clean default. **Dev-server restart required before testing** (new files, shared-module + service-worker changes).

...
- **D5 — Pressure ladder (from Coaches Experience).** In-portal pre-event surfaces stay **pitch-free**; the **one earned ask** is at the afterglow. The "free to operate" marketing pillar applies to the **public marketing site**, not the portal interior. **⚠ PROMOTED 2026-07-27 — this rule is now BINDING and lives in `docs/agents/strategy/BUSINESS_DECISIONS.md` (2026-07-27 entry), which also enumerates the complete list of surfaces where an ask IS permitted. Read that entry, not this line, before proposing any in-portal upsell; it was promoted precisely because living only here let B3's own plan section propose breaking it.**
...
- **Flow A — Subscriptions (org → FieldLogicHQ for a plan).** Built AND **LIVE** — ⚠ *corrected 2026-07-28: this line previously said the production cutover was "still off." It is not.* Live account, live prices, production webhook, branded live customer portal, live keys, billing audit clean; **Tournament and Tournament Plus are purchasable today**. Held plans (Premium Coaches Portal, League, Club) are gated by **product** decisions, not Stripe readiness. What remains on its **own clock** is the **Founding-Season conversion (Jan 1 2027)**: the 4C conversion flow + the manual comp-cohort runbook must land before that cliff, and the deferred live-card smoke test should run ahead of it. **NOT** part of the deferred Flow-B enhancement and **not** freely deferrable. Tracked in `PROGRAM_BILLING_AND_ENTITLEMENTS.md`.
...
### Journey-audit inputs — J3 house-league admin + J7 house-league parent (2026-06-11)

Two more journey walks route design input here — J3 routes 8 findings, J7 routes 7 (full findings tables + evidence in docs/projects/active/journeys/JOURNEY_J3_HOUSE_LEAGUE_ADMIN.md and docs/projects/active/journeys/JOURNEY_J7_HOUSE_LEAGUE_PARENT.md; the reports stay the source of truth, this section carries the actionable design input). Both reports' verifiers confirmed none of these are owned by any active plan today. **Only one is fix-now-tagged in its report: J3-007.**

**League acquisition / express-interest fixes (J3).** The express-interest funnel is this plan's *only* acquisition channel until Phases 4/8 ship, and the J3 walk found it broken at four points:

- **J3-007 (High, fix-now in J3)** — every Coaches Portal express-interest CTA fails: the preselected trigger values exist in neither the form's rendered options nor the API allowlists, so nothing shows pre-checked and submit returns 400 (verified live). Add a coaches_portal plan option to form + API allowlists, plus a guard/test that every trigger's seeds validate (components/EarlyAccessForm.tsx, app/api/early-access/route.ts, app/for-leagues/page.tsx).
- **J3-008 (Med)** — express-interest submission emails no one (no lead confirmation, no team alert); a board-approved buyer's interest sits in platform-admin triage until someone checks. Send a branded confirmation + ADMIN_EMAIL notification on lead insert/update via the existing Resend stack (app/api/early-access/route.ts).
- **J3-009 (Med)** — re-submitting express interest silently overwrites the lead's previous plans/features/notes (only submission_count accumulates) while the funnel's cross-sell triggers encourage resubmission; the sibling writer of the same table merges. Union the arrays and append notes on update (app/api/early-access/route.ts vs lib/basic-coach-interest.ts).
- **J3-011 (Low)** — the modal frames a sales-intent click as a product survey ("Tell us what you want to see next" / "Join Early Access"); thread the triggering plan into the copy and match the submit label to the CTA (components/EarlyAccessModalTrigger.tsx, components/EarlyAccessForm.tsx).
- **J3-004 (Med)** — /pricing hides the League price ("Coming soon" where $89/mo should be, while the equally-gated Coaches Portal shows $29 and the Club bridge $19) and never links /for-leagues; the house-league segment card opens the capture modal instead of landing the buyer in the right place (components/PricingSection.tsx, app/pricing/page.tsx).
- **J3-005 (Med)** — the mobile pricing compare table renders exactly one plan column at 390px with no scroll affordance; a phone evaluator cannot compare League at all.
- **J3-001 (High, AP)** — already-planned confirmation of the no-path reality Phases 4/8 + §15 own (no checkout, trial, demo, screenshots, timeline, or "book a call"; /for-leagues is 100% text). Carry the interim asks into those phases: product proof on /for-leagues (the seeded dev-league org shows the demo-season content already exists), a stated launch window/review cadence, and a consultative-track option for board-approved buyers (lib/plan-config.ts, app/for-leagues/page.tsx).

**League free-floor / Phase-6 design inputs (J3 + J7).** League Starter's scope currently ends at registration intake; these extend the floor's definition into the season:

- **J3-061 (High)** — house league has no coach channel at all: league_teams stores only free-text coach_name (no coach_email column, API field, portal surface, or composer audience), so post-draft roster distribution is the CSV export + the admin's personal Gmail — while the same org's tournament side has coach_email, resolveCoachRecipient, a portal, and game-day reminders. Minimum viable for the League floor: coach_email on league teams + an "Email coaches" audience + one-click "send roster & schedule to coach"; full fix = extend the coaches portal to house-league teams, sequenced after the minimum step (api/admin/house-league/.../teams/route.ts, .../email/route.ts).
- **J7-016 (High)** — the team question is unanswerable through any channel: assignment is a silent column write (no email fires, the status lookup omits team_id, coach_name renders on zero public surfaces). Minimum viable: team + coach lead the status card post-assignment (pre-placement: "Team assignment coming — we'll email you"); then the team-reveal email ("Nora is a Cardinal! Coach Pat, first game June 14") — the tier's highest-delight moment and its viral loop; team-colour/OG primitives already exist tournament-side (registrations/[regId]/route.ts, league status/page.tsx).
- **J7-017 (Med)** — no my-team view and no "today" anchor on the public schedule a family checks weekly for four months: add a ?team= filter (team chips beside division tabs), auto-anchor the current week, and a lightweight league team page as the team-reveal email's destination (league schedule/page.tsx).
- **J7-022 (Med)** — every saved entry point from last season is a dead end while next season's registration is open: completed/active season pages, the closed register page, and the status page never mention the registration_open season in the same table. Re-registration is the tier's cheapest acquisition channel — banner the open season's Register CTA on every terminal surface.
- **J7-023 (Low)** — season close is a shrug ("This season has concluded.") whose most prominent CTA can land on admin-voiced jargon; a completed-season recap (division champions, team records, organizer thank-you) ending in the open-registration CTA turns the memory moment into next year's registration (league [seasonSlug]/page.tsx, standings/page.tsx).

**Parent-facing model gaps (J7).** Schema/flow holes the League floor's flagship registration loop cannot answer without:

- **J7-008 (High)** — the registration fee is never actionable: no method, payee, deadline, or consequence anywhere in the season page, form, success panels, or any email — and league_seasons has no payment_instructions field while tournaments have one. Per-season payment_instructions (schema parity with the tournament setting) rendered at five surfaces — season fee block, form note, success panels, approved + waitlist-promoted emails, status card — with the existing reference code as the e-transfer memo. This is the missing half of §9's locked manual-fee-tracking interim: §9 defers only *online* collection; the instructions surface is unowned. Without it the tier's flagship flow ends in payment limbo for every family (components/league/RegisterForm.tsx, lib/email.ts, lib/types.ts).
- **J7-009 (Med)** — the approved path — the default outcome — ends with no "what happens next", and the reference code arrives with zero guidance (the next-steps callout renders only for pending/waitlist). Render it on the approved path: payment step (ref as e-transfer memo), team-assignment timeline, and the prefilled status link framed as the bookmarkable season home (RegisterForm.tsx, lib/email.ts).
- **J7-006 (Med)** — DOB is optional and entirely unvalidated on an age-divisioned form, and divisions carry no age-bound fields. Division age data powers three things at once: required-DOB validation, the inline "DOB outside U10" warning, and the per-division guidance line the J3-075 trust block needs (RegisterForm.tsx, league register route).

Launch-gate context carried from the J3 report (backlog-routed there, tracked here as marketing-flip gates): J3-014 (the admin hub tells a paying League customer her plan is "coming soon"), J3-074 (/for-leagues sells "Public organization page" while the league public site is an unbranded stub), J3-021 (the unconditional "free through Dec 31" claim), and the fix-now public-trust package (J3-069/068/047/070/071) — none should survive to the moment League is promoted. J7 adds J7-001 (Blocker, fix-now there): the in-progress season vanishes from public navigation the moment next season's registration opens — until it lands, the in-season half of the parent experience this section invests in is unreachable during the overlap months.

### Journey-audit inputs — J2 rep head coach + J9 house-league coach (2026-06-13, Phase 4)

The Phase-4 walks route 11 findings here. The reports are the source of truth (full detail/evidence/verdicts in [journeys/JOURNEY_J2_REP_HEAD_COACH.md](journeys/JOURNEY_J2_REP_HEAD_COACH.md) and [journeys/JOURNEY_J9_HOUSE_LEAGUE_COACH.md](journeys/JOURNEY_J9_HOUSE_LEAGUE_COACH.md); screenshots in the gitignored `tests/uat/results/journeys/J2,J9/`). 0 refuted in verification.

**J2 — the Coaches marketing flip is now the highest-leverage coach move (Phase 8's subject, confirmed live):**
- **J2-001 (High, already-planned)** the live free `/start/team` product is unreachable from the *entire* signed-out marketing arc (`/`, `/for-coaches`, `/pricing`, navbar, footer never link it; every coach CTA is the express-interest modal). Basic is live + browser-verified, so the Phase 8 flip is now pure opportunity cost — if §13 instrumentation is the gate, that's the unblocking work to prioritize.
- **J2-002 (High, already-planned)** "Get Started" (navbar + homepage CTAs) sends a coach to `/auth/signup` = "Create Your Organization" with no coach exit — the Phase 1 "stop making `/auth/signup` universal" tail the marketing chrome never adopted. Cheapest interim: a "Coaching one team? Start free →" line on the signup card.
- **J2-024 (High, NEW — an unbuilt feature behind a shipped promise, not a copy fix)** "It carries over automatically if your organization joins FieldLogicHQ" (hub + Teams empty state + 4 marketing pages) is **undelivered for the org-join path**: the only carry-over code keys off a *tournament-registration* match, never an org-join; the rep-team create + coach-assign routes have zero `basic_coach_teams` linkage. The free-floor-to-Club thesis rests on this. Either build the org-join import (offer to import roster/schedule/fees when an org claims/assigns a coach who owns a matching basic team by email) or soften the copy until it exists.
- **J2-003 (Med)** scope the lineup/budget marketing promise by tier at flip time (the standalone floor explicitly excludes lineups/attendance/docs/dues). **J2-006/007/008 (design-lens)** the flip's visual layer: `/for-coaches` reads like a waitlist (ghost CTA, no "Start free" lime button), the `/start` Coach card isn't visually findable, and the coach pricing view shows **$39 before Free** — all undercut the conversion the flip depends on. **J2-014 (Low)** the hub premium pitch demands a club name + sends no confirmation.

**J9 — League free-floor / Phase 6 coach-identity work (confirms J3-061's root from the coach's seat + supplies the sequence):**
- **J9-010 (High, keystone)** add `coach_email` to `league_teams` and mint the coach's account via **claim-by-email**, reusing the proven `coachPortalUrl` pattern (the lightest of three already in the tree) — the prerequisite that unlocks every other coach job. Once addressable, the four jobs light up from primitives that are *already built and merely admin-gated*: roster read (J9-011, `league_registrations`), practices read (J9-012, `league_practices`), rainout notify (J9-013, `lib/notify.ts` `userIds`), score report (J9-014, the scorekeeper pattern). **J9-001/009 (High/Low)** a lightweight per-team public page (coach, schedule-with-location, record) serves the anonymous coach today and becomes the claim email's destination tomorrow. (Structural root = J3-061 above; J9 supplies the implementation path the §16 entry left open. The coach-portal *extension* findings J9-002/003/006/011/012/013 route to COACHES_EXPERIENCE_EVAL.)

...
> **Status:** In Progress — **Phases 1 + 2 BOTH BUILT on `dev` (P1 2026-07-08 owner-browser-passed + /review complete w/ 5 fixes folded; P2 2026-07-09 typecheck/lint clean) + `/docs` synced 2026-07-09. All uncommitted. Pending: owner browser pass of Phase 2 (dev-server restart REQUIRED — shared nav components + `lib/coach-nav-visibility.ts` + `SeasonRecordWidget` touched) + `/review` on the Phase-2 diff + commit.** Direction locked 2026-07-08 (D1 = 1B tabs + filter bar v2 · D2 = Insights destination · D3 = "Insights"; all logged to `memory/design_decisions.md`). Mockup `public/mockups/lineups-ia-options.html` is throwaway — delete once the owner verifies Phase 1+2 in the browser.
> **Created:** 2026-07-08
> **Branch:** `dev`
...
- [ ] V3 build (after mockup sign-off): scoreboard band + `lib/insight-findings.ts` (+ unit tests) + question tiles; playing-time report page (unified table + lineup records); composed `insights` read endpoint (or 6-fetch composition); replace the V2 grid; `/docs` re-sync; `/review`

...
- **4.2 — Operator reset-password flow FIXED + owner-verified ✅ 2026-06-29.** Generated a fresh reset link → it now lands on **"Set New Password"** → owner set a password and signed in successfully. Persona 4 complete (4.1 + 4.2 both pass).
- **4.2 — NEW (HIGH, owner-found) Operator reset-password link dead-ended at Sign In — FIXED 🔧 2026-06-29 (verify pending).** The platform-admin "Reset Password" action generated a valid recovery link, but pointed it at **`/auth/login`** — so a locked-out customer pasting it landed on the plain **Sign In** screen (token sitting unused in the URL) instead of a "Set New Password" form, with no way to actually set a password. Real impact: the support tool for unblocking a locked-out customer didn't work end-to-end. Root cause: wrong redirect target; the dedicated **`/auth/reset-password`** page already correctly catches the recovery token (implicit/admin hash flow) and shows the set-password form — and the customer-facing forgot-password flow already points there. Fix: one-line — point the operator reset link at `/auth/reset-password` (matches the customer flow). API route, hot-reloads. Lint clean. *(Note: the customer forgot-password flow also wraps its link in a scanner-safe confirm page; the operator link is copied+shared by hand, no email scanner in the path, so the direct redirect is sufficient.)*

### Phase F — Reliability, Triage & Daily Health (Persona 6) — in progress

- **6.1 — NEW (owner ask) Two export buttons → single "Export ▾" dropdown — FIXED 🔧 2026-06-29 (verify pending).** The Observability/Issues screen (and, same pattern, **Audit** and **Feedback**) showed two separate export buttons ("↓ Export XLSX" + "CSV"). Converted all three to the **shared `ExportMenu`** dropdown already used across the console (Customer Users, Orgs, Early Access): Excel on primary click, CSV in the menu — one control, consistent everywhere. Client-only; typecheck + lint clean; hot-reloads.

- **6.1 — NEW (owner ask) Observability "two error numbers" clarity — FIXED 🔧 2026-06-29 (verify pending).** Owner asked why errors are calculated two ways. Not a bug — two intentional measures: **error rate + calls-vs-errors chart** = instrumented (wrapped) routes only (needs a call denominator for a true rate); **captured errors + issue list** = global error capture (all sources, more complete, no rate). They diverge because wrapped-route coverage is a rollout in progress (0% instrumented rate can coexist with captured errors from unwrapped/client paths). Relabeled for clarity (no logic change): "Errors" tile → **"Captured errors · [window]"** + sub **"all sources"**; error-rate sub → **"instrumented traffic only"**; coverage note rewritten to state the reconciliation explicitly. Lint clean; hot-reloads.

- **6.1 — NEW (owner ask) Observability filter did a full-page reload + scroll-to-top — FIXED 🔧 2026-06-29 (verify pending).** The issues filter was a plain server `<form method=GET>`, so each Filter submit did a full browser navigation (reload flash + jump to top). Converted to a client filter bar that does a **soft App-Router navigation** (`scroll:false`) — the list re-renders in place, scroll position is preserved, Filter shows a brief "Filtering…" pending state, and Clear still empties the fields (form keyed on applied filters so uncontrolled inputs reset with the result set). Export dropdown + Clear folded into the client bar; no behavior change to results. Typecheck + lint clean. *(Same server-form-reload pattern likely exists on other platform-admin list filters — candidate for a later consistency sweep.)*

- **6.1 — REGRESSION (self-introduced) Clear button unreadable — FIXED 🔧 2026-06-29.** When the observability filter's "Clear" changed from a `<Link>` to a `<button>` (soft-nav fix above), it inherited the native button box (low contrast). Added a button reset to `.filterClear` (transparent bg, no border, pointer) so it reads like the original text link + a disabled state. Owner-caught.
- **6.2 — COMPLETE ✅ (owner-marked 2026-06-29).** Email Campaigns / audience walkthrough done. Table layout + copy fixes verified; audience-exclusion (H10) not observable with seed data (2 founding orgs, none on Club) but previously fixed/verified; editable-campaigns gap scoped to `/plan`.
- **6.2 — NEW (owner-found) Email Dashboard table overflowed the screen — FIXED ✅ (owner-verified 2026-06-29).** **Root cause:** the **Subject** cell carried the right truncation styles (`max-width:280px` + ellipsis) but on an **inline `<span>`**, where `max-width`/`text-overflow` are ignored — so the full subject rendered on one line and forced the whole table past the viewport (a scroll wrapper alone couldn't contain it). Fix: `.subject` → `display:inline-block` so the width cap + ellipsis actually apply (full text still on hover via `title`). Also wrapped both wide tables (Scheduled Sends 7-col, Sent History 10-col) in an `overflow-x:auto` container as a narrow-screen safety net; brought Subject + Send-date font down to the compact data size (0.65rem) to match the other columns. CSS/layout only.

- **6.2 — NEW (owner-found, feature gap) Founding-season campaigns aren't editable in the UI → scoped to `/plan` 2026-06-29.** The 10 Scheduled-Sends campaigns (subject/timing/audience/content) are **hardcoded in code** — the dashboard only Previews + Sends. The separate **Email Templates** editor (DB-backed, override + reset) covers only **transactional/system** emails, not these marketing ones. So the console implies campaign management it doesn't offer. Scoped: extend the template system to a Marketing/Campaign category (P1 content-editable + collapses the send-vs-preview content duplication → SoT SOT-6/-14); schedule/audience kept mostly system-defined (logic/consent-backed, no free-text targeting); dashboard clarity P3. Plan + PM brief in `docs/projects/active/EDITABLE_EMAIL_CAMPAIGNS_PLAN.md`; TODO linked.

- **6.3 — COMPLETE ✅ (owner 2026-07-08).** Audit investigation walkthrough done: friendly action names ✅, new **Export ▾** dropdown downloads Excel + CSV ✅. M4 (text search only covers the loaded page) **not reproducible** with current data (too few rows for pagination) — deferred to a data-heavy retest; still an open confirmed-in-code item.
- **6.3 — CONFIRMED (owner) Audit action dropdown showed raw keys — FIXED 🔧 2026-07-08 (verify pending).** The audit **table** already rendered friendly action names (via `actionLabel`), but the **filter dropdown** listed raw keys (`generate_reset_link`, `ban_user`, …). Pointed the dropdown at the same `actionLabel` helper + sorted options alphabetically by label; also upgraded the helper's fallback (for any action without a curated label) from lowercase-with-spaces to **Title Case**, so both the dropdown and table read consistently and never show snake_case. Server component; lint clean; hot-reloads.

- **6.4 — NEW (owner-found) Audit log table overflowed the screen — FIXED 🔧 2026-07-08 (verify pending, 2nd pass).** First pass trimmed the two Old/New value columns 290→180px — insufficient. **Real cause:** the table was `width:100%`, so it **stretched** to fill an over-wide container and dumped the extra width into the text columns (the giveaway = huge empty gaps in Superuser/Org), pushing it off-screen. Fix: `.table` → `width: max-content` so it sizes to its content instead of stretching (gaps gone; `.tableWrap`'s `overflow-x:auto` still scrolls on genuinely narrow screens). Value-column trim kept for compactness. CSS-only.
- **6.4 — NOTE (discoverability, not a bug) "Take metric snapshot" is under the Overview → "Metric Notes" tab** (4th tab), role-gated to super-admin/product. The 6.4 step wasn't missing, just tucked away — optional follow-up to surface it more prominently if wanted.

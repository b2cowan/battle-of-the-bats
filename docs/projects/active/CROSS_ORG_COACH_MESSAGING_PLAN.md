...
**Parent plan:** `docs/projects/active/COACH_CHAT_PLATFORM_PLAN.md` (the shared chat engine + the three in-org surfaces). This cross-org capability is a **fourth surface** on the same engine, not a separate build.
...
2. **The coach directory is opt-in.** Coaches do not appear as discoverable to other orgs unless they explicitly opt in. (Directory itself is deferred to Phase 4.)
...
| **1 — Core chat engine + in-org surfaces** | Build the shared engine + the tournament chat (and peer chat) from the parent plan. Cross-org is an extension of this, not a parallel build. | ~3–5 weeks (per parent plan) |
| **2 — Cross-org transport fixes** | The three structural fixes in §5. | ~1–1.5 weeks |
| **3 — Cross-org DM: invite-by-link V1** | Link generation in the coach portal (single-use + expiring, rate-limited), accept/join flow, an inbox entry, reuse of the shared chat panel; either party can leave/block; initiator moderates. No directory. | ~1.5–2 weeks |
| **4 — Opt-in directory + connection requests (deferred)** | Opt-in directory, connection request → accept → room, block/report/rate-limit. **Blocked by a CASL/PIPEDA legal review.** Defer until real standalone coaches exist and V1 proves demand. | ~4–6 weeks + external legal review |
...
6. **CASL/PIPEDA** — stranger-to-stranger contact across orgs is a harder anti-spam/consent case than one-directional admin messaging. Directory (Phase 4) needs legal sign-off; invite-by-link V1 does not (the coach opts in by generating the link).
...
3. **Directory fields (Phase 4)** — name only, name + org, or name + org + sport/age group. Each added field widens consent/PII scope.
4. **Block/report depth (Phase 4)** — simple mutual-leave/block vs. a reportable-to-platform-admin flow.

## 10. Dependencies

- Coaches Portal commercial launch (Phase 0).
- Core chat engine (parent plan, Phases 1–3).
- The three structural fixes (§5) before any cross-org thread.
- CASL/PIPEDA legal review — **Phase 4 only** (the directory); Phases 1–3 do not require it.

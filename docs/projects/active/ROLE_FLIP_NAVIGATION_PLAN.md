...
- **Coach twins:** any public page → the team's **tournament record** (fee status, registration status, next game — the owner's "check my fees" loop lands here); record + coach schedule + overview → public **Schedule**; other coach pages → public Overview fallback.

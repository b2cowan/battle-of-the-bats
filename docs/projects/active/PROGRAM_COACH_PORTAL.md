...
*H2 (next):* the three round-trippable import templates (month grid · simple list · payables schedule), preview-first with per-row outcomes on the roster-bulk pattern, **amount cells EMPTY (D-G1)**, an explicit client-supplied `sortOrder` on the lines POST so sheet order survives, and an "Import a spreadsheet" door on Chunk G's first-run surface. Split out deliberately so a brand-new write path gets its own undivided HIGH-tier review. *(original entry below)*

...
> **Status:** Scoped 2026-06-13. **Priority: P3 — opportunistic polish backlog.** **Batch 1 (Groups B + C + D) BUILT + COMMITTED 2026-06-14 (1ea63ec, not pushed).** **Batch 2 (Group E, all 7 billing items PAB-005/006/009/010/012/013/014) BUILT + COMMITTED 2026-06-14 (546f683, not pushed).** Both pending owner browser smoke-test. **Note:** PAG-004 (Group F, Action-Queue non-billing note) was already shipped as part of F4 (43f8cde) — drop it from F5 scope. **Group A ✅ verified already-done (no-op, 2026-06-14).** **Group F (growth) BUILT + COMMITTED 2026-06-14 (e599520, not pushed) — all 7 items (PAG-005/006/007/008/009/010/011); PAG-004 was F4. ⚠ touches shared lib/email-sender.ts → dev-server restart needed. Reviewed via /review.** **Group H (read-only observer) BUILT + COMMITTED 2026-06-15 (d2c134e, not pushed): PAR-005 (snapshot button hide + API manage_product gate) + PAR-007 (Billing form → read-only summary for observers); PAR-002 was already done (F2). No shared-module edits → no restart.** **Group G (super-admin / nav IA) BUILT + COMMITTED 2026-06-15 (4b6f68e, not pushed) — all 8 items: PA0-005 nav regroup (System → Support & Diagnostics + Governance; Email Templates → Billing & Product; Dev Tools still injects via preserved 'System' label; every item's area gating preserved), PA0-006 Email → "Email Campaigns", PA0-008 "Change Requests" → "Approval Queue" across all user-facing copy (route/area key/`#change-requests` anchor/DB table/API path UNCHANGED) + Plans-&-Pricing relationship HelpCallout, PA0-011 + PA0-013 page kickers aligned to new groups, PA0-012 Delete-Org "Danger Zone" divider + `.dangerSection` (already last card — visual framing only; premise that it was the "first" card was STALE), PA0-009 Customer Users employee-exclusion HelpCallout (CTA→Platform Users gated to super_admin), PA0-014 defensive `requirePlatformAreaView` on orgs/orgs[id]/audit — **3 pages not 4** (customer-users was already guarded); orgs[id] swap of getPlatformAdminContext→requirePlatformAreaView is strictly MORE secure. ⚠ shared PlatformAdminNav.tsx → dev-server restart before browser QA. Reviewed via /review high-risk funnel (4 lenses; nav/security/CSS clean): 2 findings fixed (literal `&amp;` in a HelpCallout title string prop; missed "change requests"→"approval queue" in a Help Hub desc).** **Remaining: Group I (date-format sweep) only.**

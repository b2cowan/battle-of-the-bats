...
> **Status:** **1.1 BUILT + verified on `dev` 2026-06-30** (unpushed; not committed — awaiting owner OK to commit). Mig 164 APPLIED TO DEV + snapshots refreshed (watermark #164) + dictionary green; lint 0-err; typecheck clean. **1.2 ⏸ PAUSED 2026-06-30** — deferred with all parent/guardian/athlete-user work to a dedicated "parent users on the platform" decision at the **end of the Tryouts & Evaluation project**. Parent-user deferral logged in `BUSINESS_DECISIONS.md` (2026-06-30).
>
> **⚠ Before browser-testing:** restart the dev server (shared modules `lib/types.ts`/`lib/db.ts` + new files changed). **⚠ Before promoting to master:** apply mig 164 to PROD (`node scripts/apply-migration-api.mjs supabase/migrations/164_tryout_consent_and_org_privacy.sql --prod`) — it is **DEV-only / prod-pending** (consent inserts 500 on prod until applied).
>
> **Build log (2026-06-30) — 1.1 consent gate + privacy pipe, all DONE:** consent migration 164 (5 consent cols on `rep_tryout_registrations` + `organizations.privacy_policy_url`, additive/nullable, applied dev) · types + mappers + `createRepTryoutRegistration` threaded · 3 required consent checkboxes on the public `TryoutRegisterForm` (marketing draft copy) · server validates all-three + stamps `consent_at` + server-side `consent_ip` via `clientIpFrom` (never client-trusted) · admin **Consent** compliance column (✓ + date / "No consent on record") · consent columns in the admin export (Consent Given/Date non-sensitive, IP sensitive) · **privacy-policy pipe**: `getOrgPrivacyPolicyHref` seam (`lib/privacy-policy.ts`) + nullable `organizations.privacy_policy_url` + consumed in the form (link variant when set, plain text when not) · DATA_DICTIONARY updated. **Deferred (by design):** admin UI to *set* the org privacy URL (lands with the League/Club public-site privacy page; pipe is inert-but-ready until then); optional export-catalog stale-entry cleanup; PDF consent log (export is CSV/XLSX — PDF render is a platform stub).
> **Created:** 2026-06-29
> **Branch:** dev
...
- [ ] **Form** (`components/rep-teams/TryoutRegisterForm.tsx`) — add three **individually-ticked, required** checkboxes before Submit (data-collection / email-comms / eligibility-guardian). Block submit until all three are checked. **Copy: handed to `/marketing` 2026-06-30** (DRAFT, no legal agent — finalize later if needed). The data-collection checkbox includes a **Privacy Policy link** via the seam below.
- [ ] **Privacy-policy link pipe (forward-compatible seam)** — a single resolver (e.g. `getOrgPrivacyPolicyHref(org)`) returns the org's privacy-page URL when one exists, else null → the consent renders **without a broken link** (plain text now, link lights up later). Add a **nullable org-level privacy-policy reference** (additive — an external URL the org can set, and/or a platform route `/{orgSlug}/privacy`) so the pipe has somewhere to point. **Minimal V1 = the seam + nullable storage (+ optional minimal public `/{orgSlug}/privacy` route)**; the richer org-authored public privacy page belongs with the not-yet-finished League/Club public website and integrates through this same seam later. *(if the org column is added → migration is additive/nullable + dictionary + snapshots)*

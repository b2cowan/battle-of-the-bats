...
**Status:** Phase 0 + Phase 1 **BUILT + DEPLOYED to production** (2026-06-19) — a *silent anchor* (sport stored + threaded end-to-end; migrations 135/136/137 applied to dev + prod; **no organizer-facing change yet**). **Phase 2 signed off (brief ready) but ⏸ PAUSED 2026-06-24** — owner is focusing on getting softball/baseball fully working first; resume multi-sport (reveal picker + sport-aware labels + org primary sport) afterward. Phase log: `memory/project_multisport_tournaments.md`.

> **⚠ Build guardrail while paused (owner directive 2026-06-24):** while building softball/baseball features now, keep new logic **sport-neutral** — route score/period/label vocabulary + sport rules (points-per-win, diff-cap, default tie-breakers) through the `lib/sports.ts` Sport Pack / `getSportPack`, reuse `DEFAULT_SPORT`/`SPORT_OPTIONS`, keep scores + standings math generic. Do **not** scatter fresh "Runs"/"innings"/mercy-cap/`wins*2+ties` hard-codes. Goal: adding basketball later = **add a pack object, not rework**. See `memory/feedback_sport_neutral_no_debt.md`.
...
0. **Org primary sport (default, not lock — decision 6; sub-decisions settled 2026-06-24)** — resolve an org-level primary sport: **explicit org-settings "Primary sport" control if set → else inferred** from the org's existing league/tournament sports → else softball. Add the **"Primary sport"** control in org settings. The org primary sport **seeds the default for new tournaments, leagues, AND rep teams** (unify all module defaults — now in scope, not optional). **Creation sport picker** (from `TOURNAMENT_SPORT_OPTIONS` = softball + basketball + Other) **pre-fills from the org primary sport**, overridable per tournament; include `sport` in the setup-tournament body + the update whitelist. **Event Settings** sport field in "Tournament Overview", editable post-creation (with the decision-3 standings-safety warning once games exist).
...
6. **Sport is per-tournament with an org-level PRIMARY SPORT as the default — NOT a per-org lock** (owner decision 2026-06-24). An org has a primary sport that **pre-fills** every new tournament (one-tap confirm; single-sport orgs effectively never re-answer "what sport?"), but any tournament can override it. Rejected the hard org→sport lock: it would contradict the already-per-entity `league_seasons.sport`/`rep_teams.sport` model, box out genuinely multi-sport clubs (Club-tier target), and is a costly one-way door. Org primary sport = **inferred for existing orgs** (from their current league/tournament sports, else softball) **+ an explicit "Primary sport" control in org settings** for new orgs. Granularity stays one-sport-per-tournament (not per-division). ✅

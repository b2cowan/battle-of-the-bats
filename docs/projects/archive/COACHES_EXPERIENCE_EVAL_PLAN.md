...
### Journey-audit inputs — J4 club president (2026-06-11)

The J4 club-president walk routed 5 verified coach-portal/portal-links findings here (full detail, evidence,
and verification verdicts in [journeys/JOURNEY_J4_CLUB_PRESIDENT.md](journeys/JOURNEY_J4_CLUB_PRESIDENT.md) — that report is the source of truth; screenshots in the gitignored `tests/uat/results/journeys/J4/`):

- **J4-012 (High)** a coach who belongs to any second org is 403-locked out of the entire coach surface — the portal lists their team but every operation (allocations, payment requests, events) fails; live-verified. 36 coach API files call bare `getAuthContext()` (which resolves the FIRST membership) then 403 on slug mismatch; only 3 routes pass `{ orgSlug }` correctly, including `assignments/route.ts:9`, so the shell happily lists a team the coach cannot operate. Root is J3-012 (`lib/api-auth.ts:90-92`, fix-now); the ask filed here is the sweep scope — pass `{ orgSlug }` into `getAuthContext` in every `resolveCoachContext` under `app/api/coaches/[orgSlug]/**` (36 call sites; pattern exemplar `teams/[teamId]/events/route.ts:16-18`), folded into the J3-012 fix so the coach-route sweep ships with it.
- **J4-034 (High)** "Coaches Portal Links" collides head-on with the built-in coach portal — an org owner's most likely path to onboard their own coach is a jargon dead-end: the page manages link requests from EXTERNAL paid workspaces but never says so, its invite field solicits a "primary coach email" and fails with "No Team workspace owner was found for that email." with no pointer to the real path, while the real path's assign panel dead-ends without a Members link. Ask: one sentence of scope copy on the hub + the not-found error (pointing at Members → team → Coaches), link the assign panel to Members invite (ideally inline invite-by-email pre-filling Coach + auto-assign on accept), consider renaming ("External Portal Links"), and collapse the five zero-count sections behind a first-run empty state in benefit language. Refs: `coaches-portal-links/page.tsx:259-264+305-320`, `AdminSidebar.tsx:280-284`, `lib/team-org-links.ts:405-429`, `coaches/page.tsx:218-221`.
- **J4-037 (Med)** the portal-links hub approves recurring money without showing a price and leaks internal jargon — "Approve Annual"/"Approve Monthly" for org-billed Premium show no dollar figure anywhere (price first appears at Stripe checkout), the ownership card says "Phase 5A records mutual approval…", and the billing-mode label "Platform override" is ops vocabulary shown to a customer. Ask: show the price (annual + monthly) on the approve surface mirroring the rep-teams E4 modal pattern (`rep-teams/page.tsx:662-667`), replace the Phase-5A sentence with a customer-facing next step, relabel "Platform override" ("Complimentary — provided by FieldLogicHQ"). Refs: `coaches-portal-links/page.tsx:504-522+600-602+61-66+285-289`.
- **J4-038 (Low)** the "Club may be a better value now" nudge is not plan-aware — it upsells Club to an org already on Club (count-only check on 3+ org-billed portals, no planId filter); at that threshold the right message for a Club org is the opposite: convert linked portals into owned rep teams. Ask: gate on `planId !== 'club'` and add a Club-specific variant suggesting ownership transfer. Refs: `lib/team-workspace-entitlements.ts:147-149`, `team-links/route.ts:42-48`, `coaches-portal-links/page.tsx:278-292`.
- **J4-047 (Low)** the admin-hub "Coaches Portal" tile is shown ungated to every rep-capable admin and dead-ends a non-coach owner on "No team assignments yet." — the sidebar and org-hub equivalents correctly gate the same link on `useCurrentOrgCoachAccess`, but the dashboard tile gates only on `canSeeRepTeams` (always true for a club owner). Ask: apply the `useCurrentOrgCoachAccess` gate to the tile, or relabel/redirect it to the portal-links admin surface for non-coaches. Refs: `AdminHubClient.tsx:208-213` vs `AdminSidebar.tsx:405-407` + `org/page.tsx:56-61`, `coaches/page.tsx:85-94`.

### Journey-audit inputs — J2 rep head coach + J9 house-league coach (2026-06-13, Phase 4)

The Phase-4 walks route 15 shell/IA findings here (reports are the source of truth: [journeys/JOURNEY_J2_REP_HEAD_COACH.md](journeys/JOURNEY_J2_REP_HEAD_COACH.md), [journeys/JOURNEY_J9_HOUSE_LEAGUE_COACH.md](journeys/JOURNEY_J9_HOUSE_LEAGUE_COACH.md); screenshots gitignored under `tests/uat/results/journeys/J2,J9/`). 0 refuted.

**J2 — the org-attached Premium portal shell/IA (the headline is the lineup gap):**
- **J2-031 (High)** the marquee **lineup builder has zero navigation presence**, a false overview checklist (the "Prepare game lineups" row completes on game-count, never reading a saved `rep_team_lineups` row), and a 680px grid that forces sideways scroll in a 720px mobile sheet — it's embedded in the Schedule game-event slide-over *below* attendance with no nav item. Add a Lineups team-nav item + a per-game "Build lineup" affordance + a tab strip (Details/Attendance/Lineup) in the sheet; base the checklist on a saved lineup row; a mobile-first layout. (Refs: `CoachesSidebar.tsx:10-17`, `teams/[teamId]/page.tsx:225-229`, `schedule/page.tsx:1064-1211`, `coaches.module.css:1111-1122`.)
- **J2-032 (High, design-lens)** the premium portal entry has a mobile H1/breadcrumb title collision and a tournament-upsell card that visually dominates over the one team the coach came to manage. **J2-036 (Med, design-lens)** the Team Calendar reads as a loading failure (two events in a black void) with a blue off-accent Add Event button. **J2-019 (High, design-lens)** the Basic team home reads as a list view, not a polished product (no hero, undifferentiated sections, text-link Add actions, Welcome buried below the financials). **J2-023 (Low)** the standalone TeamHQ strip lacks the team-colour hero the tournament/public variants get.
- **J2-013 (Med)** the always-on "Tournaments" tab dead-ends a never-tournament coach (hub hides the section, shell doesn't). **J2-012 (Med)** the duplicate-email login round-trip offers only org-creation as a fallback + the banned "Tournament Management Platform" sub-copy. **J2-028 (Low)** the navbar "Upgrade" branch is unreachable dead code (SiteChrome nulls the navbar on the exact paths it gates on). **J2-029 (Low)** two labels ("Explore the Coaches Portal" vs "Express interest") for the same gated `/coaches/start` destination.

**J9 — the house-league coach-portal extension (confirms house league is the only module whose operator has no operator surface):**
- **J9-002 (High)** a coach-role member lands on a permanent "No team assignments yet" dead end — a promise the house-league path can never fulfil (the portal recognises only `rep_team_coaches`; the `coach` role grants only `module_rep_teams`); the design lens scored the empty state as a "blank black void that looks broken" (J9-006). **J9-003 (High)** the only coach-onboarding workaround — a `module_house_league` capability override — leaks **all** guardian PII (the registrations GET gates on the module cap alone; authenticated vector, distinct from J3-069's public oracle). Both resolve by introducing a **team-scoped coach relationship** that exposes only that coach's own roster — never gate coach access via the module capability.
- **The extension itself (J9-011/012/013, all Med):** extend the existing `/{orgSlug}/coaches` portal to surface house-league team assignments by unioning a league-team source into the assignments resolver (keyed off `league_teams.coach_email` per the J9-010 keystone in FTS §16), then read-only roster (J9-011, `league_registrations`), a schedule tab (J9-012, `getPracticesForTeam`), and rainout notify (J9-013, `lib/notify.ts` `userIds`) — the portal shell, team-card UI, and routing already exist; only the data source needs to include league teams.

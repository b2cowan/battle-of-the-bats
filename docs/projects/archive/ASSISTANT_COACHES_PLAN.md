...
### Phase 2 — Head-coach administration UI + invites + comms (paid) — ✅ COMPLETE on `dev` (core 2026-07-02, admin remainder 2026-07-04; unpushed; owner browser-test pending)
...
> **Remainder — ✅ BUILT 2026-07-04:** a new admin **Assistant Coaches** page under Rep Teams (linked from the landing) = org-wide list of all assistants (team + granted-areas summary + Remove), a **pending-approvals** section (Approve re-mints the token + emails the assistant / Decline), an **invited-awaiting-acceptance** section (Revoke), and the **"require admin approval" toggle**. Rep-group scope enforced; org-ownership verified per action; no new migration (reuses coach_settings + the invite table). Adversarial 2-lens review folded in (module gate on the settings route, invite-status validation, per-user email lookup, server-authoritative `canWrite`, additive grant summary). ⚠ migs 173+174 → PROD before promoting.

...
- **Phase 5 — Account trim + polish:** Account restructure, install/help/legal rows, unified cross-tab badge policy (invites vs live-count vs unread), success-metrics instrumentation. **BUILT ON DEV 2026-07-20** (uncommitted; typecheck + verify:changed gate green; NO migration; see memory `project_unified_home_redesign.md`). **Owner decisions 2026-07-20:** Legal row **OMITTED + flagged** (no platform ToS/Privacy pages exist anywhere → would 404; writing/linking them is a follow-up for /marketing + legal); Help & support = **mailto** `hello@fieldlogichq.ca` (no consumer-facing help page exists); unified badge policy = **RED "needs you" = Chat unread [built] + Home pending-invites [new]; Scores gets NO nav badge**; **shared nav WARMED** (scoped to the 4 tabs; auth/select-org/suspended stay dark). Metrics reuse the existing `platform_events` store (free-text `event_type`, no migration) — 3 client events (`home_ready`/`home_card_tapped`/`chat_tab_opened` via `/api/events/consumer`) + 4 server events (`directory_search`/`chat_inbox_loaded`/`auth_workspace_landing`). **DEFERRED (flagged):** the `/account/notifications` sub-page warm reskin (kept fully dark + nav dark there = a clean theme handoff, not a mid-page seam — its own focused pass later).
- **Phase 6 — Fast-follow: whole-tournament + org follows:** generalize the follows API beyond `entityType:'team'`, follow buttons on tournament/org surfaces, org landing page (prerequisite — confirm/scope `/{orgSlug}` public page), feed entries for tournaments ("bracket posted", next game day) and orgs ("{n} upcoming tournaments"), Organizations subsection on Home, org rollups in Scores. ~~Account-only for v1 (no device-local path for non-team follows)~~ → **FREE-FIRST (owner 2026-07-20, supersedes the account-only recommendation; logged in BUSINESS_DECISIONS.md):** tournament/org follows work device-locally with no account exactly like team follows — device write + account mirror when signed in; signed-out Home/Scores render them; the explicit claim-on-sign-in offer covers all three entity types; alerts stay account-only; a device tournament/org follow never seeds a my-team pin. Owner explicitly declined the all-account alternative (team-follow device path stays). Decide seeded-pin interaction: recommend fully independent (following a tournament/org never seeds a team pin). **Mockup round F1–F6 ALL RATIFIED 2026-07-20 (artifact `d6a7c08b` rev 3 = binding spec; All-following retained per the F2 clarification; logged to design_decisions.md + BUSINESS_DECISIONS.md). Build plan: `UNIFIED_HOME_PHASE6_FOLLOWS_PLAN.md`.**

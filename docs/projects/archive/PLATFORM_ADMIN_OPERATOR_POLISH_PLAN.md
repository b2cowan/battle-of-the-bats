...
> **Status:** Scoped 2026-06-13. **Priority: P3 — opportunistic polish backlog.** **Batch 1 (Groups B + C + D) BUILT + COMMITTED 2026-06-14 (1ea63ec, not pushed).** **Batch 2 (Group E, all 7 billing items PAB-005/006/009/010/012/013/014) BUILT + COMMITTED 2026-06-14 (546f683, not pushed).** Both pending owner browser smoke-test. **Note:** PAG-004 (Group F, Action-Queue non-billing note) was already shipped as part of F4 (43f8cde) — drop it from F5 scope. **Group A ✅ verified already-done (no-op, 2026-06-14).** **Group F (growth) BUILT + COMMITTED 2026-06-14 (e599520, not pushed) — all 7 items (PAG-005/006/007/008/009/010/011); PAG-004 was F4. ⚠ touches shared lib/email-sender.ts → dev-server restart needed. Reviewed via /review.** **Group H (read-only observer) BUILT + COMMITTED 2026-06-15 (d2c134e, not pushed): PAR-005 (snapshot button hide + API manage_product gate) + PAR-007 (Billing form → read-only summary for observers); PAR-002 was already done (F2). No shared-module edits → no restart.** **Group G (super-admin / nav IA) BUILT + COMMITTED 2026-06-15 (4b6f68e, not pushed) — all 8 items: PA0-005 nav regroup (System → Support & Diagnostics + Governance; Email Templates → Billing & Product; Dev Tools still injects via preserved 'System' label; every item's area gating preserved), PA0-006 Email → "Email Campaigns", PA0-008 "Change Requests" → "Approval Queue" across all user-facing copy (route/area key/`#change-requests` anchor/DB table/API path UNCHANGED) + Plans-&-Pricing relationship HelpCallout, PA0-011 + PA0-013 page kickers aligned to new groups, PA0-012 Delete-Org "Danger Zone" divider + `.dangerSection` (already last card — visual framing only; premise that it was the "first" card was STALE), PA0-009 Customer Users employee-exclusion HelpCallout (CTA→Platform Users gated to super_admin), PA0-014 defensive `requirePlatformAreaView` on orgs/orgs[id]/audit — **3 pages not 4** (customer-users was already guarded); orgs[id] swap of getPlatformAdminContext→requirePlatformAreaView is strictly MORE secure. ⚠ shared PlatformAdminNav.tsx → dev-server restart before browser QA. Reviewed via /review high-risk funnel (4 lenses; nav/security/CSS clean): 2 findings fixed (literal `&amp;` in a HelpCallout title string prop; missed "change requests"→"approval queue" in a Help Hub desc).** **Group I (date-format drift sweep, PF-5) BUILT + COMMITTED 2026-06-15 (06566dd, not pushed):** ONE shared `lib/format-date.ts` applied across all 17 date-rendering surfaces, replacing ~11 ad-hoc helpers. **Context-typed rule** (refined from the audit's blanket "relative <7d", owner-approved): forensic timestamps stay ALWAYS absolute (audit/overrides/feedback/email/change-requests/pricing); freshness signals (last sign-in, note time) relative-when-recent + absolute >7d + exact-time tooltip; deadline "(Nd left)" preserved. `fmtSince`/`fmtDaysLeft` CLIENT-ONLY → 6 server components stay absolute (no hydration mismatch). Operator en-CA locale, NOT Toronto-pinned. 3 drift bugs fixed (observability missing year, email wrong API, plans-pricing 3 granularities). Audit log keeps a documented seconds-precision exception. ⚠ new shared `lib/` module → dev-server restart before browser QA. /review high-risk funnel (3 lenses) = 0 confirmed defects, 2 intentional advisories; typecheck+lint+verify:changed clean. **✅ F5 COMPLETE — all groups built (A no-op; B–I built). Plan ready to archive once owner browser smoke-tests pass.**
...
| ID | Surface | File / Line | Fix direction | Effort |
|----|---------|-------------|---------------|--------|
| PA0-005 ✅ | Nav — System group | `app/platform-admin/PlatformAdminNav.tsx` | **DONE (4b6f68e).** Split "System" into **Support & Diagnostics** (Observability, Feedback) + **Governance** (Platform Users, Audit Log); **Email Templates → Billing & Product**; "System" now holds Help only. **Dev Tools injection preserved** (still keys off the surviving `'System'` group label). Every item's `area` gating unchanged — IA only, no access change (verified by review). | M |
| PA0-006 ✅ | Nav — Email vs Email Templates naming | `app/platform-admin/PlatformAdminNav.tsx` | **DONE (4b6f68e).** Growth "Email" → **"Email Campaigns"**; transactional editor keeps "Email Templates" (kicker resolved via PA0-013). Owner chose Campaigns over "Marketing Email". | XS |
| PA0-008 ✅ | Change Requests → Approval Queue | `change-requests/*`, `plans-pricing/PlansPricingClient.tsx`, `page.tsx`, `help/page.tsx`, `lib/help-content/platform-admin.tsx` | **DONE (4b6f68e).** Owner chose full rename: **"Change Requests" → "Approval Queue"** across ALL user-facing copy (nav label, page title + header, Plans-&-Pricing refs, overview orientation banner, Help Hub cards + SOP heading/links/prose). **Route `/platform-admin/change-requests`, area key `change_requests`, anchor `#change-requests`, DB table, and API path intentionally UNCHANGED.** Added a HelpCallout on the queue page explaining the Plans-&-Pricing relationship. | XS→S |
| PA0-011 ✅ | Feedback / Observability — header kicker | `feedback/page.tsx`, `observability/page.tsx` | **DONE (4b6f68e).** Both kickers "System" → **"Support & Diagnostics"** (matches the resolved nav group). | XS |
| PA0-012 ✅ | Org detail — Delete Organization danger framing | `orgs/[id]/OrgDetailClient.tsx`, `orgDetail.module.css` | **DONE (4b6f68e).** Premise was STALE — the Delete card was **already last** in the Support tab, not first. Reframed to the real value: a **"Danger Zone"** divider + `.dangerSection` (red-tinted border/bg via `--danger-rgb`) so it reads as destructive. No logic change. | XS |
| PA0-013 ✅ | Email Templates — header kicker | `email-templates/page.tsx` | **DONE (4b6f68e).** Kicker "Platform Admin" → **"Billing & Product"** (Email Templates now lives in that nav group). | XS |
| PA0-014 ✅ | ALL_ROLES pages — defensive guards | `orgs/page.tsx`, `orgs/[id]/page.tsx`, `audit/page.tsx` | **DONE (4b6f68e). 3 pages, not 4** — `customer-users/page.tsx` was ALREADY guarded (`requirePlatformAreaView('customer_users')`). Added `requirePlatformAreaView('organizations')` to orgs + orgs[id] (the latter **replaces** the non-redirecting `getPlatformAdminContext()` call — strictly more secure), `requirePlatformAreaView('audit')` to audit. Zero behavior change today (all three are ALL_ROLES); future-proofs against a later matrix tightening. | XS |
| PA0-009 ✅ | Customer Users — employee exclusion note | `customer-users/CustomerUsersClient.tsx` | **DONE (4b6f68e).** Dismissible HelpCallout: "Platform employees are excluded from this list… use Platform Users." CTA → Platform Users **gated to super_admin** (`canDelete`) so non-super-admins don't link to a page they can't view. (Lives in the client, not page.tsx, where the header renders.) | XS |

### Group H — Read-only observer polish

| ID | Surface | File / Line | Fix direction | Effort |
|----|---------|-------------|---------------|--------|
| PAR-007 ✅ | Org detail — Billing tab | `app/platform-admin/orgs/[id]/OrgDetailClient.tsx` | **DONE (d2c134e).** The plan/limit summary grid (Current Plan / Limit / Tournaments) stays for all roles; the editable Plan select / Limit input / Reason + Review button now render only when `canManageBilling`. Observers see the read-only summary + the existing "View-only" note instead of disabled fields. | S |
| PAR-005 ✅ | Overview — MetricSnapshotButton | `app/platform-admin/MetricSnapshotButton.tsx`, `page.tsx`, `app/api/platform-admin/metrics/snapshot/route.ts` | **DONE (d2c134e).** API now `requirePlatformPermission('manage_product')` (was bare `requirePlatformAdmin` = any role incl. read_only — closed the accidental-write channel). Button takes `canSnapshot` (hasPlatformPermission(role,'manage_product')) and returns null otherwise. | XS |
| PAR-002 ✅ | Org detail — Coaches Portal textarea | `app/platform-admin/orgs/[id]/OrgDetailClient.tsx` | **ALREADY DONE (verified 2026-06-15).** The transfer textarea + button already render only when `canManageSupport \|\| canManageBilling`; observers get a "Support or billing permission is required" note. Likely landed in F2. No change. | XS |

### Group I — Date-format drift (cross-surface)

| ID | Surface | File / Line | Fix direction | Effort |
|----|---------|-------------|---------------|--------|
| PF-5 ✅ | All platform-admin surfaces | NEW `lib/format-date.ts` + 17 surfaces | **DONE (06566dd).** Discovery found 17 of 21 surfaces render dates, almost all already absolute (only 2 used any relative). Built ONE shared `lib/format-date.ts` (`fmtAbsoluteDate`/`fmtAbsoluteDateTime`/`fmtSince`/`fmtDaysLeft`) and replaced ~11 ad-hoc local helpers. **Refined the rule from the audit's blanket "relative <7d" to a CONTEXT-TYPED rule** (owner-approved): forensic/legal/precision timestamps stay **ALWAYS absolute** (audit log, override created/revoked, feedback submitted, email sent, change-request history, catalog/pricing changes) — operators need exact times; "freshness" signals (last sign-in, support-note time) render **relative-when-recent (≤7d) + absolute beyond**, with the exact timestamp on hover; deadline "(Nd left)" framing preserved (override expiry). **`fmtSince`/`fmtDaysLeft` are CLIENT-ONLY** (read `Date.now()`) → the 6 server components stay absolute (no hydration mismatch / stale relative). **Locale = operator browser (en-CA), NOT Toronto-pinned** (internal operator tooling; pinning would reintroduce server/client mismatch). **3 drift bugs fixed:** observability "Last seen" was missing the year; email dashboard called `toLocaleDateString()` with time options (wrong API); plans-pricing rendered 3 different granularities (one no-year). Audit log keeps a **documented local exception** for second-level forensic precision. `snapshot_date` (overview) left as the raw Postgres `date` string (reformatting risks TZ off-by-one). Presentation-only — no permission/data/behavior change. typecheck + lint (0-err) + verify:changed clean; /review high-risk funnel (3 lenses) = 0 confirmed defects, 2 intentional advisories. ⚠ new shared `lib/` module → **restart dev server** before browser QA. | M |

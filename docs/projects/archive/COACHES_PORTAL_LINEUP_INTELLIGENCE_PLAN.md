...
- New **Pitching** mini-section: "Is a pitcher?" toggle → reveals Rank (Ace / #2 / #3 …) and Max innings/game.
  - **The mound (P) is ALWAYS excluded from the Best/Okay/Never fielding chips** (built 2026-07-02, owner UX call: showing both a "P" chip and a "This player pitches" toggle was confusing). Pitching is managed *only* in the Pitching section (toggle + rank + cap) — one place, one concept. `playerPositionPrefs` and `buildLineupProfileWrite` strip the mound from every fielding bucket unconditionally, so P can never be stored or shown as a fielding preference. A player who "can pitch in a pinch" is flagged a pitcher with a low rank; the generator still fills the mound from a non-pitcher only as a last resort when no eligible pitcher exists.

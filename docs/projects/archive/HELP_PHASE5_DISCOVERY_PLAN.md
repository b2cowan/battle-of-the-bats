...
**Status:** **Phase 5a + the 5a fast-follow + 5b persona panel all BUILT on `dev` 2026-06-20** (owner-approved; commits `bea5558` → `747f539` (review) → `f07b53d` (fast-follow + persona), awaiting browser sign-off). **Still deferred (held for explicit go):** cross-device dismiss (needs a schema change) + the page-level capability spotlights (data-import, fan-alerts) on individual pages.
...
>
> **Build status (5a fast-follow + 5b, commit `f07b53d`):** **D4b shipped** — the shared `HelpDrawer` now renders the same lifecycle-filtered "I want to…" shortcuts at the top, on **every** tournament work page's "?", with zero per-page wiring (the drawer is mounted inside Org+Tournament providers, so it reads context directly; guarded to module `tournaments` so the coaches drawer is untouched). `getStageShortcuts` now takes `{orgSlug, planId}`; added `daysUntilStart()` so the drawer computes stage during render without the purity error. **D3 persona panel shipped** — `PersonaPanel.tsx` + `.module.css`, collapsible "What everyone else sees" on the dashboard (draft + active), three audience cards linking to preview / registration / Staff Kit, plan-accurate fan-alerts copy, auto-opens once on first-time draft then stays collapsed (per-tournament localStorage, SSR-safe). `lint:focused` 0 errors / `typecheck` clean; running dev server compiles the route (307). **Deferred:** cross-device dismiss (schema change), page-level spotlights, post-event "next steps" row (folded into the completed rail).

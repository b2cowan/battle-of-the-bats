...
**`practice_plan_sent_at` / `_by` / `_audience` / `_count` / `_email`** (timestamptz / uuid → `auth.users` SET NULL / text CHECK `named|coaches|staff` / smallint / boolean; all nullable; **mig 303**) — the LAST "Send to staff" on this practice (`COACH_PRACTICE_WHO_RUNS_IT_PLAN.md`, ruling I): when, who pressed it, which audience (*named* = everyone whose linked staff tag is on the plan; *coaches* = head, assistants, helpers; *staff* = everyone), how many it reached, and whether the coach's own email went with the bell and push. The `family_shared_at/by` idiom — the last send only, overwritten by the next; nothing reads a history. Written by `stampRepTeamEventPracticePlanSent` (season-scoped, and deliberately WITHOUT touching `updated_at` — sending is not editing). NULL on a practice never sent, and on every game. ⚠ `_email = true` records that the coach's email WENT (the sender reported at least one delivered — a provider error or a missing key leaves it false; the tick itself is remembered by the sheet, not here) — and it went **regardless of the recipients' notification preferences and the master pause** (ruling J — a colleague's addressed message, on the dues-reminder ground; `lib/practice-plan-email.ts`).

...
**`actual_source`** (text, **NOT NULL**, default `'typed'`; CHECK `typed|fundraiser|sponsor`; added mig 280) — **who reports this word's actual figure**: `typed` = the coach records each arrival themselves, `fundraiser` = the team's drives report it, `sponsor` = the team's sponsors report it. ⚠⚠ **THIS IS WHAT A TEAM BUDGET LINE'S `line_kind` IS DERIVED FROM**, and it is why the add-a-line form asks ONE question instead of two. It used to ask *"This line is: Expense / Expected fundraising / Expected sponsorship / Expected other income"* **and then** *"Category & Item"* — and because the picker filters by `direction` and never by the kind just chosen, *Expected sponsorship* + *Tournaments → Concession revenue* was offerable. That pairing is a trap, not untidiness: the kind decides where the row's actual comes from, a sponsorship line takes its actual from sponsor arrivals and **refuses a typed income record** (so the same dollar is never counted twice — `lib/coach-money-derived.ts`), so the coach got a budget line they could never record their concession takings against with nothing on screen saying why. The mapping is `direction='out'` → `cost`, else `fundraiser`→`funding`, `sponsor`→`sponsorship`, `typed`→`other_income` — the exact inverse of `LINE_KIND_ACTUAL_SOURCE`, computed in one place (`budgetLineKindForItem`, `lib/coach-budget-totals.ts`) with a unit test pinning both directions in step. ⚠ **The stored enum is untouched**: removing the QUESTION is a form change; removing the CONCEPT would be a migration across six readers. ⚠ **Default `'typed'`, and no write path accepts it** — a word a club or a coach invents has no machinery behind it, so it is born typed and stays typed; defaulting it to a derived source would CLOSE its rows to typed income, which is the only way their money can be recorded. That is a real narrowing accepted on purpose (owner, 2026-09-07): a coach who invents their own income word can no longer have the drive machinery report against it and uses the standard *Fundraising drive* instead. ⚠ **A money-OUT word is always `typed`**, enforced by `budget_items_out_is_typed_check` — safe because a word never changes sides (owner ruling 2026-08-17; the coach item PATCH refuses a `direction` in the body outright). The derivation asks `direction` FIRST anyway, so the constraint is a belt, not the trousers. ⚠ **`Grant` is `sponsor` while KEEPING its `Fundraising` category** — it is given directly and arrives as a cheque, and the sponsorship option's own help text has always called a grant a sponsorship; whether it also *moves* category is a separate decision, deliberately left open, because the category is the report's shelf and moving an item renames rows a treasurer has already read and exported. ⚠ **No backfill was needed and no figure moved**: verified on dev AND prod 2026-09-07, every money-in budget line in existence points at a **platform** item and already used the obvious pairing, and there are no club- or coach-created money-in items anywhere.

...
<!-- dict:col:organizations.club_book_sharing_enabled -->
...
**`list_in_directory`** (boolean, NOT NULL, default `true` since mig 197; was `false` in mig 158) — controls whether the tournament appears in the public cross-platform discovery directory ([/discover](../../../app/discover/page.tsx)). **Default flipped to `true` (opt-OUT) in mig 197** (owner decision 2026-07-22 — most events want to be found): a NEW tournament lists automatically; the organizer hides it by turning off the toggle in Event Settings → Tournament Overview. **No backfill** — existing rows keep their stored value, so a historically-unlisted event stays unlisted unless its organizer opts in. **ANDed with the public-status gate** at directory query time (`status IN ('active','completed')`), so a default-listed **draft** never surfaces until it goes active — the directory introduces no second visibility model and only links to already-public pages (player PII stays behind the existing `toPublicTeam` choke point). Available on **all plans** (no tier gate). Set via the `update` action in [api/admin/tournaments/route.ts](../../../app/api/admin/tournaments/route.ts); mapped as `listInDirectory` in `mapTournament` ([lib/db.ts](../../../lib/db.ts)). Partial index `tournaments_list_in_directory_idx` (`WHERE list_in_directory = true`) supports the platform-wide directory query. _Dev/prod:_ mig 158 on prod (2026-06-29); ⚠ mig 197 **DEV-only / prod-pending**.
...
<!-- dict:col:tournaments.chat_reminder_last_sent_at -->
**`chat_reminder_last_sent_at`** (timestamptz, nullable; mig 206) — when the **chat sign-up reminder batch** was last claimed/sent for this tournament. NULL = never sent. Written **only** by the batch route ([api/admin/tournaments/[tournamentId]/chat/remind-signups/route.ts](../../../app/api/admin/tournaments/%5BtournamentId%5D/chat/remind-signups/route.ts)). Drives the **24h per-tournament cooldown** (owner decision **S1-A**, 2026-07-29, Coach Chat program Step 1): the route rejects a blocked POST with **429** + a human-readable `userMessage`, and the dashboard's "Coach Sign-ups & Chat" panel renders a disabled "Reminded N hours ago" button from the same stamp. **Per tournament, deliberately not per coach or per sender** — so two co-organizers on one event cannot double-mail the same pending coaches.
**⚠ This is a CLAIM, not just a record — read it with `chat_reminder_last_sent_count`.** The window is claimed by a single conditional UPDATE **before** any email is sent (`.or(at IS NULL, at < now-24h, and(count IS NULL, at < now-10min))`), so the check and the write are atomic and a concurrent second click loses the race and gets the 429 — a read-then-check-then-send sequence would leave the whole multi-second batch inside a gap where both requests pass. If the batch then reaches nobody (resolver failure, zero targets, zero deliveries) the claim is **released**, restoring all three columns to their pre-claim values. **A row with a recent `_at` and a NULL `_count` is a claim in flight or a stranded claim**, reclaimable after a 10-minute grace — this is what stops a failed release write (usually the same outage that broke the send) from phantom-locking the organizer out for 24h. Verified against dev 2026-07-29 across all six branches incl. the concurrent race.
Also surfaced read-only on the dashboard funnel payload as `reminderLastSentAt` ([api/admin/tournament-dashboard/route.ts](../../../app/api/admin/tournament-dashboard/route.ts)). The client mirrors the stamp for the current session so the button stands down instantly rather than on the next 30s poll — **the server check is authoritative**. Not mapped in `mapTournament` (dashboard-only concern). _Dev/prod:_ mig 206 **applied to DEV 2026-07-29** (⚠ prod-pending — the dashboard + send routes both SELECT this column, so **prod MUST get mig 206 before this code promotes**; `npm run check:migrations` gates it).

<!-- dict:col:tournaments.chat_reminder_last_sent_by -->
**`chat_reminder_last_sent_by`** (uuid, nullable, `REFERENCES auth.users(id) ON DELETE SET NULL`; mig 206) — who last fired the chat sign-up reminder batch. **Audit only** — the cooldown keys on `chat_reminder_last_sent_at` alone, never on the sender, so removing a staff account can never re-open a send window (hence `ON DELETE SET NULL` rather than cascade or restrict). Not read by any UI today; exists so "you emailed me three times" has something to check. _Dev/prod:_ mig 206 **applied to DEV 2026-07-29** (⚠ prod-pending).

<!-- dict:col:tournaments.chat_reminder_last_sent_count -->
**`chat_reminder_last_sent_count`** (integer, nullable; mig 206) — how many teams the last chat sign-up reminder batch **actually reached**. Counts `sendEmail()` results whose status is `'sent'` — **not** settled promises: `sendEmail` resolves (never throws) with `'skipped'` on a missing API key and `'provider_error'` on a Resend rejection, so counting fulfilled promises would report a full send and arm a 24h lockout for a batch that delivered nothing.
**Doubles as the claim-completion marker (see `chat_reminder_last_sent_at`):** set to NULL when the window is claimed and written only once the batch finishes, so `NULL` = in-flight-or-stranded and non-NULL = completed. That makes it load-bearing for the cooldown's stranded-claim recovery, **not** pure audit context. _Dev/prod:_ mig 206 **applied to DEV 2026-07-29** (⚠ prod-pending).
...
**`coach`** (text) — coach **display name** (gotcha 6). _Dev/prod:_ prod's NOT NULL is a **live failure path** — the registration bulk-import writes `coach: string | null` ([import/shared.ts:116](../../../app/api/admin/tournaments/[tournamentId]/registrations/import/shared.ts#L116)), so an import with a blank coach column **fails on prod** and succeeds on dev (49/229 dev rows are null). Migration 202 loosens prod — **applied to BOTH envs 2026-07-27**. ✅ **Absence is now spelled ONE way: `NULL`** (migration 204, both envs 2026-07-27). Until then the interactive admin path coerced to `''` to satisfy prod's old NOT NULL, so **17 of prod's 21 teams carried `coach=''`** while dev used `NULL` — two encodings of "absent" in one column. 204 normalized the 17 rows (no name was altered; prod's 4 real names untouched) and the write path is now `|| null` ([admin/teams/route.ts:321](../../../app/api/admin/teams/route.ts#L321)) so the split cannot re-open. **Why it mattered despite being invisible:** every reader falls back on falsy (`team.coach || team.name`, `team.coach ?? ''`), so behaviour never differed — but `WHERE coach IS NULL` silently missed 17 of 21 prod teams. The coaches self-serve PATCH still 400s on an empty submitted name — now a **product** rule (a coach shouldn't blank their own team name), no longer a schema constraint.
...
3. **`team_workspace_id` = the Basic→Premium bridge, bidirectional.** NULL for a pure free team; set when the coach upgrades — or when a paid workspace mints its own shadow (2026-09-13, `source='premium_upgrade'`; every owned workspace has one since mig 297). Written together with `team_workspaces.basic_coach_team_id` (forward write at [lib/team-workspace-provisioning.ts:287](../../../lib/team-workspace-provisioning.ts#L287), reverse at [:301-304](../../../lib/team-workspace-provisioning.ts#L301-L304)). Treat it as the "has this free team been monetized?" flag.
...
**`source`** (text, NOT NULL, default `'tournament_registration'`; CHECK `tournament_registration|coach_created|premium_upgrade|backfill`) — provenance. Runtime writes `'tournament_registration'` (register→link), `'coach_created'` (on-ramp), and — since 2026-09-13 — `'premium_upgrade'` for the free-team SHADOW a paid workspace mints for itself (provisioning, the lazy resolver, and mig 297's backfill; see `team_workspaces.basic_coach_team_id`). `'backfill'` is migration-only. Write-only metadata (no reader yet).
...
**Gotchas (read first):**
1. **This row replaced email matching.** `canUserAccessTournamentRegistration` grants access solely by finding a row here + confirming team ownership ([lib/basic-coach-teams.ts:1360-1377](../../../lib/basic-coach-teams.ts#L1360-L1377)). It returns the **linked `basic_coach_team_id`** (truthy) or `null` — as of B3 (2026-07-27) it no longer returns the opaque `'explicit'` marker, because access and team identity are the same fact from the same query and the record page was running the lookup twice. Callers that need only yes/no read it as truthy; **never re-query for the id after calling it**. The old `teams.email ILIKE` model is gone (mig 092 dropped the `'email_fallback'` source).
2. **`tournament_team_id` UNIQUE** — a registration links to **at most one** basic team. Built by `linkTournamentRegistrationToBasicCoachTeam` ([lib/basic-coach-teams.ts:338](../../../lib/basic-coach-teams.ts#L338)), called from the register POST ([register/route.ts:368](../../../app/api/register/route.ts#L368)) and the portal claim ([coaches/basic-teams/route.ts:85](../../../app/api/coaches/basic-teams/route.ts#L85)) — both of which make the coach PROVE ownership (registration email = signed-in email). **Third writer (2026-09-13):** the hosted-tournament "Add my team" write ([lib/host-own-team.ts](../../../lib/host-own-team.ts)) inserts the row directly with `link_source='explicit'` — no proof needed, the server holds the whole chain (org → workspace → rep team → shadow) and the registration is written as the head coach.
...
2. **Never an email match; never auto-linked.** Rows are written only by the passive admin "Link to rep team" control (`link_source='explicit'`), the one-time backfill tool (`link_source='backfill'`), and — since 2026-09-13 — the hosted-tournament **"Add my team"** write ([lib/host-own-team.ts](../../../lib/host-own-team.ts), `link_source='explicit'`), where the org IS the workspace org and the rep team is that workspace's, so both tenant assertions are the same org. There is no registration-flow auto-link (unlike the basic bridge) — the shared `/register` route has no rep-team concept.
...
**`basic_coach_team_id`** (FK → `basic_coach_teams.id`, nullable) — the **reverse free→paid bridge** (kept in sync with `basic_coach_teams.team_workspace_id`). **Since mig 297 (2026-09-13) every owned workspace has one** — the free-team SHADOW the coach-side tournament record, roster submission and the public register form's picker all key on. Three sources, in order: the upgrade/claim wrote it at provisioning; the source registration's free team is adopted on first read; else provisioning (and, lazily, `resolveBasicCoachTeamIdForWorkspace` in [lib/basic-coach-teams.ts](../../../lib/basic-coach-teams.ts)) **mints** one named after the rep team, `source='premium_upgrade'`, owned by `primary_owner_user_id`. Mig 297 backfilled the stock. Nullable only for a workspace with no primary owner (nothing to own the shadow). The from-scratch defect this closes: a direct-signup workspace resolved NULL forever, so its coach registering on any public page minted a separate free team the paid portal could never see (COACH_HOST_OWN_TEAM_ENTRY_PLAN.md F04).
...
**`primary_position` / `secondary_position`** (text, nullable; mig 070) — **free text, no CHECK** (any string accepted). Not populated by tryout conversion. **Remain authoritative for the top-two "Best" positions** even after `lineup_profile` (mig 171) — the profile only carries ranks 3+, "Never", pitcher & A-squad data (the third "Okay" bucket was folded into Best by migration 291, 2026-09-12). Readers reconstruct the full preferred list as `[primary_position, secondary_position, ...lineup_profile.morePreferred]`.

<!-- dict:col:rep_roster_players.lineup_profile -->
**`lineup_profile`** (jsonb, nullable; mig 171) — Lineup Intelligence player profile that enriches lineup auto-fill beyond primary/secondary. **Additive**; NULL on legacy rows and on rows created by non-picker paths (quick-add, tryout-accept, season rollover) — the generator/readers fall back to `primary_position`/`secondary_position` alone when it's NULL. Shape (**app-enforced in `lib/lineup-profile.ts`, NO DB CHECK** so the vocabulary can evolve): `{ morePreferred: string[] (Best ranks 3+), never: string[] (hard exclusions the auto-fill never assigns), pitcher: { rank: number, maxInnings: number|null } | null (P2; null = not a pitcher), aSquad: boolean (P4; gold-medal starter) }`. **Three states per position** (owner, 2026-09-12): a position is a ranked Best, a Never, or absent from both — and absent means *fine anywhere they're not Never*. **`canPlay` (the old fourth "Okay" bucket) no longer exists**: migration 291 (data-only, invisible to the schema gates) appended each player's Okay positions to their Best tail and removed the key; the normalizer ignores a stray `canPlay` on read and drops it on the next write, so a row the migration has not reached still loads. Positions validated against the team's Sport Pack (`lib/sports.ts`) at write time. See docs/projects/active/COACHES_PORTAL_LINEUP_INTELLIGENCE_PLAN.md and COACH_DEPTH_CHART_THREE_STATES_PLAN.md.
...
**`capabilities`** (jsonb, nullable; mig 173) — per-assistant capability grants (`AssistantCapabilityGrants`, `lib/coach-capabilities.ts`). NULL = assistant least-privilege defaults; **ignored for head coaches**. App-shaped, no DB CHECK (loose-jsonb pattern). Set by the head coach via the coach-portal staff panel; read into the effective `CoachCapabilities` on every `CoachingAssignment`. ⚠ **ONE key is read by the DATABASE as well as the app** (mig 292, 2026-09-12): `development` (boolean) — the Development grant. The write policies on the five development tables admit `coach_role = 'head_coach' OR (capabilities->>'development') = 'true'`, so this is the first grant whose spelling is load-bearing in SQL; rename it and the policies must move with it.
...
**Purpose:** the unified per-team, per-season calendar — practices, games (`league_game` — labelled **"Game"** since 2026-09-20 — and `tournament_game`), multi-day `external_tournament`s, and generic `team_event`s. ⚠ **`scrimmage` is NOT a kind since mig 306**: a scrimmage is a Game with `is_scrimmage` set (see that column and gotcha 8).
...
4. **Recurrence is practice-only and timezone-naive** — `isRecurring` on any non-`practice` type is silently ignored; occurrence dates are built from local-time strings with no offset (can shift across a UTC boundary). (Superseded by gotcha 2's parenthetical for league games and team events; since mig 306 a ticked Game repeats too — the box rides onto every occurrence, D5.)
...
**`event_type`** (text, NOT NULL; CHECK `external_tournament|tournament_game|league_game|practice|team_event` since mig 306 — `scrimmage` was folded into `league_game` + `is_scrimmage` and dropped from the CHECK) — validated app-side against the same list. `league_game` is the stored key for what every coach surface calls a **"Game"** (owner ruling D1, 2026-09-20; the key stays because renaming an enum value is a migration across every reader for no product value). ⚠ The kind is **fixed once an event exists** (the PATCH route never reads one) — right for the kinds that carry structure (a tournament game's parent, a tournament's days, a practice's plan, a series' children), and the reason "scrimmage" had to stop being a kind: the one thing a coach needed to change between two game kinds is now a box. ⚠ The row mapper (`mapRepTeamEvent`) still normalizes a stored `scrimmage` to `league_game` + `is_scrimmage` — the belt for a row written by pre-306 code in the window between the migration and the deploy; delete once prod is folded.
...
**`practice_plan`** (jsonb, nullable; mig 213) — Practice Plans slice 1a: the structured plan for **this** practice. Shape: `{ version, goal?, description?, includeFocusAreas?, kit?, blocks: [{ id, shape: 'activity'|'rotation', title, description?, goal?, duration: { minutes | toMinutes | restOfPractice }, staff?[], playerIds?[], equipmentTagIds?[], coachingPoints?[], stations?[], rotation?{ totalMinutes, intervalMinutes, groups[], groupSource, arrangement?{ stationIds[], groupIds[], rounds, placements[round]{ groupId: stationId | null } } } }] }`. `rotation.arrangement` (practices re-evaluation stage 3, D14, 2026-09-16) is a HAND-ARRANGED grid — which group stands at which station each round, `null` = sits that round out — read by `computeRotation` instead of the standard carousel only while it still FITS the block (same named stations and groups by id, same round count); `settleArrangements` (part of `settlePlanLevels`, run by the sanitiser and the editor alike) drops one that no longer fits, and the editor says so. No DB change — a key inside the document. **App-validated/capped, NOT by DB constraints** ([lib/rep-practice-plan.ts](../../../lib/rep-practice-plan.ts) — `sanitizePracticePlan` runs on every write **and on every read**, so a row written before a cap tightened can't reach a surface malformed). Caps: 30 blocks, 12 stations/block, 12 groups, 8 staff or coaching points per item. Stored NULL when the plan is empty. Written **only** by `updateRepTeamEventPracticePlan` via `PUT …/events/[eventId]/practice-plan` (head-coach-only per D3); reads ride the `schedule` capability.
...
5. **Kit lives at exactly ONE level — the activity's (practices re-evaluation stage 2, owner ruling D11, 2026-09-15).** A block carries `equipmentTagIds` only while it has NO stations; once it has any, the sanitiser MOVES the block's list rather than deleting it — onto a written first station (that station's own ids first, then the block's, capped) or, when the first station is a drill, UP into the plan-level `equipmentTagIds` (the plan's own first). Idempotent: after the move the block holds none, so a second pass (every read runs the sanitiser) finds nothing to move. Contrast `playerIds`, which the same situation still DELETES (invariant 2's cousin — recorded, stage 3's). The plan-level list is what the coach stores as extras; what the practice SHOWS and PRINTS is the derived BAG (`practiceKitBag`: plan ∪ blocks ∪ stations, as names, each once), never written down.
...
⚠ **`staff` entries are LABELS, never grants** — plain names (an outside instructor or PD coach is commonly not a portal user at all). No account, no invitation, no capability implication.

⚠ **`staffTagIds`/`equipmentTagIds` (mig 266) are the CURRENT picker's storage, at up to FIVE nested levels** — plan-level `equipmentTagIds`; each block's `staffTagIds` and (while it has no stations — invariant 5) `equipmentTagIds`; each station's `staffTagIds` and `equipmentTagIds`. Real ids into `rep_team_tags` (`kind='staff'`/`'equipment'`), validated against the team's live library by `sanitizePracticePlan`'s `validStaffTagIds`/`validEquipmentTagIds` params — same two-step check `playerIds` already gets against the roster. The legacy `staff`/`equipment` STRING fields are kept, untouched, on any plan saved before this shipped or before a coach re-edits that field; a level carries EITHER shape, never both at once. **Display-only resolution** (ids → current names, falling back to the legacy string) is `resolvePracticePlanTagNames` in `lib/rep-practice-plan.ts` — every reader (run screen, printed sheet, the history archive) calls it rather than reading `.staff`/`.equipment` off a raw unresolved plan. See `rep_team_tags` gotcha 6: a rename or merge of one of these tags changes what an ALREADY-SAVED practice displays, which is a deliberate, informed departure from this column's own "editing since cannot rewrite what June's practice says" principle (§10.3), made for this pair of kinds only.
...
<!-- dict:col:rep_team_events.is_scrimmage -->
**`is_scrimmage`** (boolean, **NOT NULL**, default `false`; mig 306) — **"This is a scrimmage."** A Game (`event_type = 'league_game'`) that stays on the schedule and in attendance like any game but is **LEFT OUT of the season record and the Scouting Book's numbers**, and whose lineup builder opens on Development. ⚠⚠ **THIS IS HALF THE RECORD RULE.** `countsTowardRecord` (lib/season-wrapped.ts) = kind in `WRAPPED_RECORD_EVENT_TYPES` **AND** `is_scrimmage = false`; every SQL reader pairs `.in('event_type', …)` with `.eq('is_scrimmage', false)` (getRepTeamHistory, getRepCurrentSeasonSummary, the masthead feed), every in-memory reader calls the predicate. A reader that filters on the kind alone counts scrimmages — the bug this column replaced a whole KIND to prevent. ⚠ **Only meaningful on `league_game`**: the events POST/PATCH coerce it to `false` for every other kind through `scrimmageFlagFor` (a tournament game is scored by its organizer; a private "doesn't count" would put two records in disagreement — owner ruling D3, 2026-09-20), and **`rep_team_events_scrimmage_is_a_game_check`** (`CHECK (NOT is_scrimmage OR event_type = 'league_game')`, added to mig 306 by `/review` 2026-09-21) refuses the row outright, so a script or a future writer that forgets the coercion gets an error rather than a scrimmage on a practice. ⚠ **Editable at any time, before or after a result, with immediate effect** — no confirm, by design. ⚠ Mig 306 folded every `scrimmage` row here (dev 4, prod 2 at authoring) and re-derived ONLY the names the product wrote ("League Game vs X" / "Scrimmage vs X" / a bare kind word with an opponent → "vs X" / "@ X" by `home_away`; a bare "League Game" or "Scrimmage" with no opponent → "Game", D2) — a typed name is never touched; the same predicate is `isAutoShapedName` in the vocabulary module (which, since `/review` 2026-09-21, also asks the SIDE: "vs X" on an away game is the coach's, because the product would have written "@ X"). ⚠ Every builder of a Season Wrapped input MUST pass the box — `WrappedGameInput.isScrimmage` is required, because the two assemblers built theirs without it and the shareable card counted every scrimmage (found by `/review` 2026-09-21 before it shipped). ⚠ There is **no team-level "count scrimmages" switch** and there was not one since 2026-09-02 (the per-device preference's only control was deleted as dead code; the reader outlived it until 2026-09-20) — owner ruling D4: the per-game box is the finer instrument.
...
<!-- dict:col:rep_team_events.practice_plan_sent_at -->
<!-- dict:col:rep_team_events.practice_plan_sent_by -->
<!-- dict:col:rep_team_events.practice_plan_sent_audience -->
<!-- dict:col:rep_team_events.practice_plan_sent_count -->
<!-- dict:col:rep_team_events.practice_plan_sent_email -->
<!-- dict:col:rep_team_events.practice_plan_sent_to -->
**`practice_plan_sent_at` / `_by` / `_audience` / `_count` / `_email` / `_to`** (timestamptz / uuid → `auth.users` SET NULL / text CHECK `named|coaches|staff|chosen` / smallint / boolean / uuid[]; all nullable; **mig 303**, `chosen` + `_to` **mig 305**) — the LAST "Send to staff" on this practice (`COACH_PRACTICE_WHO_RUNS_IT_PLAN.md`, ruling I): when, who pressed it, which audience (*named* = everyone whose linked staff tag is on the plan; *coaches* = head, assistants, helpers; *staff* = everyone; *chosen* = the names the coach ticked — "Just these people", owner ask 2026-09-20), how many it reached, whether the coach's own email went with the bell and push, and **who it reached** (`_to`: user ids, every audience, no FK — a record of a past act; the plan page names them on a *chosen* send's sent line and falls back to the count when an id no longer resolves against the current staff; NULL on every stamp from before mig 305, read as `[]`). The `family_shared_at/by` idiom — the last send only, overwritten by the next; nothing reads a history. Written by `stampRepTeamEventPracticePlanSent` (season-scoped, and deliberately WITHOUT touching `updated_at` — sending is not editing). NULL on a practice never sent, and on every game. ⚠ `_email = true` records that the coach's email WENT (the sender reported at least one delivered — a provider error or a missing key leaves it false; the tick itself is remembered by the sheet, not here) — and it went **regardless of the recipients' notification preferences and the master pause** (ruling J — a colleague's addressed message, on the dues-reminder ground; `lib/practice-plan-email.ts`).

<!-- dict:col:rep_team_events.family_shared_at -->
<!-- dict:col:rep_team_events.family_shared_by -->
**`family_shared_at` / `family_shared_by`** (timestamptz / uuid → `auth.users`, both nullable; **mig 215**) — ⚠ **LEGACY, READ AND WRITTEN BY NOTHING since 2026-09-20.** They recorded Chunk D 1.8's "Share game link": when a coach explicitly shared THIS game's public page (`/{org}/teams/{slug}/games/{id}`), and who. The owner removed that feature outright — the coach's Share control, the public page, the share route, and the "Game page →" links on the family schedule, the public team page and the calendar feed — because the page was not production-ready. **Retained, not dropped:** a column drop is a production-ordered migration and two nullable columns nobody reads are not worth one; rows shared before the removal keep their stamp as history. Partial index `(team_id) WHERE family_shared_at IS NOT NULL` stays with them. If a later migration drops them, take the index too and delete this entry.
...
4. **`status` resets to `'draft'` on every ordinary save made BEFORE the event's start time** (`upsertRepTeamLineup` sets `status`/`ready_at`/`ready_by` back to draft/null/null; mig 304). **At or after the event's `starts_at` the same save leaves all three columns untouched** (`keepReadiness`, D11d 2026-09-20 — the lineup route's `PUT` decides from the server-read event and `gameHasStarted`; a Game-Day Mode substitution rides this same PUT and must not un-ready the plan the coach took to the field). Only `markRepTeamLineupReady` (the lineup route's `PATCH`, "mark ready" only) ever writes `'ready'`, and since D11 it is gated on *wrongness*, not completeness: the saved grid must have at least one assignment and no position clash; open roles never block, so a `'ready'` row may legitimately hold blank cells. There is deliberately no manual "revert to draft" action; the only way back to Draft is another edit before game time.
...
3. **INSTRUMENT, not record (owner ruling, ratified 2026-08-04)** — live-season-only surfaces; routes must never join a season the caller names (the guard's `HISTORY_ENDPOINTS`) (asserted by `tests/unit/coach-history-endpoint-guard.test.ts`).
...
6. **The read is WRITE-gated and LIVE-SEASON-ONLY.** `/api/coaches/[orgSlug]/teams/[teamId]/money-imports` requires money **write** (a history of who changed the budget is not offered to a read-only assistant) and deliberately does **not** serve a season the caller names — importing is an instrument, not a record, so it resolves the ACTIVE program year and cannot address a past season. The menu hosting it is write-gated and therefore already absent in an archive.
...
6. **`'staff'`/`'equipment'` (mig 266) are the ODD ONES OUT — `merge_rep_team_tags` does NOT re-point them.** Every kind above links through a real table (join row or FK column), which the function updates. Staff/equipment picks live as ids **inside `rep_team_events.practice_plan` jsonb** — at up to five nested levels (plan-level equipment; each block's staff and, while it has no stations, its equipment; each station's staff and equipment) — which no SQL function walks. `lib/coach-tag-routes.ts`'s `STAFF_TAG_LIBRARY`/`EQUIPMENT_TAG_LIBRARY` carry `afterMerge`/`afterDelete` hooks that run `lib/rep-practice-plan-tag-repoint.ts` in the SAME request, walking every one of the team's practice events and rewriting the jsonb by hand. **A THIRD kind added to this pattern needs the same hook, or a merge silently strands ids inside old plans.** This deliberately reopens a call `COACH_PRACTICE_PLANS_PLAN.md` §10.3 made the other way (plain names, specifically to avoid this problem) — see mig 266's header and `COACH_PRACTICE_STAFF_EQUIPMENT_TAGS_PLAN.md`.
7. **A `'staff'` tag can BE a person (mig 303, `user_id`) — a label still, never a grant.** `COACH_PRACTICE_WHO_RUNS_IT_PLAN.md` (owner rulings A–K, 2026-09-17): "that's you" on the field and the "You're on …" line on the plan used to be a NAME match against `organization_members.display_name` (nullable), which failed silently for "Craig" vs "Craig Whitfield" and always on a blank display name. The link is the identity the word always needed: the practice-plan GET resolves the reader's blocks/stations by tag id (`levelsForStaffTags`), the picker offers the season's staff first (`getPracticeStaffPeople` — the memberships, not the season row), and "Send to staff"'s *Named in this plan* audience matches on it. **Team tags and the staff kind only** (the CHECK), **one person per tag per team** (partial unique on `(team_id, user_id)` — a club coach on two rosters is linked once per team), `ON DELETE SET NULL`. Linking gives the person NOTHING — every read still gates on their own capabilities (D12's rule, kept whole). **Merge carries the person** (`carryStaffPersonOnMerge` in `lib/rep-practice-plan-tag-repoint.ts`, inside the before-destructive slot): a linked loser folded into an unlinked winner keeps its identity; two DIFFERENT people are refused by sentence before anything is destroyed. `merge_rep_team_tags` itself never learns the column.
...
**`kind`** (text, NOT NULL; CHECK `game|expense|focus|staff|equipment`, widened by mig 221 and mig 266) — `'game'` tags apply to `rep_team_events` (schedule); `'expense'` tags are the MONEY vocabulary and reach both directions of the ledger: `rep_team_expenses` via `rep_team_expense_tags` (mig 184) and `rep_fundraisers` via `rep_team_fundraiser_tags` (mig 239); `'focus'` (mig 221) is the ONE shared vocabulary behind drills, plan templates, practice plans (`rep_team_event_tags`) and players' focus areas; `'staff'`/`'equipment'` (mig 266) back the practice plan's staff and kit pickers — see gotcha 6, they are NOT relationally linked like the others. ⚠ There is deliberately no `'fundraiser'` kind — a label a coach spends against must be the same label they can raise against, or the money-tag report has two vocabularies that look like one.
...
<!-- dict:col:rep_team_tags.user_id -->
**`user_id`** (FK → `auth.users.id` ON DELETE SET NULL, nullable; **mig 303**) — the portal user this `'staff'` word IS (gotcha 7). CHECK `rep_team_tags_user_is_staff_team_tag`: NULL unless `kind = 'staff'` AND `team_id IS NOT NULL`. Partial unique `rep_team_tags_team_user_uniq (team_id, user_id) WHERE user_id IS NOT NULL` — one person per tag per team. Written only by the staff library's PATCH/POST (`lib/coach-tag-routes.ts`, which proves the person is on THIS team's staff this season before writing — the FK alone would hold a stranger) and by the merge carry. NULL = a word that is nobody ("a name only"): an outside instructor, a PD coach, or a word written before the link existed.
...
**Purpose:** a coach's own per-team award library (MVP, Best Hitter, Hustle Award to start, fully editable) that `rep_player_awards` records point at. Added by migration 182 (Coach Tags & Player Awards Phase 2); delete, merge and the DELETE RLS policy added by migration 289 (Awards Join the One Tag Idiom Part B). **⚠ mig 289 DEV-ONLY / PROD-PENDING at author time — see `rep_player_awards`' gotcha 7 for the same flag on its two unique indexes.**

**Gotchas (read first):**
1. **Delete is now app-side, but only ever on an UNUSED type.** `DELETE /award-types/[typeId]` (mig 289) attempts a hard delete; the RESTRICT FK on `rep_player_awards.award_type_id` (gotcha 2 below) turns a used type's delete attempt into a 23503, which the route maps to a 409 stating the given-count. A used type's real removal path is **retire** (unchanged — a plain `is_active=false` UPDATE, "Restore" the inverse) or **merge** (gotcha 2a). Every past `rep_player_awards` row keeps resolving a retired-or-renamed type's current `name`/`emoji` at render time, so neither retiring nor renaming ever rewrites history.
2. **Merge exists now** (mig 289, replacing the "no merge tool" posture mig 182 shipped with) — `merge_rep_team_award_types(winner, loser, team)` re-points every `rep_player_awards` row from the loser type to the winner, then hard-deletes the loser type. It **collapses R5 collisions first** (see `rep_player_awards` gotcha 7): a loser award whose (player, occasion) already holds a winner award would double the player up the instant it's re-pointed, so the LATER-created of each colliding pair is dropped instead (the earlier one is the record the coach actually made). Modelled on `merge_rep_team_tags` (mig 221), which can lean on `ON CONFLICT DO NOTHING` because its link table has a bare PK; awards are rows with their own notes and creation times, so this collapse is explicit rather than a conflict clause. Both ids must belong to the caller's own team — a shared, org-authored type is never merged from a team route.
...
4. **`team_id` is NULLABLE — NULL = org-authored SHARED award type** (widened in mig 184, Phase 3). A shared type surfaces in every team's give-award picker alongside the team's own; `getRepTeamAwardTypeLibrary(team, org)` returns team types + org-shared active types. Shared types are managed only from the admin Shared Library screen; `rep_player_awards` from any team may reference one. A shared type is scoped OUT of every team-route delete/merge attempt (404 — never this team's to remove).
5. **Seeded ONCE, at team creation — not on read** (mig-289-era build; the prior read-time seed, `ensureRepTeamAwardTypesSeeded`, is REMOVED from `lib/db.ts`). `createRepTeam` inserts MVP 🏆 / Best Hitter 💪 / Hustle Award 🔥 as part of creating the team. The read-time version was a live trap the moment delete became real: a coach who removed all three unused starters got them back on the very next read. A team created before this change was seeded on its first read under the old code and is unaffected — the seed only ever runs once either way.
6. **Coach-managed (team types) + admin-managed (shared types), via service role** (`supabaseAdmin`). Coach gate: `canManageAwards` (⚠ **named, not inlined** — A1 2026-08-03 narrowed it from `schedule || roster !== 'off'` to `hasRecordAccess`, closing a hole where a Helper's `schedule: true` admitted a parent volunteer to the awards API; restating the boolean here is what let this line go stale, so don't); shared-type gate: owner/admin + `module_rep_teams`. RLS write policies (coaches on assigned teams + org admins, `WITH CHECK`) are a defense-in-depth backstop; the org-admin policies already cover the team_id-NULL rows. The mig-289 DELETE policy follows the same coach/org-admin shape — the RESTRICT FK stays as the actual backstop, since RLS alone would still let a coach attempt (and fail) to delete a used type.
...
**Purpose:** the award record itself — one row per "MVP to Ethan Brar for the Jul 6 game." Added by migration 182 alongside `rep_team_award_types`; the two once-per-occasion unique indexes added by migration 289 (Awards Join the One Tag Idiom Part B). **⚠ DEV-ONLY / PROD-PENDING at author time.**
...
2. **`award_type_id` is `ON DELETE RESTRICT`, not `CASCADE`** — deliberately different from `rep_team_event_tags`' cascade-on-delete-the-parent-tag. This row IS the historical record (not a disposable link); a type can now be hard-deleted app-side (`rep_team_award_types` gotcha 1), but only once it is UNUSED — `RESTRICT` is what makes that a hard invariant rather than a route-level promise.
...
5. **Hard-deletable itself.** The `/awards/[awardId]` DELETE route removes a single mis-given record outright (undo a mis-click) — this is a different concern from deleting a *type* (gotcha 2), which only ever succeeds while unused.
...
7. **A player holds a given award ONCE PER OCCASION (R5, owner ruling 2026-09-11; mig 289).** Two partial unique indexes enforce it at the DB level: `rep_player_awards_once_per_game_uniq (player_id, award_type_id, event_id) WHERE event_id IS NOT NULL` and `rep_player_awards_once_per_general_occasion_uniq (player_id, award_type_id, awarded_at, coalesce(tournament_label, '')) WHERE event_id IS NULL`. "Occasion" = the game for a game-linked award, or the date + free-text label for a general one — the exact definition lives as a pure predicate in [lib/rep-award-occasion.ts](../../../lib/rep-award-occasion.ts) (`sameAwardOccasion`), which the give/edit routes' app-level check (`findRepPlayerAwardCollision`) and the merge preview (`previewMergeRepTeamAwardTypes` in lib/db.ts) both mirror — a DB constraint and a DB query can't literally call the same JS function, so the three homes are kept in sync by hand. **The indexes could not be created over an existing double**, so mig 289 dedupes first (keeps the earliest-created award per collision group, drops the rest, carries a dropped award's note onto the survivor only when the survivor's own is NULL) — verified ZERO doubles on both dev and prod before the migration was written (2026-09-11); the dedupe step ships anyway as cheap, idempotent insurance. Same rule is what `merge_rep_team_award_types` (gotcha 2 on `rep_team_award_types`) collapses before it re-points anything.
...
3. **`unit` lives on the type AND is snapshotted onto each entry** at log time (see `rep_player_measurables.unit`), and **the unit is FIXED once a result exists** (owner ruling 2026-09-15, mig 300; held by the route — `applyDefinitionPatch` refuses a unit change on a test with readings, 400). A coach who wants a new unit retires the test and defines a new one; nothing links the two. So every entry under one test shares its unit, and a line through them is arithmetic in that unit.
...
8. **THE SUCCESSOR RULE IS GONE (mig 300, owner ruling 2026-09-15).** Migs 293/294 had a retired definition point at the one that "replaced" it (`replaced_by_id`) and a SECURITY DEFINER function (`replace_rep_team_measurable_type`) that retired a test, inserted a successor under its name and linked the two on a unit change — so one name carried two units and the Results table had to explain the join. Mig 300 drops the function, the `replaced_is_retired` CHECK, the link's partial index and the column. What replaced it is gotcha 3 (the unit is fixed) and gotcha 10 (delete). ⚠ The dropped function is invisible to the drift gates (pg_proc only) and a dropped column is invisible in the "prod is missing" direction; `MANUAL_PROD_STEPS.json` carries 300, ORDER-CRITICAL with the promote.
...
10. **A definition nothing points at can be DELETED; one with records is retired, never deleted** (owner, 2026-09-15). `DELETE /measurable-types/[typeId]` checks the three referencing tables (`rep_player_measurables`, `rep_player_observations`, `rep_evaluation_not_assessed` — all `ON DELETE RESTRICT`, so a record landing between the check and the delete surfaces as 23503 → 409). The id is stripped from any session's `scope_metric_ids` / `scope_attempts` FIRST (a snapshot of intent, not a FK — a plan that kept naming a test that no longer exists would count it "unrecorded" for ever), each write a compare-and-swap on the list as read (two deletes touching one plan in the same instant would otherwise put each other's test back — /review 2026-09-15), and only then is the row deleted. The sheet offers Delete only when the editor's read says `hasRecords: false`. Mig 301 adds the matching DELETE RLS policy on the development-writer predicate (the policy set said nobody may; the routes are the gate, the policy stops lying).
...
**`unit`** (text, **nullable since mig 293**; CHECK `1 ≤ char_length(btrim(unit)) ≤ 20` when present; `unit_by_kind` CHECK: NOT NULL on a test, NULL on a skill) — free-text unit ("seconds", "mph"); snapshotted onto entries and FIXED once a result exists (gotcha 3). ⚠ App code narrows with `isMeasuredTest()` before reading it as a string.
...
**`method`** (text, nullable; CHECK ≤ 600; mig 293) — how the test is run, in the coach's words, so results are comparable. NULL = not recorded (every legacy row; the product says so). The coach's optional note: writing, changing or erasing it is an ordinary edit (owner, 2026-09-14).

<!-- dict:col:rep_team_measurable_types.attempts_per_session -->
**`attempts_per_session`** (smallint, NOT NULL, default 1; CHECK 1–5; mig 293) — **no longer the definition's fact (re-evaluation stage 2, C1, 2026-09-15).** The count is planned on the SESSION, per test (`rep_team_evaluation_sessions.scope_attempts`); the definition keeps the HEADLINE (how a result is read). The sheet stops writing this column (a new test writes 1); it is read only as the seed for a session's pre-fill when no session has ever run the test (last session's count → else this → else 1). Do not build on it.
...
7. **Live-season only — no archive door** (owner ruling 2026-08-01). A drill library is a reusable *instrument*, not a record of a season. Its routes deliberately do **not** use `resolveCoachSeasonRead`, the Development hub hides the Drills door in a completed season, and `tests/unit/coach-history-endpoint-guard.test.ts` asserts the absence so it can't be reversed by accident. The one cross-season read is `development/drills/past-seasons`, which reads the team's own **past practice plans** (genuinely season-locked) so they can be copied forward — it writes nothing into a finished season.
...
7. **Live-season only, no archive door** — a template is an *instrument*, like the drill library. Its routes must not serve a season the caller names.
...
**`session_id`** (FK → `rep_team_evaluation_sessions.id` ON DELETE SET NULL, nullable; mig 190) — the evaluation session this reading was collected in (gotcha 7); NULL = recorded from the player's record, outside a session (the bench-side sheet — re-evaluation stage 3 E7, 2026-09-15 — records up to five attempts on one date, and the reader groups them by that day into ONE result, as it groups a session's).

<!-- dict:col:rep_player_measurables.attempt_no -->
**`attempt_no`** (smallint, NOT NULL, default 1; CHECK `1–5`; mig 295) — which attempt within its result: inside a session, the session's (its `scope_attempts` says how many were planned for the test — a floor, never a ceiling: the row may hold up to five, C2); outside one, the DAY's (re-evaluation stage 3 E7, 2026-09-15 — the bench-side sheet writes attempts 1..N on one `recorded_on` with `session_id` NULL, and `groupBySession` keys them on that date; the app refused attempt > 1 without a session until then). **Every row written before 2026-09-13 is attempt 1**, which is the truth: one reading was taken. Part of the per-attempt partial unique inside a session (gotcha 7); outside one the day's attempts are not constrained by the database — the measurables POST hands a bench-side attempt the next free number for that day (five the ceiling) so a day never holds two of one number by the app's own hand, and the sheet reads a day's attempts by their ORDER (a legacy day with two "attempt 1" singles is two boxes), never by the number. Two writes inside the same second can still twin; a partial unique index on `(player_id, measurable_type_id, recorded_on, attempt_no) WHERE session_id IS NULL` would close it.
...
6. **The count is planned on the session, per test (mig 298, 2026-09-15 — re-evaluation stage 2, C1/C2).** `scope_attempts` is a jsonb map `{"<metric uuid>": 1..5}` — how many attempts the coach planned for each test in THIS session, one more fact of the scope snapshot (Finding #41 item 3's reasoning: read only by its own session, never queried by member, so a map beside the id lists rather than a join table). Keys ⊆ `scope_metric_ids` and values 1..5 are the route's proof (a CHECK over jsonb values needs a function); the database holds only "an object" and "no count without a scope". NULL on every session created before it = no count claimed: the grid shows as many boxes as a row holds and the review never says "fewer than planned". **A floor, never a ceiling:** a row may hold MORE attempts than planned (the "+" on the row, up to five — the reader's cap), and lowering a count never hides a saved attempt. A skill has no count (an observation is one thing). The pre-fill at "Start session" reads the newest session's count for that test, else the definition's stale `attempts_per_session`, else 1.
...
- **A linked session's date IS the practice's date (re-evaluation stage 2, C10, 2026-09-15 — REVERSES Practice Plans §10.2 ruling 1 on its reason).** "When?" is one question: *at a practice* (the session takes the practice's day — the session PATCH and the create both DERIVE `session_date` from the event and refuse a date that disagrees with a link) or *on a date* (no event; the coach types it). The old rule's worry — a rescheduled practice dragging results to a day they did not happen — cannot arise: a session is created AT the practice, so a practice whose DAY changes after results were taken is a date correction, and the results follow. The events PATCH answers 409 with the linked sessions (id, note, attempt count) unless the body carries `moveSessions: true`; with it, each linked session is re-stamped and moved (sessions first, then the event, so a failure leaves a state the next save repairs). A deleted practice leaves its session on the date it had (`SET NULL`, gotcha 1) — it reads "on a date". "Tested at 5:35 before the 6:00 practice" is the same day and stays expressible; a different day is "on a date".
...
**`scope_metric_ids`** / **`scope_player_ids`** (uuid[], nullable; mig 295) — the session's scope (gotcha 5): the definitions (tests AND skills) and the roster rows it was for. Whole-or-neither. NULL = no scope (every pre-2026-09-13 session).

<!-- dict:col:rep_team_evaluation_sessions.scope_attempts -->
**`scope_attempts`** (jsonb, nullable; mig 298) — `{"<metric uuid>": 1..5}`, the attempts PLANNED per test in this session (gotcha 6). CHECK `rep_team_evaluation_sessions_scope_attempts_object_check` (an object or NULL) and `…_scope_attempts_needs_scope_check` (NULL whenever `scope_metric_ids` is NULL). NULL = no count claimed (every pre-2026-09-15 session). Keys and values are proved by the route, never the database. A floor, never a ceiling.
...
**Purpose:** Dated review events on a goal (plan §7 Goal review; F08 — "a goal is a current state, not a history"): the status chosen (required — F19), an optional note, the next review date and evidence references. The goal's `status` (and next-review date) is always whichever review is currently the LATEST, written in the same step by the route. Added by migration 295 (development lifecycle Phase 2, 2026-09-13). **⚠ DEV-ONLY / PROD-PENDING at author time (order-critical after 293/294).**

**Gotchas (read first):**
1. **Editable since migration 299 (owner ruling 2026-09-16) — it was append-only, and that was a POLICY fact, not a table fact.** RLS originally carried 2 SELECT + INSERT and NOTHING else; mig 299 (policy-only, no schema change) added UPDATE + DELETE on the same development-writer predicate as every other write policy here, because this is a coach's own working record, not an audit log — a mistake should be correctable, not merely answerable by a second entry. An edit or a delete re-runs the "status is the latest review's" step (goals gotcha 5) against whatever review is now newest; deleting every review resets the goal to its pre-review default (`working`, no next review). Deleting the GOAL still cascades its reviews (the goal is the record; a review without its goal is meaningless).
...
7. **Writes follow the Development grant at BOTH layers** (mig 292, 2026-09-12; head-coach-only under D1 until then — confirming a returning player is a development write in the app, so the policy moved with the four development tables). RLS requires `coach_role='head_coach' OR (capabilities->>'development')='true'`, no org-admin writes, DROP-guarded → re-runnable. Matching (guardian-email + name similarity + exact DOB) is app-side — this table stores only outcomes.
...
9. **Carry-forward was a ONE-TIME decision recorded on the LINK row (mig 192) — and since 2026-09-16 the product neither reads nor writes it.** `'carried'` = the prior season's *working* goals were COPIED into new `rep_player_development_goals` rows on the current player (copies, not moves — the archive keeps the originals; those rows carry `origin='carried'` and stay valid); `'fresh'` = declined; NULL = never answered. The per-player "bring forward last season's goals?" offer, its route and its writer left the player's page on the owner's ruling that linking and what follows it are decided for the TEAM, never one profile at a time (plan §20.1 E9). The columns stay: existing rows are history, the UAT seed still stamps `'fresh'`, and the team-level surface that replaces the offer decides whether a bulk carry reuses them. Measurables are NEVER carried across seasons (fake trend data).
...
**`carry_status`** (text, nullable; CHECK `carried|fresh`; **mig 192**) / **`carry_decided_by`** (FK → `auth.users.id` SET NULL) / **`carry_decided_at`** (timestamptz) — the one-time rollover carry-forward answer + its audit (gotcha 9). ⚠ Dormant since 2026-09-16: the product no longer reads or writes these (the offer left the player's page); rows written before then are history.
...
7. **Live season only.** The route is built on `resolveLiveCoachTeamContext`, never a resolver that admits a season it was handed: seeding a season's development plan is an *instrument*, not a record, so a finished season cannot be addressed at all (a year parameter is a decision — CLAUDE.md, 2026-08-16).
...
**`category_id`** (FK → `budget_categories.id`, nullable) / **`item_id`** (FK → `budget_items.id`, nullable) — categorize the line against the **org Accounting chart of accounts** (cross-domain; shared with `org_budget_lines`). Both null = uncategorized. `category_id` feeds the display name and the (name-based) actual-matching. ⚠⚠ **`item_id` IS UNIQUE PER PLAN SINCE MIGRATION 286 — one word, one line** (owner ruling 2026-09-09): partial unique index `rep_budget_lines_one_line_per_item` on `(program_year_id, team_id, item_id) where item_id is not null`. A word may be used once on a season plan; picking one that is already there **adds to** the line it has (the form PATCHes it, joining the amounts and concatenating the two payment schedules — `joinPeriodSplits` in `lib/coach-budget-periods-payload.ts`). All three doors that create lines refuse a duplicate: the form (409, naming the line in the way), the spreadsheet import (skips the row and says which word) and "bring last season's plan" (only ever copies into an empty plan). **Why:** real spending is matched to the plan by ITEM, never by line, so two lines on one word were one thing wearing two labels — no variance, match or total could tell them apart, the by-period grid never showed the level at all, and a note-less second line ended up named after its own month. Migration 286 joined what existed (dev: 5 groups; prod: 1) — **re-pointing `rep_budget_periods` (ON DELETE CASCADE) and `rep_player_dues_schedules` (ON DELETE SET NULL) at the keeper BEFORE deleting**, since a plain delete would have taken the schedule with it and silently unlinked a dues schedule. ⚠ **PARTIAL on purpose:** pre-mig-243 lines carry no word, cannot be joined on one, and keep reporting in the "Not itemized" bucket — the only item row on any surface that can still hold several lines. ⚠ One item is one direction (`line_kind` is derived from the word, mig 280), so the index needs no direction column.
...
**`line_kind`** (text, NOT NULL, default `'cost'`, CHECK `in ('cost','funding','sponsorship','other_income')`) — `cost` = money the team spends (what every row written before migration 230 is); `funding` = expected **fundraising** (the team selling something); `sponsorship` = expected **sponsorship or a grant** (money given directly, **migration 237**); `other_income` = money in that is **neither** — interest, a facility rebate, a plain donation (**migration 274**, owner ruling Q5 2026-09-02). No money-in kind counts toward what the season costs, only toward what players don't have to fund, and all are excluded from actual-expense matching (gotcha 2).

⚠ **The money-in kinds differ ONLY in reporting — the maths is identical.** All subtract from what the season costs, so per-player dues and the installment generator treat them the same; any code that sums "money in" must include every kind in `FUNDING_LINE_KINDS` or it silently under-counts. The 237 split exists because migration 230's `funding` was deliberately written to cover sponsors and grants, which meant sponsorship money sat in every plan invisibly and *"did our sponsorship hit the number?"* could not be answered (owner ruling 2026-08-15, knowingly reopening the 2026-08-13 naming ruling); 274's `other_income` exists because money in that was neither got mis-filed into one of them or left off the plan. ⚠ Actuals for `other_income` are **typed by the coach** (`LINE_KIND_ACTUAL_SOURCE`) — no derived machinery answers for it the way fundraisers and sponsors do.

<!-- dict:col:rep_budget_lines.split_mode -->
**`split_mode`** (text, nullable, CHECK `in ('months','quarters','dates','names')`, **migration 274**) — HOW the period split was entered, so the editor reopens in the mode the coach built it in (owner ruling Q2 2026-09-02). NULL = written before 274 (or no split): readers fall back to `inferSplitMode`, the reopen-guess this column retires. Written by the budget-line editor on every save; meaningless when the line has no `rep_budget_periods` rows; **never read by arithmetic** — the period dates stay the only thing the reports consume. Carried by the season-rollover budget copy and the "bring last season's plan" door.

<!-- dict:col:rep_budget_lines.notes -->
**`notes`** (text, nullable) — free text. Since 2026-09-02 (P1) this is also where the add-form's *"What makes this line different?"* answer lands when a coach writes a **second** line against one item — the only per-line words the screen and the export can tell same-item twins apart by.
...
2. **`credit_type` CHECK `contribution|fundraiser|overpayment|other|forgiven|reimbursement`** (default `contribution`; 6-value since mig 233). `fundraiser` is system-generated by the fundraiser flow; `overpayment` by the payment reconciler (gotcha 6); `reimbursement` by an out-of-pocket expense (Pass 2); `forgiven` is **debt relief, not the family's money** — it lowers bills like any credit (first, before the family's own credits, in every application mode) but is never owed back and can never be paid out.
...
6. **`payment_id` (mig 232) marks an AUTO-CREATED overpayment credit** — set only when recording a dues payment larger than what was left on the schedule (owner ruling 2026-08-13: automatic, no prompt). **ON DELETE CASCADE from `rep_dues_payments`:** removing the payment removes the credit with it. Manual credits: NULL. ⚠⚠ **IT POINTS AT `rep_dues_payments`, NOT `rep_payable_payments`** — a *dues* receipt, money coming IN. P4 (mig 267) deliberately did **not** overload it for the money-OUT side: a payment-level reimbursement is keyed by `(expense_id, player_id)` instead (gotcha 7).
7. ⚠⚠ **A COST'S REIMBURSEMENT CREDITS ARE A SET, ONE PER HOUSEHOLD (mig 267) — `.maybeSingle()` on `(expense_id, credit_type)` IS NOW WRONG.** Before P4 exactly one family could front a cost, so one credit per expense held the sum of every payment on it. A commitment can now carry payments from more than one household (a parent pays the $200 deposit; the team pays the balance; a second parent pays a later piece), so the credit set is keyed by **(expense, effective payer)** and each row holds **that household's** payment total. Enforced by partial UNIQUE `uniq_rep_dues_credits_reimbursement_per_household` on `(expense_id, player_id) WHERE credit_type='reimbursement' AND expense_id IS NOT NULL` — without it a racing retry could write a second row for one household and silently double a family's figure. `reconcileReimbursementCredits` (lib/db.ts) inserts/restates/deletes the whole set under a compare-and-swap on each amount.
8. ⚠⚠ **A REIMBURSEMENT CREDIT CAN BE PAID OUT IN CASH, AND SHRINKING ONE BELOW WHAT WENT OUT FAILS SILENTLY.** `payoutCeiling` excludes only `forgiven`, so these are payable out like any other credit. Lower one below `Σ rep_dues_payouts` for that household and the arithmetic does **not** go negative — `applyCreditsToBills` clamps with `Math.min(paidOut, issued)`, so `owedBack` and the ceiling both floor at zero and the team is simply out the money with nothing on any screen saying so. Every write path that can lower a credit asks `assertReimbursementFloor` **before its first write** (undoing a payment, restating a schedule) and refuses with a sentence naming the payout; the check inside the reconciler is the backstop. ⚠ The refusal must come **before** the payment row is deleted — refusing afterwards leaves a household credited for money nobody paid.
...
**`credit_type`** (text, NOT NULL, default `contribution`; CHECK 6-value since mig 233 — gotcha 2).
...
4. **`amount` exists although the ruling is whole-debts-only** (owner agreed 2026-09-07), and it means **the whole of what REMAINS**. ⚠ **A row here can legitimately carry LESS than its credit's `rep_dues_credits.amount`, and since 2026-09-09 it does** — the sentence this gotcha used to carry ("the form never writes a partial") was true only by accident. A coach still cannot pay back half a debt: the standing figure is taken in full or the credit is not ticked at all. But where a **pre-281 payback** already consumed part of a credit, the whole of what remains IS less than the issued amount. No such row could exist before that date because the save valued a legacy-touched credit at its full issued amount and its own ceiling then refused it — every time, for every family with any payback history (the defect fixed in `COACH_MONEY_CREDITS_AND_PAYBACKS_PLAN.md` §6f). Recording the amount still makes a future *chosen* partial a screen change rather than a second migration. ⚠ **Migration 281's own column COMMENT still says "the column exists so a partial is a screen change" and its header says "The form will never write a partial"** — an applied migration is history and is not edited; the current meaning is the one stated here.
...
<!-- dict:col:rep_fundraisers.budget_item_id -->
<!-- dict:col:rep_fundraisers.budget_category_id -->
**`budget_item_id`** (FK → `budget_items.id`, **nullable**, ON DELETE SET NULL; index `idx_rep_fundraisers_budget_item`) / **`budget_category_id`** (FK → `budget_categories.id`, nullable, ON DELETE SET NULL) — **mig 285**: the budget line this record is **raising for**, and the shelf that word sits on. ⚠⚠ **THIS IS WHY BUDGET vs. ACTUAL CAN PLACE A DRIVE AT ALL.** Until 285 nothing linked a drive to a line, so `placeDerivedActual` placed the whole per-source **pool** by what the plan happened to claim: one fundraising line and the money landed there; **two and BOTH rows went blank** while the money collected under "Not itemized" — a team punished for planning carefully. Each record now lands its own realised total on its own row. ⚠ **The picker is filtered by `budget_categories.income_source` and the server re-checks the match**: a `kind='fundraiser'` record may point only at a word on a `fundraiser` shelf, a `kind='sponsor'` record only at a `sponsor` one. Pre-filled with the shelf's standard word (*Fundraising drive* / *Team sponsorship*); **neither required nor nudged** (owner, 2026-09-08). ⚠ **NULL is a real state, not an oversight.** Mig 285's backfill linked a record only where its season's plan was **unambiguous** — every money-in line of that kind pointing at ONE item — precisely so today's report was preserved rather than improved: blanket-defaulting a team that budgeted only *Merchandise sales* to *Fundraising drive* would have jumped their money to a row they never planned. A NULL record still places by the legacy pool rule and shows one quiet nudge in its own room, which is the only place a coach meets the legacy state. ⚠ **The category is stored ALONGSIDE the item and re-derived FROM it on every write** (mig 282 part 2's rule): the report reads the two levels in different orders, so a record keeping one heading while its word lives under another reports under two headings depending which half of the page is asking. ⚠ **SET NULL does NOT re-point the drive** — deleting the line leaves the record raising for nothing, and its money then reads as income not in the plan, which is honest and deliberate (plan §4). ⚠ **`rep_fundraisers` is in `BUDGET_ITEM_REFERENCES`** (`lib/coach-budget-item-usage.ts`) — the list the fold, the publish route and the remove guard walk, and which `tests/unit/budget-item-references-guard.test.ts` fails the build over.
...
**Gotcha 7 — THREE ways a cost reaches the budget coexist, and they roll up to the same rows.** Since mig 240 a row may carry `budget_item_id` (the coach said what the cost IS), or `budget_category_id` alone, or only free-text `category` (every older row, and every import that matched nothing). Budget vs. Actual resolves each cost in strict order of trust — **the item, then the category id, then the text category matched by NAME** — so all three land under the same headings, which is what makes shipping without a backfill safe. A cost resolving to none of them gets its own honest **"No category"** row rather than a separate list nobody scrolls to. ⚠ **A word carries ONE budget line since mig 286** (owner ruling 2026-09-09), so the 2026-08-15 rule that two lines on one item SUM into a single report row now has nothing left to sum on the plan side — it survives only for the word-LESS "Not itemized" bucket, and the roll-up still owns it. **Whether a cost was BUDGETED is never stored** — it is derived, by asking whether a line exists for the same category + item, which is why the old "Not in the budget" declaration could be deleted outright. The rule lives in `lib/coach-budget-rollup.ts`, which owns it for the route and the screen together.
...
7. ⚠⚠ **A PAYMENT CARRIES ITS OWN PAYER SINCE MIGRATION 267, AND ASKING THE *COST* WHO PAID IS NOW A BUG.** The EFFECTIVE payer of a payment is `rep_payable_payments.paid_by_player_id ?? rep_team_expenses.paid_by_player_id` — `effectivePayerId()` / `paymentMovedTeamCash()` in `lib/payable-standing.ts`, and every reader must go through them. Before P4 a cost was fronted or it was not, so `!expense.paid_by_player_id` was a complete answer; a $600 entry can now have its $200 deposit paid direct by a parent and its $400 balance paid by the team, and at cost level that bill counts **$600 of team cash or $0 — both wrong by real money**. Three readers were corrected in the same change and each is load-bearing: the register's `movesCash` (its running balance IS cash on hand), `expenseTotals().cashPaid` in `lib/season-settlement.ts` (**this figure sets every family's end-of-season refund**), and Budget vs. Actual's `familyPaidDirect` cash strip. ⚠ A **scheduled** row still reads the cost, correctly — no payment exists yet to carry an answer.
8. ⚠ **A fronted payment posts NO ledger entry, so the two columns can never both be set.** `paid_by_player_id` set ⇒ `accounting_entry_id` null (no team cash moved — gotcha 3's first meaning). The writer sets each branch's column explicitly rather than letting one ride a spread.
...
<!-- dict:col:rep_payable_payments.paid_by_player_id -->
**`paid_by_player_id`** (FK → `rep_roster_players.id`, nullable, **ON DELETE SET NULL**; **mig 267**; partial index `idx_rep_payable_payments_paid_by` on `(expense_id) WHERE paid_by_player_id IS NOT NULL`) — the family who paid **this payment** directly (gotchas 7–8). NULL = the team paid it, which is the ordinary case. Set means no team cash moved and the team owes that household a `reimbursement` credit. ⚠ **Deliberately NOT set when the cost already names a fronting family** — that column already claims every payment against it, and storing the same fact twice is how the two start disagreeing; `createOutOfPocketExpense` leaves this null. ⚠ SET NULL matches the cost-level column: losing a provenance pointer must never delete the record of money that moved. The real protection is `rep_dues_credits.player_id` being NOT NULL plus the roster undo-guard, which refuses to remove a player carrying credits.
...
9. **⚠⚠ AND THE CODE ANSWERED THE ONE QUESTION THAT BELONGS TO THE COACH, UNTIL MIGRATION 271 (2026-08-30).** Once mig 250 let club money file itself, Budget vs. Actual read **every** approved `charge_to_org` as a *reimbursement* — netting into the cost it repaid. Right for a repayment; wrong in the mirror image for a **grant**, which then vanished into a cost line it was never about, leaving that line claiming spending the club had actually carried and the club's contribution visible nowhere. Both readings net out at the SEASON level, so no total-based guard could ever have caught it — the harm is entirely in the lines a coach plans next season from. Nothing on the record distinguishes the two (same direction, amount, club, approval), so `money_in_meaning` is **asked**, never derived.
...
<!-- dict:col:rep_team_payment_requests.money_in_meaning -->
**`money_in_meaning`** (text, nullable; CHECK `funding|reimbursement`, **and** CHECK `NULL OR request_type = 'charge_to_org'`; **mig 271**) — what money arriving FROM the club **means**, in the coach's reading (gotcha 9). `funding` = new money for the season (a grant, or a cost the club agreed to carry) → its own **revenue** row on Budget vs. Actual, filed against the money-**IN** side of the item library. `reimbursement` = the club paying the team back → **nets** into the cost it repaid, filed against the money-**OUT** side, never income. ⚠⚠ **NULL means LEGACY, not "unanswered"** — every request raised before mig 271 keeps the reimbursement reading it has reported under since it was approved (no backfill, by standing rule), and **every reader must treat NULL as `reimbursement`**; `clubRequestReportSide` ([lib/coach-club-money.ts](../../../lib/coach-club-money.ts)) is the one place that decides it. The write paths **require** it on every new `charge_to_org` and force it to NULL on `payment_to_org`, including when a coach flips a pending request's direction. ⚠ It changes **one line of one report**: cash on hand, the register, the season close-out pot and every family's dues read both answers identically — a dollar arrived either way — and neither answer ever touches a payment schedule.
...
5. **Coach-side family management is LIVE-SEASON-ONLY.** The coach routes deliberately do NOT
   read a season parameter, so they resolve the active season and cannot address a past
...
**Purpose:** the top level of a **shared two-level chart of accounts** (categories → items) used by BOTH the org budget planner and the rep-team budget planner. `org_id IS NULL` rows are platform-seeded read-only defaults; `org_id`-set rows are org customs — **and since mig 277 an org custom is either CLUB-SHARED (`team_id` NULL) or one team's own (`team_id` set)**.
...
2. **`scope` (`org|team|both`, default `both`) gates which planner shows the category** — the `org`/`team` filters BOTH include `both`; `'both'` is universal. ⚠ **`scope` is a DIFFERENT AXIS from `team_id`** and the two compose: `scope` says *which planner offers it*, `team_id` says *whose it is*. Reading either as the other is the trap — a `scope='team'` category is not "a team's category", it is one the team planner shows.
3. **A category is now RENAMABLE, by exactly one party each (mig 277), and still deletable by nobody.** `PATCH /api/admin/accounting/budget-categories/[catId]` renames a club-shared one (owner/treasurer, refuses platform rows and refuses a team's own); `PATCH /api/coaches/[orgSlug]/budget-categories/[catId]` renames a team's own (that team's money-write coaches, refuses everything else). **No DELETE exists on either** — deliberate, not incomplete CRUD: `budget_items.category_id` is `ON DELETE CASCADE`, so dropping a category would take its items with it and blank the classification on every record filed against those items at once. `org_budget_lines.category_id` + `rep_budget_lines.category_id` are `ON DELETE SET NULL` (a deleted category — were it possible — silently uncategorizes lines). Renaming reaches every one of those records and loses none, which is why it is the offered remedy.
4. **The three tiers are read through the SHARED predicates, never a local `!team_id`** — `budgetCategoryTier` / `categoryVisibleToTeam` / `categoryOfferedToClub` in [lib/coach-budget-item-tiers.ts](../../../lib/coach-budget-item-tiers.ts), which are *aliases of the item predicates*, not copies: the two levels carry the same two columns and must never drift. Every category-listing read applies one of them — the coach picker + import (visible-to-team), the club's planning picker (`?forPlanning=1`, offered-to-club), and the club's category panel (unfiltered by design, gotcha 5).
5. **The club SEES every team's category and OWNS none of them** (owner ruling Q1, 2026-09-04). Two different questions, two different answers: *"what may the club plan under?"* → offered-to-club, enforced on the write paths (`resolveOrgBudgetCategory`) and by `?forPlanning=1`; *"what exists in this club at all?"* → the unfiltered list the category panel and cross-team reporting read. Answering the second with the first makes a team's heading vanish from the club's own report; answering the first with the second lets the club file its plan under a word it cannot rename.
...
<!-- dict:col:budget_categories.team_id -->
**`team_id`** (FK → `rep_teams.id`, nullable, **ON DELETE CASCADE**; mig 277) — **who owns this heading, and therefore whose picker it appears in.** Three tiers, read from this column and `org_id` together, identical to `budget_items.team_id` (mig 240): `org_id` NULL = **platform**, everyone sees it and nobody renames it; `org_id` set + `team_id` NULL = **club-shared**, every team plans under it and the club's own budget files against it, renamed by an Owner or Treasurer; both set = **that team's own**, offered in that team's picker ONLY, renamed by that team's money-write coaches, and **read but never edited** by the club (gotcha 5). ⚠ **This reverses mig 240's deliberate decision** to give items the tiers and leave categories org-wide ("clubs want them shared, and the report's top level would fragment if each team invented its own" — the comment it retired sat in the coach route). ⚠ **CASCADE, not SET NULL**, for the same reason items use it — a deleted team's private headings must not silently become the club's — but note the cascade is **TWO HOPS**: `budget_items.category_id` is itself CASCADE, so deleting a team drops that team's categories *and* the items filed under them; records pointing at those items are protected separately by their own `ON DELETE SET NULL`. ⚠ **No backfill** (owner ruling Q2, 2026-09-04): every pre-277 category stays NULL, i.e. club-shared, because nothing recorded who created it and guessing would hand a heading six teams plan under to one of them. Partial index `budget_categories_team_idx`.

<!-- dict:col:budget_categories.name -->
**`name`** (text, NOT NULL, ≤80 app-side) — category name. **No unique index** (only `(org_id)` + PK) — unlike `budget_items`/`org_payees`, duplicate category names are **silently allowed**; the POST route's `23505→409` branch is dead code for categories. ⚠ **Case-insensitive uniqueness is enforced in the app, and since mig 277 it is scoped to what the ASKER can see** — visible-to-team on the coach's create and rename, club-visible on the club's rename. Checking the whole org would refuse a coach a name on the strength of a private heading belonging to another team, which they cannot find, open or use; items hit that exact wall between migs 240 and 248. Deliberately **not** promoted to a real unique index: none has ever existed, so one would fail outright on any org already holding a duplicate.
...
<!-- dict:col:budget_categories.income_source -->
**`income_source`** (text, **NOT NULL**, default `'typed'`; CHECK `typed|fundraiser|sponsor`; added **mig 285**) — **who fills in the money-in words on this shelf.** `typed` = the coach records each arrival themselves under *Other money in*; `fundraiser` = a drive on the Fundraising tab reports it; `sponsor` = a sponsor does. ⚠⚠ **THIS IS WHAT A NEW WORD'S `budget_items.actual_source` IS DERIVED FROM** (owner ruling 2026-09-08, *"the category a word sits in decides who fills its number in and where it reports"*). Before it, every coach- and club-created word was born `'typed'`, and `'typed'` derives `other_income` — so a word filed on the **Fundraising** shelf reported under *Other income* while its own row read "Fundraising · …". The row said one thing and the heading said another, forever. ⚠ It also **filters the "Raising for" picker** — a fundraiser is offered only `fundraiser`-source words, a sponsor only `sponsor`-source ones — and the fundraisers write path re-checks the match, so a drive cannot raise for a sponsorship word by API either. ⚠ **Exactly two rows are non-`typed`, both PLATFORM** (`org_id IS NULL`): *Fundraising* → `fundraiser`, *Sponsorship* → `sponsor`, set once **by name inside mig 285** — the one place a shelf is ever looked up by name, because after it nothing in the product does. Every other shelf is `typed`, **including a club heading that happens to be called "Fundraising"**: that club's word has no drives behind it. ⚠ **No coach or club write path accepts it.** Only a migration moves a shelf, and moving one re-files every word on it the next time each is saved — the same rule `budget_items.actual_source` records, one level up.
...
6. **A word now decides a budget line's KIND (mig 280).** `direction` + `actual_source` together derive `rep_budget_lines.line_kind`, so editing either on a platform row re-files every future line filed against it. Neither is editable by a club or a coach (`direction` is refused outright by the coach PATCH; nothing accepts `actual_source` at all), which is what keeps that power ours. See `actual_source` below.
...
**`name`** (text, NOT NULL, ≤80 app-side) — **the name of a budget row** since mig 240, and the key both the plan and Budget vs. Actual group on. Case-insensitive uniqueness per category per scope: `budget_items_unique_default_name` for platform rows, and **`budget_items_unique_scope_side_name`** for org rows (mig 248, replacing mig 240's `budget_items_unique_scope_name`). ⚠ **What makes two words the same word is source + SIDE + category + name** (owner ruling 2026-08-17) — the side joined the key because mig 246 made `direction` mandatory and the picker filters by it, so a team can legitimately want *Grant* as income (a cheque that arrived) and *Grant* as an expense (the fee to apply for one); under the 240 key those collided and the create path answered with a 409 that had no good remedy. The picker offers one side at a time, so the pair can never be ambiguous at the point of choice. It still coalesces `team_id` to a zero uuid, because a plain multi-column unique index would let every club-published row escape it entirely (NULL never equals NULL), while two teams must still each be able to invent "Provincials entry". No backfill was needed: the new key is strictly weaker than the one it replaced.
...
**`direction`** (text, **NOT NULL** since mig 246; CHECK `in|out`; added mig 243) — **which side of the books this word belongs to**: `in` = money the team receives, `out` = money it spends. ⚠⚠ **THE PICKER FILTERS BY IT, AND MIG 246 REVERSED MIG 243 TO MAKE THAT POSSIBLE.** 243 shipped it nullable as *"a picker hint that sorts, never a constraint"*, NULL on everything a club or coach created. Owner ruling 2026-08-16 — *"the items coaches add need to be tied to income or expense, they cannot be unlinked or untagged to those. So a coach clicking income should not see expense items or vice versa"* — made it part of what an item **is**: 246 backfilled every untagged row to `out` (money-in items did not exist before 243, so the whole pre-existing library is a spending vocabulary — the same reasoning 243 §3d applied to platform rows) and set NOT NULL, so no create path can skip the question. All four writers pass it: the coach picker and the club-admin picker take it from the surface they were opened on, the budget-plan importer is always `out` (it refuses money-in lines at the door), and the demo coach seed tags its funding/income pairs. ⚠ **The report STILL never reads it** — a row's direction comes from what was actually filed against it (`lib/coach-budget-rollup.ts` rule 5), which is why one item can legitimately appear as two rows, one in each section of the statement, and why **moving a word to the other side re-files nothing and moves no money**. ⚠ **A refund chooses from the `out` list**, because what it repays is something the team spent — the money form's "This is a refund" tick flips the direction of the money and deliberately leaves the word list alone. ⚠ **The chosen item always survives the filter in the UI**: a saved record whose word has since moved sides must never lose its item, so the picker offers the current selection whichever way it points. ⚠ **Editable after the fact** — a team's own words only, via `PATCH /api/coaches/[orgSlug]/budget-items/[itemId]` behind Budget Plan → *Manage our items*; standard and club-published words are read-only to a coach (the one-way tier rule). ⚠ **Categories are deliberately NOT tagged**: *Tournaments* holding both its revenue and its costs is the entire premise of the by-activity report lens.

<!-- dict:col:budget_items.actual_source -->
**`actual_source`** (text, **NOT NULL**, default `'typed'`; CHECK `typed|fundraiser|sponsor`; added mig 280) — **who reports this word's actual figure**: `typed` = the coach records each arrival themselves, `fundraiser` = the team's drives report it, `sponsor` = the team's sponsors report it. ⚠⚠ **THIS IS WHAT A TEAM BUDGET LINE'S `line_kind` IS DERIVED FROM**, and it is why the add-a-line form asks ONE question instead of two. It used to ask *"This line is: Expense / Expected fundraising / Expected sponsorship / Expected other income"* **and then** *"Category & Item"* — and because the picker filters by `direction` and never by the kind just chosen, *Expected sponsorship* + *Tournaments → Concession revenue* was offerable. That pairing is a trap, not untidiness: the kind decides where the row's actual comes from, a sponsorship line takes its actual from sponsor arrivals and **refuses a typed income record** (so the same dollar is never counted twice — `lib/coach-money-derived.ts`), so the coach got a budget line they could never record their concession takings against with nothing on screen saying why. The mapping is `direction='out'` → `cost`, else `fundraiser`→`funding`, `sponsor`→`sponsorship`, `typed`→`other_income` — the exact inverse of `LINE_KIND_ACTUAL_SOURCE`, computed in one place (`budgetLineKindForItem`, `lib/coach-budget-totals.ts`) with a unit test pinning both directions in step. ⚠ **The stored enum is untouched**: removing the QUESTION is a form change; removing the CONCEPT would be a migration across six readers. ⚠⚠ **SINCE MIG 285 IT IS DERIVED FROM THE SHELF, AND THAT REVERSES THE NARROWING THIS ENTRY USED TO RECORD.** The note here read *"no write path accepts it — a word a club or a coach invents has no machinery behind it, so it is born typed and stays typed"*, and called the consequence "a real narrowing accepted on purpose (owner, 2026-09-07): a coach who invents their own income word can no longer have the drive machinery report against it". The owner reversed that on **2026-09-08**: the category a word sits on decides who fills it in, so a `direction='in'` word takes `budget_categories.income_source` **from its category, at creation, on both the coach and the club item-create paths** — never from the request body, which still accepts nothing. A word filed on the platform *Fundraising* shelf is a fundraising word from birth. Everything else is still born `'typed'`, because every other shelf is; defaulting a *shelf* to a derived source would CLOSE its words' rows to typed income, which is the only way their money can be recorded. ⚠ Mig 285 part 3 re-asserts `actual_source = income_source` across every money-in word so the two facts can never sit out of step (it matched **zero rows** on both databases — the invariant, not a repair). ⚠ **A money-OUT word is always `typed`**, enforced by `budget_items_out_is_typed_check` — safe because a word never changes sides (owner ruling 2026-08-17; the coach item PATCH refuses a `direction` in the body outright). The derivation asks `direction` FIRST anyway, so the constraint is a belt, not the trousers. ⚠ **`Grant` IS `sponsor`, AND IT MOVED TO THE `Sponsorship` SHELF — migration 282.** This entry said for a week that Grant *kept* its Fundraising category and that moving it was "a separate decision, deliberately left open"; the owner closed that question on 2026-09-07 and 282 moved the word **and all six referencing tables' stored categories** with it. The reason it could not stay open: sponsor money is placed by the lines that claim it, so a team budgeting only a Grant line saw every sponsor cheque report under *Fundraising*, and a team budgeting a Grant **and** a Team sponsorship had claims in two categories and lost the whole pool to the no-category bucket — **budgeting sponsorship properly read worse than budgeting it carelessly.** (Corrected 2026-09-08, found by `/review` while mig 285 extended this same entry.) ⚠ **No backfill was needed and no figure moved**: verified on dev AND prod 2026-09-07, every money-in budget line in existence points at a **platform** item and already used the obvious pairing, and there are no club- or coach-created money-in items anywhere.
...
4. **UPDATE-only; no code INSERT path.** Rows exist only from migration seeds (048: 16 rows; 065: +8). The governed change-request apply path updates `price_id` + the three audit columns only; `plan_id`/`billing_cycle`/`environment`/`product_name` are seed-fixed slots. A new billable plan key needs a migration to seed its 4 rows. **`updated_at` is CODE-maintained** (no trigger, probed live): the apply path sets it explicitly ([change-requests/route.ts:292-303](../../../app/api/platform-admin/product-catalog/change-requests/route.ts#L292)).
5. **Every `price_id` write requires an APPROVED catalog change request — no unilateral edit path.** Approving a `stripe_price_update` proposal on the change-requests PATCH applies it inline with a 409 optimistic-concurrency guard + slot-drift check, validates the `price_` prefix, runs `validateStripePriceForSlot` (hard-block), then auto-marks the request `implemented` ([change-requests/route.ts:685](../../../app/api/platform-admin/product-catalog/change-requests/route.ts#L685)) and double-logs to `platform_audit_log` (`update_stripe_price_id`) + `platform_catalog_change_applications`. (A standalone `PATCH /api/platform-admin/stripe-prices` apply-endpoint existed until the 2026-07 codebase cleanup removed it as a less-guarded duplicate — `/billing`-approved.) Reads: the Plans & Pricing page loads slots server-side via `getAllStripePrices()`; any platform-admin role incl. `read_only` can view.
6. **Cross-environment edits are accepted UNVALIDATED.** Stripe-API active-price validation runs only when the row's `environment` matches the runtime key's environment ([change-requests/route.ts:273-290](../../../app/api/platform-admin/product-catalog/change-requests/route.ts#L273)) — editing a `live` row from a sandbox-keyed deployment (exactly how prod rows would be staged from dev) skips validation; a typo'd live id saves fine.
...
2. **Mark-read is a POST with an `action` body — there is NO PATCH verb** ([route.ts:76](../../../app/api/notifications/route.ts#L76); actions `mark-read` / `mark-all-read` / `clear`). `read_at` is set to the request-time ISO string ([:93](../../../app/api/notifications/route.ts#L93)); unread = `read_at IS NULL`. **`clear` is a third, separate state** (mig 278) — see `cleared_at` below; `mark-all-read` deliberately excludes every `'act'` event type so it can touch neither state on a row that still owes a decision.
...
<!-- dict:col:notifications.cleared_at -->
**`cleared_at`** (tstz, nullable) — NULL = the recipient is still owed a decision on this one; ISO timestamp = they pressed **Clear** and are finished with it (mig 278). This is what pins an `'act'`-category event to the bell's **"Needs attention"** zone: the zone filters `cleared_at IS NULL AND category = 'act'`, in both the dropdown ([NotificationPanel.tsx](../../../components/notifications/NotificationPanel.tsx)) and the "See all" feed ([useNotificationFeed.ts](../../../components/notifications/useNotificationFeed.ts)). Written **only** by the `clear` POST action ([route.ts](../../../app/api/notifications/route.ts)); there is no clear-all, by design. **⚠ NOT a second `read_at`.** `read_at` means the words were seen; `cleared_at` means the thing was handled. Before mig 278 the zone was built on `read_at IS NULL`, so opening a failed payment and fixing nothing dropped it out of the zone and took the count down with it — the heading asked "have you dealt with this?" and the data could only answer "have you looked at it?". Owner ruling D3 (2026-09-03) — *"Mark all read" must leave act rows alone* — depends on the two staying apart, and mark-all-read must never learn to write this column. Clearing **does** set `read_at` when it is still NULL (guarded, so an old row's first-seen stamp is never rewritten); the reverse never happens. **Deliberately NOT indexed** — no query filters on it. The API returns a page for (user_id, org_id) ordered by created_at and the zone split is done CLIENT-side, so a partial index on `cleared_at IS NULL` could never be chosen by the planner and would cost this write-hot table maintenance for zero reads (mig 278 says so in full). NULL on every non-`act` row and harmless there.
...
5. **Count is live truth, not the migration — and this line proves why.** Re-measured 2026-09-08 **after the promote** against BOTH live databases: **dev 32 rows and prod 32 rows**, each 21 transactional/system + **11 marketing** — the two agree, mig 284 having reached prod that day. This gotcha previously read "34 rows (24 transactional/system + 10 marketing)" and a Phase 1 edit carried the stale 24 forward as "35" without re-measuring; the real 21 is what remains after **migration 264 deleted three retired tryout templates — which has run on PRODUCTION** (re-verified 2026-09-08 by querying prod for the three keys: all absent, and mig 083 had seeded them there). CLAUDE.md described 264 as “held” for weeks and was corrected the same day. ⚠ Never restate this count without querying both environments: it is data, no gate can see it, and it has now been wrong twice. Seed-only: a new key needs a migration (284 added `founding_nudge`). ⚠ **The marketing row count is NOT the number of campaigns an operator can send.** As of Founding Season 2027 Phase 1 a campaign can be **RETIRED**: its row stays (so the copy survives for a product that is only parked) while the app drops it from the dashboard, the recipient counts and the send route's allowlist. Three are retired — `spotlight_club`, `spotlight_league`, `spotlight_club_last` — so **8 are live**. Retirement is deliberately not a DELETE: a data-only row deletion is invisible to both drift checks, so nothing can tell you whether it ran — which is why 264 was believed to be outstanding for weeks after it had in fact been applied to production. The live/retired split is declared in [lib/marketing-email-defaults.ts](../../../lib/marketing-email-defaults.ts) — the campaign registry every consumer derives from — **never inferred from this table**. `updated_at`/`updated_by` are code-maintained (no trigger).
...
<!-- dict:col:user_preferences.coach_tour_dismissed_at -->
**`coach_tour_dismissed_at`** (tstz, nullable; mig 209) — when this coach finished or **skipped** the Premium coach-portal tour. **NULL = never decided ⇒ still offer it.** Account-scoped on purpose: the owner requirement is that skipping ends the offer across **every team the coach holds and every device they sign in on**, which device-local storage could never meet (a new device would re-offer a tour they explicitly killed). A **timestamp rather than a bool** so "skipped in week one" is distinguishable from "skipped a year later" without a second column. The tour stays reachable on demand from the help button and the season-setup chip regardless of this value — this kills the *offer*, not the *feature*. Read/written via `lib/user-preferences.ts` (`getCoachOnboardingPrefs` / `setCoachOnboardingPrefs`) behind `/api/coaches/onboarding-preferences`.

<!-- dict:col:user_preferences.coach_setup_hints_off -->
**`coach_setup_hints_off`** (bool, NN, DEFAULT false; mig 209) — whether this coach turned off Premium **season-setup guidance** (the header chip's count + the next-action line under the page header). `false` = show guidance, the default a brand-new coach gets. **Per COACH, not per team** — a coach with three teams dismisses once. Plain bool with a NOT NULL default rather than `theme`'s nullable tri-state: there is no third "unset" meaning here, and defaulting to `false` keeps the read path free of null-coalescing. Fully reversible from the season-setup chip ("Turn setup hints back on"); nothing is destroyed while it is true. **Per-team setup STEP skips are NOT here** — those stay device-local (`flhq.coachSetupSkips.<teamId>`) because they record a fact about a season, not a preference about the coach.
...
**`ref_sub_id`** (uuid, nullable) — sub-room identity. For `tournament`: **NULL = the default "All coaches" room** (one per tournament, zero-config, undeletable); a **fresh opaque uuid = an organizer-created division room** whose covered divisions ride `settings.divisionIds`. The mig-149 partial-unique guard keys on `(surface, ref_id, ref_sub_id)`, and each division room gets a fresh uuid, so no two rooms collide (even with identical division sets/names). For `coach_peer` (Project 2A, 2026-07-29): **the rep_team id — one STAFF room per team** (`ref_id` = the owning org); `ref_sub_id NULL` stays reserved for a future org-wide room (deliverable 2B, held for the Club evaluation).
...
<!-- dict:col:chat_room_members.history_visible_from -->
**`history_visible_from`** (timestamptz, nullable; mig 208, extended by mig 209) — the member's **history watermark** (Coach Chat Project 2A / CH-5 as amended 2026-07-29: in a team STAFF room nobody inherits history). NULL = full history — **every tournament-room row, always**; only `syncStaffChatRoom` sets it, stamping `now()` when a staff seat is created or **re-activated** (a returning coach sees from the most recent join). Enforced in depth (2026-07-29 adversarial review): (1) `chat_messages` SELECT + moderator-UPDATE **RLS** (`sent_at >= history_visible_from`); (2) mig **209** extends the same predicate to `chat_message_reactions` + `chat_poll_votes` SELECT (reaction/vote metadata of a pre-join message is invisible, incl. via realtime); (3) service-role read paths take a `visibleFrom` (history/pinned/unread/preview/reaction-summaries/poll-tallies/reactors/voters); (4) interactions (react, vote, poll-close, pin, moderator-delete, **reply-quote at send**) refuse a pre-watermark target; (5) **coach_peer rooms persist reply-quotes as `metadata.replyTo = { id }` ONLY** — never the quoted text/name (the denormalized snippet was readable via realtime payloads and direct selects by members seated between target and reply); readers get quotes hydrated per-viewer by `resolveReplyRefsForViewer` (hidden stub carries a BLANKED id — the raw id was an oracle key). Tournament rooms keep the shipped denormalized quote shape (no watermarks there). DISTINCT from `last_read_at` (what you've seen) and both mute columns.

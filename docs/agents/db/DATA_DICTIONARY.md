...
**`attempt_no`** (smallint, NOT NULL, default 1; CHECK `1–5`; mig 295) — which attempt within its session (the SESSION's `scope_attempts` says how many were planned for the test — a floor, never a ceiling: the row may hold up to five, C2). **Every row written before 2026-09-13 is attempt 1**, which is the truth: one reading was taken. A single reading (`session_id` NULL) is always 1. Part of the per-attempt partial unique (gotcha 7).

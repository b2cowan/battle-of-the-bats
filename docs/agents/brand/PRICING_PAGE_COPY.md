...
> - **Basic Coaches Portal** — the free floor (tournament participants + standalone free teams).
> - **Premium Coaches Portal** — the paid tier (standalone $29/mo, org-billed, or Club coaches).
>
...
## AMENDMENT — In-portal Premium bridges, free Coaches Portal (applied 2026-07-27, B3)

Free Coach Portal Experience **Phase B3**. Records the in-product upsell copy that ships with it. **No price, plan-name, capacity, or gate change** — every price/date line still reads from the existing Founding Season promo gate, and all four surfaces reconcile to `docs/agents/strategy/PLAN_PRICING_FACTS.md` (Premium Coaches Portal $29/mo · $290/yr, $0 until 2027-01-01).

**The binding constraint this copy works inside — the pressure ladder** (`FREE_TIER_STRATEGY_PLAN.md` D5): in-portal surfaces before and during a tournament stay **pitch-free**; the single earned ask is the post-event afterglow. Team-tools pages (roster / schedule / fees / announcements / Explore / Overview) carry the quiet info-first ScopeShelf as a standing exception. B3 adds nothing to the pre-event or game-day tournament record. **Owner call 2026-07-27 (◆L1): the plan's proposed multi-game-day lineup callout was DROPPED rather than excepted** — the ladder stands.

**New / changed copy:**
- **Afterglow lead (post-event tournament record, free tier only):** "That's the first line of {Team}'s season history." — falls back to "That's a wrap for {Team}." when no completed scores exist. Supporting line: "Your Coaches Portal stays free between tournaments — but the record, who played and who earned an award go with the event unless you keep them." **Deliberately does NOT restate the win-loss record** (the hero result card directly above owns that number).
- **Afterglow upgrade panel** (`ScopeCeilingInterest`, checkout-open branch) retitled to the moment rather than the toolkit: eyebrow "Premium Coaches Portal" / heading **"Keep the season, not just the weekend"** / body "Season history across every event, playing time, attendance and team awards — plus lineups, dues automation, documents and a season budget." Promo/price sentence unchanged. (Supersedes the 2026-07-26 "…your season operations HQ: lineups, attendance, documents, dues automation, and a season budget" wording **for this panel only**; the Explore block keeps "Ready for the full toolkit?" + the operations-HQ framing.)
- **Team Tournaments list shelf** (NEW surface, `ScopeShelf section="tournaments"`, renders only at 2+ entries): "Second event, same roster. Premium carries your roster, staff and results from one tournament to the next — and into next season — instead of starting from a blank list each time." + the standard "Your free tools stay free" / promo / CTA lines.
- **Fees shelf lead** (NEW, renders only at 5+ players owing): "{N} players still owe — that's {N} reminders to send by hand." A statement of the coach's own numbers, not a claim about the product.

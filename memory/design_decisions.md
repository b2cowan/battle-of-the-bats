...
### 2026-09-08 — FUNDRAISING MONEY HAS ONE WAY IN: the category decides who fills the number in and where it reports (owner: "this is much better, simpler, and basically covers all functionality")
**Decision (owner, on mockup `docs/projects/active/COACH_FUNDRAISING_ONE_WAY_IN_MOCKUP.html`, artifact `8aa1e633` final version).** (1) **Fundraising and Sponsorship words are ALWAYS filled in from the Fundraising tab.** A fundraiser names which Fundraising line it is *raising for* (pre-filled *Fundraising drive*), a sponsor which Sponsorship line (pre-filled *Team sponsorship*); each picker is filtered to its own shelf — the standard words and the team's/club's own, never the other shelf. (2) **"Other money in" holds only typed words** (Tournaments, Other Income, any coach-made category); the four Fundraising/Sponsorship words are **REMOVED** from its picker, not blocked, because Record money's first question already routes. Optional: one sentence on the existing empty state for the coach who types "merch" first. (3) **A line reports under its category's shelf** — Fundraising → Expected fundraising, Sponsorship → Expected sponsorship, everything else → Expected other income grouped by category; a word a coach adds to Fundraising is a fundraising word from birth (fixes the live defect where a coach's own word reports as other income while its row reads "Fundraising ·"). (4) **"The whole team" is a drive entry** with no player and no family share — the ONLY way team-raised money gets in once the typed path closes, so it ships with the model; the drive's board shows a dash where a share would be and the facts line counts team entries separately ("2 of 14 players · plus 1 team entry"). (5) **The budget picker's "From a drive" / "From a sponsor" note comes OFF and nothing replaces it** — a coach choosing a budget word is choosing a kind of money, not a room. (6) **No sponsor-or-grant switch**: the two behave identically (a promise, then arrivals, optionally crediting families); the only consequence of the difference is which Sponsorship line it reports against, and "Raising for" IS that. Three labels say "grant" where they did not: *Which sponsor or grant?* · *A new sponsor or grant…* · *Sponsor or grant* ("e.g. Riverdale Dental, or Community Sport Grant"). (7) "Raising for" is **pre-filled, neither required nor nudged**. (8) **Tournament revenue is other income** (asked, because hosting a tournament is a classic team fundraiser; settled). (9) A **coach-invented category's** money-in words report as other income, **never asked as a question** (a category is not one side of the books — Tournaments holds revenue and Officials; most coach categories are cost categories); stated on the create panel's existing footer sentence ("Saved as money coming in — because that is what you are recording. **Will report under Other income.**"). ⚠⚠ **BUILT on `dev` 2026-09-08** (migration 285, on prod 2026-09-08 (job 262); Owner QA §157, walk artifact `f8e843e3`) — and **TWO of the words above were corrected before any code, both put to the owner and both answered**: (a) the create panel says "Will report under **Other income**.", NOT "Expected other income" — the budget-ladder release took "Expected" off every section heading the same day, so the drawn wording would have named a heading no screen carries; (b) **"Who raised it" is a DROPDOWN** with *The whole team* first and a **Players** group under it, not the mockup's open list with a "No family share" chip — every sibling identity question on that form is a dropdown and the house rule says form selects are dropdowns, so the chip's fact became a hint under the field. The open item (a whole-team entry on a drive that pays a share) is **built ALLOWED** and is walk step **F4**, the one thing on §157 that asks for a ruling rather than a tick.
**Rationale.** The owner's simplification beat the three-door design that preceded it (blocked rows with "Record it there" links, a "carry me there" switch, a separate "cut the weld" change): closing one of the two routes fundraising money could take made all of it unnecessary. The distinction that survives: a note telling a coach *where the app will make them stand* is our plumbing and is deleted; a line telling them *where their money will appear on their own report* is a fact about their books and is kept. Five defects made the case, all verified in code: the money form's "Other money in" claims grants ("Interest, a grant, anything else") two rows below the sponsor answer that also claims them; a drive entry requires a player, so a hoodie table has no way in; the derived warning is only a warning (Save is live, the server refuses with the same sentence, and only when a budget LINE happens to exist); ⚠⚠ **a drive is not attached to a budget line at all** — the whole per-source pool is placed by claiming lines, so budgeting two fundraising lines blanks BOTH into "Not itemized" (a team is punished for planning carefully; mig 282 fixed only the across-categories case); and coach-made words are born "typed". **Nobody's report moves:** mig 280 measured zero coach/club money-in words on either database and Tournaments' words already report as other income. **What a coach gives up, accepted as a trade:** fundraising or sponsorship money cannot be recorded without a fundraiser or sponsor record — one more step, and in return the money lands on the line they planned. ⚠ Not yet ruled: whole-team entries on a drive that has a family-share % — build ALLOWED as drawn, confirm at QA §157.
**Applies to:** `accounting/budget/panel.tsx` (`ACTUAL_SOURCE_TAG` deleted), `accounting/expenses/panel.tsx` (`CONV_BRANCH` sub-lines, the other-in picker filter, the sponsor labels, "Who raised it" with The whole team), `fundraisers/panel.tsx` + `DriveRoom.tsx` + `SponsorRoom.tsx` (Raising for; the board), the fundraiser and entries routes, `budget-vs-actual/route.ts` (per-record placement; `placeDerivedActual` survives only as the legacy fallback), `lib/coach-money-derived.ts` (structural refusal), `components/accounting/BudgetItemPicker.tsx` (footer sentence), the coach demo seed and narration, four help articles. Migration 285 (`budget_categories.income_source`; `rep_fundraisers.budget_item_id` + `budget_category_id`, backfilled ONLY where a program year has exactly one line of the record's kind — never blanket-defaulted). Plan `COACH_FUNDRAISING_ONE_WAY_IN_PLAN.md`. Amends the 2026-08-16 sponsorship-stays-distinct ruling only by giving each record its line; the never-both rule, player rebates and the derived pools all stand.

### 2026-09-08 — THE BUDGET PLAN ADDS UP ON THE PAGE: two bands, two subtotals wearing the tiles' names, and a closing ladder (owner: "yes this looks so much better … proceed with the build")
**Decision (owner, on mockup `docs/projects/active/COACH_BUDGET_PLAN_LADDER_MOCKUP.html`, artifact `e94d05d9` round 2).** (1) The plan's List and By-period grid split into a **COSTS** band and a **FUNDING** band — the shared `.moneyGridBand` recipe, olive tint, room above, real empty cells never a colSpan — and each band closes on a subtotal in the shared `.moneyGridTotal` recipe. (2) **THE RULE: the three tiles above the table are the table's three subtotals, with the same names verbatim** — Planned costs · Planned funding · Player installments — so the tiles read as the table's headline, not a second summary. (3) The close is a **ladder**: *Costs less funding* (cat weight, plain ground, 2px cap, one quiet sub-line "What player installments need to cover") → *Player installments* wearing the tile's Scheduled tag (RESTYLED: no category tint, ordinary ink — it is an input now, not a section) → *Short of covering the plan* in amber / *Planned buffer* plain / no row when equal (UNCHANGED). Before dues exist the block is ONE row, *Player installments (Estimated)*, captioned "Costs less funding, until dues are set · ≈ per player · set dues for all players". (4) A season estimate that differs from the lines gets two line-weight rows under the last category — *Lines so far*, then *Still to itemize* or *Over your estimate* in the tile's red — and *Planned costs* stays the estimate. (5) On a phone the Scheduled/Estimated tag drops UNDER the label so the name never wraps into the figure column. (6) Words: bands are bare nouns; kind rows are bare (Fundraising, Sponsorship, Other income); the one qualifier is **Planned**, on both subtotals; **"expected" leaves the plan** (Business Decisions Log 2026-09-08). (7) **One name, one number, kept by `/review` the same day:** under the When filter the List's subtotals read **Costs shown / Funding shown** (a slice may not borrow the tiles' names); the By-period grid's cost subtotal reads **Lines so far** whenever a season estimate is set and differs (the grid spreads LINES; the estimate has no dates), with a footnote saying so; the export is always the WHOLE plan; band rows are UPPERCASE in the file (the REVENUE / EXPENSES convention) with a blank money cell.
**Rationale:** the owner's screenshot showed six rows at one level, two of them green, and a close with nothing on screen showing what it was subtracted from — a coach had to add four rows, add two, subtract, then subtract again. The recorded reasons for the old shape all explained the missing *Revenue* band; none explained the missing *subtotal*. Bands over subtotals-alone because a green row still reads as a category until you have passed it. Cost accepted: ~five rows, ~220px, on a tab whose table is the thing it exists for. **Two build-time calls, open to QA:** subtotals always render when their band does (stable shape; the tile's name is always in the table); the season's close steps aside under the When filter, where the subtotals sum the SHOWN rows (a slice has no shortfall; the tiles keep saying it). Installments are NOT spread into the By-period grid — that is Budget vs. Actual → Months' job with real arrivals.
**Applies to:** `accounting/budget/panel.tsx` (List + grid), `budget.module.css` (`.ladderRow`, `.ladderGap`, `.estimateRow`, `.estimateOver`, `.ladderBadge`), `lib/coach-budget-totals.ts` (`LINE_KIND_SECTION` bare nouns; `PLAN_LADDER_LABEL`, the one home for every ladder word), `lib/coach-budget-periods-view.ts` (`costTotals` / `fundingTotals`), `lib/coach-money-exports.ts` (both files mirror the screen, bands as `section` rows), the set-dues window's arithmetic line, five help articles. Plan `COACH_BUDGET_PLAN_LADDER_PLAN.md`. Amends the 2026-08-13 "expenses minus the funding" entry only in its words (Expected → Planned); the no-minus-sign, green-money-in, dues-are-the-answer rulings all stand.

### 2026-09-08 — A ONE-ANSWER FIELD IS STATED IN THE BAND, ABOVE EVERY FIELD — and text inside a modal takes a ladder token or it is 16px (owner, design review: "i agree with your mockups, go ahead and build")
**Decision (owner, on mockup `docs/projects/active/COACH_MONEY_MODAL_STATED_KIND_MOCKUP.html`, artifact `4fe1a703`).** (1) When a form's kind question has exactly ONE answer, it is stated in the olive stated band (`.convLockBand`) that Record money wears under a locked door — at the TOP of the form above every field, with NO label above it, because the band names itself. The Add-a-credit form's Type moves there: line one the kind ("Adjustment"; an older credit of a retired kind reads its own name), line two the kind's sentence ("A credit with no money behind it — it counts as no revenue."). A band IN the grid beside an input was drawn and rejected: its olive edge matches the input's height and it reads as a second field, and the explainer still lengthened one column. (2) The teaching sentence that points at the three other doors stays, one quiet full-width line under the band; the definition of an adjustment moved up into the band. Notes takes the whole row. (3) The Pay-out tick-list ("What are you paying back?") takes the FULL row — it sat in one column of the two-column grid, 229px of 472px, with Date paid alone opposite and How landing under the list — its titles and amounts are `--type-body`, rows centre their checkbox, and "Paying back" is the list's own footer row on the `--bg` ground rather than a separate bordered line beneath it. Date paid and How now pair on one row.
**Rationale:** both defects had ONE cause — two texts placed inside a modal with no type token inherited the browser's default 16px and were the only text in either form off the ladder, so each read as a heading beside 14px inputs. **Standing rule:** every text node a coach reads inside a modal takes a `--type-*` token explicitly; "inherits" is not a size. And a field that has become a statement goes where Record money puts its statements: above everything that is still asked. On a phone both grids were already one column, so the half-width list was desktop-only — the reverse of the usual miss, and no sweep opens a modal.
...
2. **How large a picture may be drawn is TWO rules, not one — conflating them costs legibility (owner correction, 2026-08-20, mid-walk).** A capture whose **subject is the phone experience** never grows past the size it was taken: enlarged on a laptop it stops looking like a phone and starts claiming to be a desktop app. A **desktop capture with a tight crop** misrepresents nothing by being bigger — it was simply cropped small — and capping it leaves it stranded in a frame with room to spare. Since captures are taken at double density the pixels are already there, so a desktop crop grows to fill the stage **bounded by its own pixel count**, and can never be drawn softer than 1:1. The build shipped the blunt "never enlarge anything" version first; the owner caught it on the real page, where it had left the playoff bracket at 480px inside a 671px frame.
...
**⚠⚠ RIDER 2 (2026-08-20, P2a) — AND THE SAME ARITHMETIC BINDS ON HEIGHT: A PHONE CAPTURE MUST BE SHORT ENOUGH TO BE DRAWN AT FULL WIDTH.** The entry above records that cropping ROWS shortens a picture without narrowing it, so only a phone-width re-capture could close the phone defect. Re-capturing closed it (Player Dues 33% → **92%**, Season settlement 39% → **92%** at 390px, measured on the real page at four widths, shape distortion under 0.9%) — but only after the second half of the rule was paid for. The settlement sheet's first phone crop was the whole modal: **390×1000, a 0.39 ratio, which the fixed stage draws about 165px wide on a laptop** — because the stage spends its height budget through the width, a very tall picture is punished exactly as a very wide one is on a phone. Dropping the team's-money summary from frame (nine lines of small figures, illegible at that scale anyway) took it to 359×395 and bought ~90% at phone width **and** ~100% on a laptop. **So "photograph it on a phone" is necessary and not sufficient: the crop's RATIO decides whether the picture is legible anywhere.** Aim near the scorekeeper capture's proportions (~0.65), and do not reach for a taller crop because it contains more.

**⚠ And a rule about rings that only appears when a picture is re-photographed:** slide #01's two rings marked the desktop table's Balance and Status COLUMNS, and the phone layout has no columns — one card per family instead. They were **dropped rather than re-pointed**, because a ring on a card is a row position wearing a different shape (which family is behind moves with the nightly re-anchor). The deeper reason is that a ring's job on this format is to tell a reader who CANNOT resolve the numbers what the sentence is about; at 92% of true size that job is gone. **A ring earns its place from the picture's SCALE, not from the sentence beside it.**

...
- Archived implementation plan: `docs/projects/archive/codex_PUBLIC_TOURNAMENT_MOBILE_EXPERIENCE_PLAN.md`.
- Archived PM brief: `docs/projects/archive/codex_PUBLIC_TOURNAMENT_MOBILE_EXPERIENCE_PM_BRIEF.md`.
...
- 2026-06-02 density correction: for public mobile tournament pages, compress support chrome before shrinking primary data. Schedule follow controls, filters, TBD notice, and score-row metadata were tightened. Standings table text/padding was restored upward because the phone table remained readable; only the summary/My Team chrome should stay compact.
- 2026-06-02 schedule row cleanup: public Schedule rows should not repeat "Scheduled" on every unscored game. Only meaningful states such as Final, Pending, or Cancelled belong in the top-right rail, before the followed-team star. Venue context should split into venue and facility/surface lines when facility data is available.
- 2026-06-02 schedule row/detail split: public Schedule rows should be compact selectable game-index rows without venue text. Venue, facility/surface, Google Maps, game notes, division, stage, status, and full matchup context belong on the dedicated public game details page.
- 2026-06-02 completed public record: completed tournament home should compact the hero, prefer Final Standings, relabel Schedule as Game Log, avoid empty upcoming-games language, and show a Final Public Record section with score counts, pending/unscored counts, accepted teams, top standings by division, and latest final scores. Browser visual testing/design refinements are closed out of this project and should move to a separate design project.
- 2026-06-02 shared public states: `components/public/PublicTournamentState.tsx` is the compact shared pattern for public empty/unavailable routes. It is applied across Schedule, Standings/Results, Teams, Registration, News, Rules, public game details, and public home schedule/news fallbacks. Direct hidden News/Rules URLs now show helpful unavailable states with contact/links instead of plain not-found.
- 2026-06-02 archive decision: public tournament mobile experience project is closed and archived as implementation-complete. Browser visual/design refinements move to a separate design project, route-specific analytics remain future product work, and Plus shareable post-event recap remains a separate product decision.

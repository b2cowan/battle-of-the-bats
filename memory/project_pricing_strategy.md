...
- **Family features packaging (decided 2026-07-11; ⚠ AMENDED 2026-08-01):** the whole Chunk D family layer is **PREMIUM-ONLY** and is BUILT on dev (guardian tier switched off pending counsel; ⚠ the follower tier + team family link were REMOVED 2026-09-12, and the per-game shared GAME PAGE + postgame draft REMOVED 2026-09-20 — the team page at Public link is the only no-login family surface). **FAMILY CHAT IS CUT — chat stays coaches/admins only, and G4's "chat basics with ANY portal" clause is VOID.** Practice-schedule visibility + richer family features = Premium Coaches Portal inclusion. No new SKU/price. Org "fan pass" = Proposed only. Also 2026-07-11: unified app ratified — per-tournament install identities retired; `advanced_tournament_branding` reframes to "branded space + QR on-ramp" (copy-only at build).

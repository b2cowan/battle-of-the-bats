...
- [ ] **Public Visual Redesign** — deeper premium/broadcast-grade visual pass, mobile-first + desktop: Phase A foundation (tabular numerals, card depth, frosted chrome, press states, animated toggles, staggered entrance, contrast guardrails, reduced-motion), Phase B signature mobile moments (My-Team game-day dock, broadcast scorecards + odometer scores, standings form-pips + RD bars), Phase C depth/identity (broadcast hero + count-up, bracket polish, team-color profiles, illustrated states), Phase D bigger bets (share cards, pull-to-refresh, offline shell, desktop left-rail). Planned 2026-06-02, not started (see [plan](docs/projects/active/PUBLIC_VISUAL_REDESIGN_PLAN.md) and [PM brief](docs/projects/active/PUBLIC_VISUAL_REDESIGN_PM_BRIEF.md))
- [ ] **Bulk Data Importer** - XLSX-first, CSV-compatible import workflow with current-data templates, empty templates, preview-before-commit, audit batches, tournament teams first, and tournament schedule second; Phase 1 tournament teams template + preview + add/update apply + Tournament Admin Data Tools hub built 2026-06-02, migration 106 applied dev + prod, dev UAT org repaired to active Tournament Plus, full add/update browser smoke passed after restart (see [plan](docs/projects/active/BULK_DATA_IMPORTER_PLAN.md) and [PM brief](docs/projects/active/BULK_DATA_IMPORTER_PM_BRIEF.md))

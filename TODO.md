...
- [ ] **Schedule Timeline (venue × time)** — SPEC'D 2026-06-04 (D2 of the admin redesign): a third schedule view next to List/Bracket — a cross-division day grid (facilities × time) with drag-to-move write-back (via `handleSaveGame`), conflict/gap shading (reuses C5 `buildConflictMap`), a TBD tray, and a mobile single-facility swipe carousel. Phased D2.1 read-only grid → D2.2 drag → D2.3 mobile → D2.4 polish. 6 open product decisions (Plus-gating, warn-vs-block on clash, all-vs-one division scope, time range, snap, mobile cross-facility move). Not started (see [plan](docs/projects/active/SCHEDULE_TIMELINE_PLAN.md) and [PM brief](docs/projects/active/SCHEDULE_TIMELINE_PM_BRIEF.md))

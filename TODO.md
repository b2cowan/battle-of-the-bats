...
- [ ] **Data Dictionary (semantic schema + maintenance rules)** — PLANNED 2026-06-08 (awaiting owner sign-off; nothing built). Build `docs/agents/db/DATA_DICTIONARY.md`: per-field purpose/relationships/gotchas (semantic layer atop the `docs/agents/db/schema-snapshots` structure dumps), gotchas-first, by domain, starting with Tournaments & Registration. Codifies "decide column existence from the live dev/prod snapshots, never from migration files" + snapshot-refresh + dev-vs-prod drift-watch rules. **Grounding pass found the snapshots are FAR staler than the kickoff assumed (82–85 vs ~103 live tables) → tooling-first: Phase 0 = build a single `scripts/refresh-db-snapshots.mjs` that refreshes dev+prod JSON + a drift report (today's `refresh-db-schema.mjs` is dev-only/markdown-only), regenerate, THEN Phase 1 content.** Also caught real errors in the kickoff seed list (resolveGameTiming signature, "playoff_game_duration_minutes removed", teams.email-ILIKE coach key) — see plan §7. Kickoff at [DATA_DICTIONARY_KICKOFF.md](docs/projects/active/DATA_DICTIONARY_KICKOFF.md) (see [plan](docs/projects/active/DATA_DICTIONARY_PLAN.md) and [PM brief](docs/projects/active/DATA_DICTIONARY_PM_BRIEF.md)).

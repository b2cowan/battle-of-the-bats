    ...
    return (
      <OrgProvider initialOrg={authCtx.org} initialUserRole={null}>
        <div style={{ display: 'contents' }} {...coachWarmAttr}>
          <CoachThemeColor />
          <div className={styles.coachesShell}>
            {/* wall: this wall hard-blocks portal function — no bell (the notifications API
                is not billing-gated, so a working feed here would undercut the wall) and no
                feedback row in the account menu; identity + exits stay. */}
            <CoachTopStrip wall />
            <main className={styles.coachesMain}>
        ...
        <div style={{ display: 'contents' }} {...coachWarmAttr}>
          ...
          <div className={styles.coachesShell}>
            {/* Desktop-only by its own CSS; on a phone the card's doors below carry the same
                ...
                with no teams has nothing for it to list). wall: a wall keeps only the
                identity + exit doors it always had (coach-wall-doors pins that surface). */}
            <CoachTopStrip wall />
            <main className={styles.coachesMain}>
              <div className={styles.notAssigned}>

...
export default function AdminDashboard() {
  ...
  const cards = [
    { label: 'Age Groups', value: stats.ageGroups,      icon: Tag,       key: 'age-groups',    color: 'purple' },
    { label: 'Teams',      value: stats.teams,          icon: Users,     key: 'teams',         color: 'blue'   },
    { label: 'Scheduled',  value: stats.scheduled,      icon: Calendar,  key: 'schedule',      color: 'amber'  },
    { label: 'Completed',  value: stats.completed,      icon: Trophy,    key: 'results',       color: 'green'  },
    { label: 'News Posts', value: stats.announcements,  icon: Megaphone, key: 'announcements', color: 'pink'   },
  ];
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.statsGrid}>
        {cards.map(card => (
          <Link key={card.label} href={`${base}/${card.key}`} className={`card ${styles.statCard} ${styles[card.color]}`} id={`dashboard-${card.label.toLowerCase().replace(/\s/g, '-')}`}>

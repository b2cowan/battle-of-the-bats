  ...
  async function patch(id: string, updates: any, confirmMsg?: string) {
    const execute = async () => {
      ...
      try {
        ...
        if (confirmMsg) {
          setFeedback({ isOpen: true, title: 'Success', message: 'Update applied successfully.', type: 'success' });
        }
      } catch (e: any) {
    ...
    setFeedback({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          setFeedback({ isOpen: true, title: 'Deleted', message: 'Team registration removed.', type: 'success' });
        } catch (e: any) {
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const updates = shuffled.map((team, i) => ({
            id: team.id,
            updates: { poolId: pools[i % pools.length].id }
          }));
            ...
            <div className={s.expandedContent}>
              ...
              <div className={s.expandedActions}>
                ...
                <div className={styles.buttonGroup}>
                  {r.status !== 'accepted' && (
                    <button className="btn btn-primary btn-xs" onClick={() => patch(r.id, { status: 'accepted' }, `Accept "${r.name}"? An automated email will be sent.`)} disabled={busy}>Accept</button>
                  )}
                  {r.status !== 'waitlist' && (
                    <button className="btn btn-warning btn-xs" onClick={() => patch(r.id, { status: 'waitlist' }, `Move "${r.name}" to Waitlist?`)} disabled={busy}>Waitlist</button>
                  )}
                  {r.status !== 'pending' && r.status !== 'accepted' && (
                    <button className="btn btn-ghost btn-xs" onClick={() => patch(r.id, { status: 'pending' }, `Restore "${r.name}" to Pending?`)} disabled={busy}>Restore</button>
                  )}
                  {r.status !== 'rejected' && (

...
import {
  TOURNAMENT_TEAM_IMPORT_MAX_ROWS,
...
async function parseUploadedFile(file: File) {
  ...
  if (extension === 'csv') {
    return parseCSV(new TextDecoder().decode(buffer), TOURNAMENT_TEAM_IMPORT_MAX_ROWS);
  }
  if (extension === 'xlsx') {
    return parseXLSX(buffer, TOURNAMENT_TEAM_IMPORT_MAX_ROWS);
  }
...
export const POST = withObservability(async (req: Request, { params }: RouteParams) => {
  ...
  try {
    ...
    const preview = buildTournamentTeamImportPreview(parsed, context, batchId);
    ...
    const { error: batchError } = await supabaseAdmin.from('import_batches').insert({
      ...
      import_type: TOURNAMENT_TEAM_IMPORT_TYPE,
    ...
    return json({ error: error instanceof Error ? error.message : 'Import preview failed.' }, 500);
  }
}, { route: '/api/admin/tournaments/[tournamentId]/registrations/import/preview' });

...
// Raw handler shared by two routes — the official score route (which wraps it as its GET)
// and the scorekeeper score route (which wraps it under its own route key, avoiding a
// double observability wrap). It lives in this non-route module because Next.js route
// files may only export HTTP-method handlers + route config, not arbitrary symbols.

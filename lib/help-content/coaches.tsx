...
const coachesHelp: HelpPageContent = {
  ...
  sections: [
    ...
    },
    {
      ...
      faqs: [
        ...
        },
        {
          ...
          answer: (
            <>
              ...
              <HelpDefs>
                <HelpDef term="Goals">What this player is working on, in plain language. <strong>+ Add goal</strong> opens a sheet; under <em>More</em> say what success looks like, set a review date and pick a focus tag (your team&rsquo;s own focus words). A goal is a row that <strong>opens in place</strong>: its facts, then <strong>Review goal</strong> · <strong>Record an observation</strong> · <strong>Edit wording</strong>, then <em>how it has developed</em> — a dated timeline of when it was set, what you observed and every review; nothing is ever overwritten. The status on the row is a <strong>pick</strong> (Working on it · Achieved · Parked); choosing one adds a dated status change the history shows as one quiet line — it is not a note.</HelpDef>
        ...
        },
        {
          ...
          answer: (
            <>
              ...
              <p>Skills &amp; Goals has four tabs across the top: <strong>Overview</strong> (where you land), <strong>Sessions</strong> (this list), <strong>Players</strong> (everyone in roster order, with the headline of the latest session per player) and <strong>Metrics</strong> (your tests and skills, where each is defined). If there&rsquo;s no Start session button, no test is defined yet — the Overview says so.</p>
            </>
          ),
        },
        {
        ...
        },
        {
          ...
          popular: true,
          answerText: 'Nothing is lost. Once a player\'s history is linked (confirmed by you, or linked automatically by Start next season), their Skills & Goals tab gains a shut Previous seasons fold at its foot, titled by the season\'s own name ("2025 Season · Archive"), that opens to that season\'s goals, then its results, then attendance, exactly as they were recorded. It is deliberately a scrapbook, not a scoreboard: past seasons sit side by side and the app never computes "better or worse than last year" — youth seasons are not comparable. A previous season\x27s focus areas and results are read-only: nothing from this season can add to, edit or delete them ("This player belongs to a past season, which is read-only"). Nothing carries over on its own — last season\'s numbers stay in that season\'s archive so this season\'s trend lines stay honest, and a goal worth keeping is set again this season with the player. The Development report in Insights (the Development tab) shows a Returning player column so you can see at a glance whose past seasons are connected.',
      ...
      subtopics: [
        {
          ...
          content: (
            <>
              ...
              <p>One block is open at a time, so three blocks read as three lines, not three forms. A block you added from a drill opens straight onto the drill&rsquo;s own words instead of asking you to write them again. Reorder with the <strong>up and down arrows under each block&rsquo;s start time</strong> in the left margin — there whether the block is open or shut, so you can shuffle the evening without opening anything — rather than dragging, so it works with cold hands on a phone. Any sheet that opens over the plan — choosing players, picking a drill, saving a template — closes with <strong>Escape</strong> and puts you back where you were.</p>
            </>
          ),
        },
        {
    ...
    },
    {
      ...
      group: 'Premium Coaches Portal',
      heading: 'Drills: write it once, then it’s four taps (Premium)',
      ...
      subtopics: [
        {
          ...
          content: (
            <>
              <HelpDefs>
                <HelpDef term="Save as template…">On any practice plan — keeps the practice as it is now. It asks for a name and nothing else; the practice&rsquo;s tags and description travel with it, and the Templates tab is where you change a template&rsquo;s name or tags later.</HelpDef>
        ...
        },
        {
          ...
          content: (
            <>
              <p>On a blank practice, <strong>start this plan from…</strong> sits beside <strong>+ Add the first block</strong> and offers a template, a previous practice <em>or</em> a past season — one control, three ways in. Pick a template and the plan appears with a line at the top saying where it came from. (Once a block is written the door goes — the plan is yours to keep.)</p>
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>An <strong>evaluation session</strong> runs your tests across the whole roster in one go, at a practice. A few sessions a season is what turns single results into a trend you can actually coach from.</p>
          <p>The page opens on an <strong>Overview</strong> — one card that names your next move on a new team, and the season&rsquo;s dashboard once sessions exist — with Sessions, Players and Metrics beside it.</p>
        </>
      ),
      subtopics: [
        {
          id: 'premium-development-layout',
          title: 'How the page is laid out — the Overview, three views, and starting from scratch',
          content: (
            <>
              <p>Skills &amp; Goals is four tabs across the top, and one lime <strong>+ Start session</strong> button in the page header once a test exists:</p>
              <HelpDefs>
                <HelpDef term="Overview">Where you land. On a new team it is one card that names the next move — <em>Start by deciding what this team measures</em>, with a button straight into defining your first metric; once you have a test, <em>Run your first session</em>. Once a session exists it is the season&rsquo;s dashboard: three cards (<strong>Sessions</strong> this season, <strong>Players measured</strong> of your roster, <strong>Goals</strong> working, each with what needs doing beside it), a <strong>Needs attention</strong> list — goal reviews due, players with no result yet, a session left half-recorded — each line a link to where you act, and <strong>Everything in Skills &amp; Goals</strong>, one row per room with a figure. Every figure here is the same one the reports read; nothing is a second opinion.</HelpDef>
                <HelpDef term="Sessions">Every evaluation session as a row — its date and note — with a search box.</HelpDef>
                <HelpDef term="Players">Every player in roster order. <strong>Show</strong> picks what the row says: <em>Current focus</em>, or one test with the <em>headline</em> of <em>that</em> test&rsquo;s latest session (<em>7.48 seconds (of 3)</em>) and the date it was recorded — never one &ldquo;last eval&rdquo; for everything — or a skill with its latest observation. A player with nothing under that metric reads <em>not recorded</em> or <em>not observed</em>. Tap a name to open their record on the right row; the back arrow brings you straight back here.</HelpDef>
          ...
          content: (
            <>
              ...
              <HelpNote variant="info" title="Three sprints are one player">
                <p>Every attempt is kept as its own record, so best, average and spread can be read later without re-testing — but everywhere the product counts players, a player with three attempts is one player: on the session, on the Players view, in Insights and on the handout.</p>
              </HelpNote>
            </>
          ),
        },
        ...
        },
        {
          ...
          content: (
            <>
              <p>Insights&rsquo; <strong>Development</strong> tab is three reports behind one <strong>Report</strong> dropdown — Coverage, Player progress and Practice review — with a <strong>Metric</strong> dropdown beside it. Every choice is part of the page&rsquo;s address, so Back returns you to the report you were on and a bookmarked or shared link opens the same selections.</p>
              <HelpDefs>
                <HelpDef term="Coverage">One row per player, roster order — a checklist, never a ranking. A sentence above the table states what it counts: <em>&ldquo;4 of 12 players have a 60-yd sprint result recorded this season. This describes the records that exist. It does not assess the attention a player received.&rdquo;</em> <strong>Selected metric</strong> and <strong>Recorded on</strong> answer for the metric you chose with <em>its</em> date — <em>8.31 seconds (of 2)</em>, <em>2 of 3 in range</em>, a skill&rsquo;s descriptor, <em>Not assessed</em> dated by the session that marked it, or <em>No result recorded for this test this season</em>. There is no single &ldquo;last measurable&rdquo; any more: one fresh sprint never refreshes an old throw. <strong>In a plan</strong>, <strong>Returning player</strong> and the count-only finding line stay; <strong>Progress →</strong> on a row opens that player in the next report with the metric kept.</HelpDef>
                <HelpDef term="Player progress">One player, one metric, on a dated chart with the change stated in words and every attempt behind it — its own page below.</HelpDef>
                <HelpDef term="Practice review">Every practice with a plan or a recap, filtered by tag, labelled by what its records support — <em>Upcoming plan</em>, <em>Past plan · no recap</em>, <em>Recap recorded</em>. Only a recap says what happened. <strong>Open the plan</strong> (or <strong>Open plan and recap</strong>) works on a finished season too, read-only.</HelpDef>
              </HelpDefs>
              <p><strong>In a plan</strong> shows a quiet tick, or <em>not in a plan yet</em> — no counts and no scores, because it is a note about where your attention has gone, not a mark against a child — and appears only once a plan names players. Goals, observations and reviews show only to coaches with Internal notes; results show on any team duty.</p>
              <HelpNote variant="info" title="Two different kinds of truth, kept apart on purpose">
                <p>The coverage table says what was <em>planned</em>. Only a practice marked <em>Recap recorded</em> describes what happened — and it earns that because you sat down afterwards and wrote it. A plan alone, past or upcoming, says what was intended.</p>
              </HelpNote>
            </>
          ),
        },
        {
          ...
          content: (
            <>
              <p>Pick a <strong>Player</strong> (roster order), a <strong>Metric</strong>, what to <strong>Show</strong> — the best attempt (or the last, if the test leads with it) or the average of attempts, offered only where the test has a choice — and what to <strong>Compare</strong>: this season, or the last two records.</p>
              <p><strong>The answer comes first, in words:</strong> <em>8.28 seconds on 26 Aug · 0.34 seconds lower since 6 May</em>. Arithmetic in the unit, dated; the aim sits on the line beneath (<em>4 recorded results · 5 attempts · lower is the aim</em>), and the page never says faster, better or on track. A range test reads <em>2 of 3 in range · moved into the range since 27 May (0 of 3)</em>, or how far the average sits past the band.</p>
              <HelpDefs>
                <HelpDef term="The chart">Dates are spaced by real time, so a long gap looks like a long gap. The line follows the headline per session (or the average, if you chose it); every attempt is a small grey mark at its own value, so spread shows without a score. Labelled ticks, nothing filled beneath the line. A range test shades its band, fills the marks that landed in it and dashes the average.</HelpDef>
                <HelpDef term="One result">One point and no line — a single result is a point, not a change.</HelpDef>
                <HelpDef term="A changed unit">The earlier results are listed beneath the table but never drawn on the same line.</HelpDef>
                <HelpDef term="Records behind the chart">Every session, every attempt, the best (or attempts in range), the average, who entered it and a link to the session — with <em>corrected — was 8.51</em> where an attempt was corrected. <strong>Open the player&rsquo;s development →</strong> returns here afterwards.</HelpDef>
                <HelpDef term="A skill">The chart gives way to the <strong>observation timeline</strong> — each entry dated, with its descriptor, what you saw, who wrote it and the goal it is evidence for — followed by that goal&rsquo;s review history. No line ever joins descriptors.</HelpDef>
              </HelpDefs>
              <HelpNote variant="info" title="The caption under every chart is the rule">
                <p>A change in this test does not explain why it happened.</p>
              </HelpNote>
            </>
          ),
        },
        {
          ...
          content: (
            <>
              ...
              <HelpDefs>
                <HelpDef term="What we&rsquo;re working on">Tick the goals for this conversation. The goal&rsquo;s own &ldquo;what success looks like&rdquo; sentence prints beneath it. Goals being worked on are ticked to begin with.</HelpDef>
                ...
                <HelpDef term="Next step">Your own words. When a chosen goal carries a review date, the paper adds <em>We&rsquo;ll look at this together again on 24 Jun.</em> To keep the step on the record, follow <strong>Save this next step as a review →</strong> to the goal — the handout itself stores nothing.</HelpDef>
        ...
        },
        {
          ...
          answer: (
            <>
              ...
              <ul>
                <li>Dates are spaced by real time; the line follows the headline per session; every attempt is a small grey mark; labelled ticks, no fill.</li>
                ...
                <li>One result is one point with no line. A test whose unit changed lists its earlier results but never draws them on the same line.</li>
                <li><strong>Records behind the chart</strong> lists every session and attempt, with a link to the session.</li>
              </ul>
              <p>Choose a skill and you get the <strong>observation timeline</strong> instead — no line ever joins descriptors.</p>
            </>
          ),
        },
        {
          ...
          answer: (
            <>
              ...
              <ul>
                <li><strong>Upcoming plan</strong> — the practice hasn&rsquo;t happened yet. <em>This practice has not happened yet.</em></li>
      ...
      content: (
        <>
          ...
          <ul>
            ...
            <li><strong>Development</strong> — three reports behind one <strong>Report</strong> dropdown: <strong>Coverage</strong> (one row per player — active focus, the metric you chose with its own date, who was named in a practice plan), <strong>Player progress</strong> (one player&rsquo;s one test on a dated chart, with the change stated in words and every attempt behind it), and <strong>Practice review</strong> (every planned practice labelled by what its records support).</li>
          ...
          content: (
            <>
              ...
              <HelpNote variant="tip" title="Closing your season is what hands recaps to families">
                <p>There&rsquo;s no separate publish step. That&rsquo;s why the preview lives here during the season: it&rsquo;s while the season is open that you can still log a last reading or hand out an award. Preview needs player-notes access.</p>
              </HelpNote>
            </>
          ),
        },

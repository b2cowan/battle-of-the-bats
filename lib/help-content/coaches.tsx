    ...
    },
    {
      ...
      faqs: [
        ...
        },
        {
          ...
          popular: true,
          answerText: 'Open any player from your Roster and open their Skills & Goals tab. It has four views across the top — Goals, Results, Observations and Previous seasons — one at a time. Goals are the things this player is working on, in plain language ("first-step quickness off the bag") with one short note; tap Add goal, and under More you can say what success looks like, set a review date and pick a focus tag — one of your team\'s focus words, the same ones your drills use. Each goal says where it came from (set with the player, carried from a previous season, seeded from the tryout) and who wrote it. Select a goal to see its facts and how it has developed — a dated timeline of when it was set, what you observed and every review — nothing is ever overwritten. Review goal asks for a status (Working on it, Achieved or Parked — required), with a note, a next review date and evidence optional; tapping the status pill still works and adds a dated status-only review. Results are your tests. Each row leads with the latest session\'s headline — "7.48 seconds · best of 3 attempts · avg 7.553", or "2 of 3 in range" for a range test — with the date and a small trend line once there are two results; open a row and every session is listed with every attempt, its best and its average. Record a result adds a single dated result with no session (it was called Log a measurable before September 2026). If a test\'s unit ever changed, the line is drawn from the current unit only and the row says so; every result stays listed in the unit it was recorded in. Observations are what you saw against a skill — a descriptor and/or a sentence, dated, who wrote it, and "Evidence for" the goal it supports; Record an observation lives here and on a goal. Previous seasons is the read-only scrapbook of linked earlier seasons — never a "better or worse than last year". The old Context lines (depth chart, innings, attendance) left this tab; attendance and playing time live on This season. Preview development handout (it replaced Print summary (PDF)) opens a page where you tick which goals, observations and results go on the handout, write a next step and print it — one row per session with its headline and the attempts listed, no comparisons to other players and no share link. Every number compares the player only to themselves. The head coach, and any assistant with the Development switch on, can record results; goals, observations and reviews also need Internal notes; reading goals and observations needs Internal notes, results need any team duty. To define, rename or retire a test, follow the Metrics link at the top of the section. If the player came through your tryout and you ran Start development from tryouts, a Tryout snapshot card sits at the top of Goals showing where their season started — context only, never a measurable, coach-eyes-only.',
          keywords: ['development', 'player development', 'focus areas', 'focus area', 'idp', 'goals', 'add goal', 'review goal', 'goal review', 'success looks like', 'review date', 'next review', 'goal history', 'timeline', 'set with the player', 'carried from', 'seeded from the tryout', 'written by', 'measurables', 'measurable', 'log a measurable', 'record a result', 'results', 'attempts', 'best of', 'average', 'in range', 'observations', 'observation', 'record an observation', 'observed skill', 'descriptor', 'evidence for', 'previous seasons', 'four views', 'goals results observations', '60 yard', 'sprint time', 'test times', 'trend', 'sparkline', 'units changed', 'changed the unit', 'not drawn on this line', 'earlier readings', 'test types', 'metrics', 'context', 'depth chart', 'innings', 'attendance', 'print summary', 'pdf', 'handout', 'tryout snapshot', 'who can', 'internal notes'],
          answer: (
            <>
              <p>Open any player from your <strong>Roster</strong> and open their <strong>Skills &amp; Goals</strong> tab. It has <strong>four views</strong> across the top — <strong>Goals · Results · Observations · Previous seasons</strong> — one at a time.</p>
              <HelpDefs>
                <HelpDef term="Goals">What this player is working on, in plain language, with one short note. <strong>Add goal</strong>, and under <em>More</em> say what success looks like, set a review date and pick a focus tag (your team&rsquo;s own focus words). Each goal says where it came from and who wrote it. Select one to see its facts and <em>how it has developed</em> — a dated timeline of when it was set, what you observed and every review; nothing is ever overwritten. <strong>Review goal</strong> asks for a status (required) with a note, next review date and evidence optional; tapping the status pill still works and adds a dated status-only review.</HelpDef>
                <HelpDef term="Results">Your tests. Each row leads with the latest session&rsquo;s <strong>headline</strong> — <em>7.48 seconds · best of 3 attempts · avg 7.553</em>, or <em>2 of 3 in range</em> for a range test — with its date and a small trend line once there are two results. Open a row and every session is listed with every attempt, its best and its average. <strong>Record a result</strong> adds a single dated result with no session. If a test&rsquo;s unit ever changed, the line is drawn from the current unit only and the row says so.</HelpDef>
                <HelpDef term="Observations">What you saw against a skill — a descriptor and/or a sentence, dated, who wrote it, and <em>Evidence for</em> the goal it supports. <strong>Record an observation</strong> lives here, on a goal, and on a session&rsquo;s skill chip — the same sheet from every door.</HelpDef>
                <HelpDef term="Previous seasons">The read-only scrapbook of linked earlier seasons — never a &ldquo;better or worse than last year&rdquo;.</HelpDef>
              </HelpDefs>
              <p>The old <em>Context</em> lines (depth chart, innings, attendance) left this tab — attendance and playing time live on <em>This season</em>. <strong>Preview development handout</strong> (it replaced <em>Print summary (PDF)</em>) opens a page where you tick which goals, observations and results go on the handout, write a next step and print it: one row per session with its headline and the attempts listed, no comparisons to other players, no share link. Every number compares the player only to themselves.</p>
              <p>The head coach, and any assistant with the <strong>Development</strong> switch on, can record results; goals, observations and reviews also need <strong>Internal notes</strong>. To define, rename or retire a test, follow the <strong>Metrics</strong> link at the top of the section. A player who came through your tryout keeps their <strong>Tryout snapshot</strong> at the top of Goals — context only, never a measurable, coach-eyes-only.</p>
            </>
          ),
        },
        {
    ...
    },
    {
      ...
      subtopics: [
        ...
        },
        {
          ...
          title: 'A player&rsquo;s record — goals, results, observations, previous seasons',
          content: (
            <>
              <p>Open a player from the Roster and open their <strong>Skills &amp; Goals</strong> tab. It has four views across the top, one at a time. The returning-player card and the carry-forward offer sit above the views; the tryout snapshot heads Goals.</p>
              <HelpDefs>
                <HelpDef term="Goals">A list with a status and where each came from — <em>Set with the player</em>, <em>Carried from a previous season</em> or <em>Seeded from the tryout</em> (a goal written before September 2026 shows no origin) — and who wrote it. <strong>Add goal</strong> is a focus and a note; under <em>More</em>: what success looks like, a review date and a focus tag. Select a goal for its facts, its actions — <strong>Review goal</strong>, <strong>Record an observation</strong>, <strong>Edit wording</strong> — and <em>How this goal has developed</em>: set → observed → reviewed, dated, with the author, never overwritten.</HelpDef>
                <HelpDef term="Review goal">The status is the required choice — Working on it, Achieved or Parked; a note, the next review date and evidence are optional. Observations already linked to the goal are named as its evidence. Tapping the status pill on the list still works, and adds a dated status-only review.</HelpDef>
                <HelpDef term="Results">Each test leads with the latest session&rsquo;s headline — <em>7.48 seconds · best of 3 attempts · avg 7.553</em>, or <em>2 of 3 in range</em> — with its date and a small trend line once there are two results. Open a row for every session with every attempt, its best and its average, who entered it and a link to the session. <strong>Record a result</strong> adds a single dated result with no session (it was <em>Log a measurable</em> before September 2026). A retired test&rsquo;s results stay listed; a series whose unit changed stays split.</HelpDef>
                <HelpDef term="Observations">A timeline per skill: the descriptor, what you saw, whether it was taken in a session, who wrote it, and <em>Evidence for</em> the goal it supports. <strong>Record an observation</strong> lives here, on a goal, and on a session&rsquo;s skill chip — the same sheet from every door. Editing one sends only what you changed; a descriptor the skill no longer offers is shown as such so you can clear it.</HelpDef>
                <HelpDef term="Previous seasons">The read-only scrapbook of linked earlier seasons. Attendance and playing time live on the <em>This season</em> tab — the old <em>Context</em> lines left this one.</HelpDef>
              </HelpDefs>
              <p><strong>Preview development handout</strong> (it replaced <em>Print summary (PDF)</em>) opens a page where you tick the goals, the observations and the results that belong in this conversation, write a next step, and print or save the paper — one row per session with the headline and the attempts, never three rows for three sprints. See <em>The development handout</em> below.</p>
            </>
          ),
        },
        {

    ...
    },
    {
      ...
      subtopics: [
          ...
          content: (
            <>
              ...
              <HelpDefs>
                ...
                <HelpDef term="Fundraiser money came in">Logs it to the drive&apos;s leaderboard and credits the player, same as logging it from Fundraising.</HelpDef>
                <HelpDef term="A sponsor came through">Records the sponsor right there — name, amount, and (optionally) which player brought them in, with a per-sponsor <em>Credit to that family</em> ($ or %, prefilled with your team&apos;s standard share). It lands on Fundraising exactly as if created there. A <em>promised</em> sponsorship is a pledge — record that on Fundraising, where its status lives.</HelpDef>
              ...
              <HelpNote variant="tip" title="One word, wherever you record money">Every door onto this form now says <strong>Record</strong> — on a family&apos;s record, on a drive&apos;s leaderboard, on a bill&apos;s row. Opened from one of those, the first questions arrive <strong>already answered and locked</strong>: standing on a family&apos;s record, the form says <em>&ldquo;A family paid their dues — Jenny Alvarez&rdquo;</em> and gets on with the amount. Wrong family? Close it and open the right one.</HelpNote>
              <p>Two kinds of button deliberately did <strong>not</strong> change. The one-tap shortcuts still don&apos;t stop to ask: <strong>Record as paid</strong> / <strong>Record rest as paid</strong> on a dues installment, and <strong>Record as paid</strong> on a club installment. And <strong>editing</strong> stays where the record lives — the pencil on a dues receipt, <strong>Edit amount</strong> on a drive row — because correcting something is a different act from writing it down. The Ledger keeps its own <strong>Add a bill</strong>, which is how payment schedules are made.</p>
        ...
        },
        {
          ...
          content: (
            <>
              <p>The <strong>Fundraising</strong> tab holds two kinds of money coming in, each in its own band, and the difference decides what you record. A <strong>fundraiser</strong> is something the whole team takes part in — a bottle drive, a chocolate sale — so its row opens into what each player raised. A <strong>sponsorship</strong> is a promise from one business or grant, kept by cheques as they arrive — so it&apos;s a row in a small ledger: what was <strong>Pledged</strong>, what&apos;s <strong>In</strong>, what&apos;s still <strong>To come</strong>, and any <strong>Credits</strong> earned by the families who brought it in.</p>
              <p>Each band has its own button — <strong>＋ Fundraiser</strong> and <strong>＋ Pledge</strong> — so nothing ever asks you which kind you mean. The sponsor button says <em>pledge</em> on purpose: it makes a promise, and money already in your hand goes through <strong>Record</strong> instead. The Money <strong>Overview</strong> lists the two separately as well, and its <strong>Sponsorships</strong> row lands you right on the sponsor band.</p>
              ...
              <p>When a cheque arrives, press <strong>Record</strong> — on the sponsor&apos;s own row, or anywhere in Money — and answer <em>a sponsor came through</em>. The form asks <strong>Which sponsor?</strong>, listing your pledges first with what each still owes; picking one records the arrival against its promise. A sponsor you never logged as a pledge is one more answer in the same list — <em>a new sponsor</em> — which creates them with their first cheque in one save. There is no Received-or-Pledged question anywhere: <strong>the numbers answer it</strong> — money in the In column means received, money in To come means still promised.</p>
              <p><strong>The date has to have happened.</strong> A cheque you are still waiting on is not recorded — it is promised, and the pledge already has an <strong>Expected by</strong> date for exactly that. Pick a day ahead and the form says so and points you there, rather than letting you fill the whole thing in first. The same rule holds on every screen where you write down money that moved.</p>
              ...
              <HelpDefs>
                <HelpDef term="A sponsor&apos;s row">Click it and it opens in place — the expected-by date, the credit split, every cheque with its date, and the buttons. No separate page.</HelpDef>
                <HelpDef term="Undo">Sits on each arrival. It takes that cheque back off the books along with the credit it gave out; undo the last one and the sponsor returns to being a pledge.</HelpDef>
                <HelpDef term="Edit">Opens <strong>Edit sponsorship</strong> — the name, pledged amount, expected-by, credit families and tags. It will refuse to shrink a family&apos;s credit below what has already been paid out to them, and says so before you can save.</HelpDef>
                <HelpDef term="Delete this sponsorship">At the foot of <strong>Edit sponsorship</strong>. A pledge with nothing arrived deletes on a plain confirm and no money moves; once a cheque has landed the button is greyed out and says how much is on the books, because the honest way out is <strong>Undo</strong> on each arrival first.</HelpDef>
              </HelpDefs>
              ...
              <p>Click a fundraiser&apos;s row and it opens right there in the list — no separate page. A quiet line states the drive&apos;s facts (its credit share, how many players have logged something so far, its dates and its tags), and under it sits one line per <em>amount logged</em>, largest first: who raised it, the day it arrived, and the credit it earned their family. Players who haven&apos;t taken part aren&apos;t drawn as empty rows — the count says how many are still to come.</p>
              <p><strong>Record</strong>, on the open drive, opens the same form the rest of Money uses with this drive already filled in. It asks which player — offering only those with nothing logged yet — and says plainly, before you save, what the family will be credited. Save and the credit is on their dues straight away; where it landed reads on the family&apos;s own dues screen. <strong>Edit amount</strong>, on a logged line, corrects the figure, the day it arrived or the note, right where it is.</p>
              <p><strong>Remove</strong> sits beside it, and takes the entry off entirely — the amount comes off the team&apos;s books and the credit comes off that family&apos;s dues, both named in dollars before you confirm. It&apos;s the only place a drive entry can be unwound: the credit it created is locked on the family&apos;s own dues screen precisely so there is one way back, and it stays available on a closed drive so a mistake is never permanent. If money has already been handed back to that family in cash, Remove refuses rather than leave them owed less than they&apos;ve received.</p>
              <HelpNote variant="info" title="Changing the credit % later">Each entry keeps the rate it was logged at. Changing the percentage in <strong>Edit</strong> applies to new entries only, so an amount you recorded last month never quietly changes value.</HelpNote>

              <h4>Tagging what came in</h4>
              <p>A fundraiser or a sponsorship can carry <strong>money tags</strong> — the same labels you put on expenses, so &ldquo;Winter dome&rdquo; means one thing whether the money went out or came in. Tags live on the record, not on the list, so pick them when you create it, or change them later in its <strong>Edit</strong> sheet — the same door for a drive and a sponsor. They come through on the export — with sponsor money split into <strong>Received</strong> and <strong>Pledged</strong> columns — which is what lets a spreadsheet show what a label cost you <em>and</em> what it brought in.</p>

              <h4>Closing one</h4>
              <p><strong>Edit</strong>, on the open drive, renames it, changes its dates or its rate, and sets it to <strong>Closed</strong> when it&apos;s over. A closed drive stays fully readable and keeps every credit it gave out — it just stops accepting new amounts.</p>
              <p><strong>Delete this fundraiser</strong>, at the foot of the same sheet, is for one started by mistake: it works only while nothing is logged. Once anyone has raised anything the button is greyed out, says what is logged against it, and points you at <strong>Closed</strong> — which is what you almost always want, because closing keeps every credit families have already earned. To delete it for real, remove the entries one at a time first.</p>

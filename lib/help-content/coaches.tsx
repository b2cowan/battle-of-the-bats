    ...
    },
    {
        ...
        },
        {
          ...
          content: (
            <>
              ...
              <p>The list shows both, each tagged, with a filter to see one kind at a time. The figures above it split the same way, so you can see how much of the season is funded by families selling things versus by sponsors.</p>
        ...
        },
        {
          ...
          content: (
            <>
              ...
              <p><strong>A payable is any commitment, not just a tournament.</strong> A dome block, an umpire invoice, a uniform order — anything with a due date belongs here. Record one amount due on one date, or open <strong>&ldquo;Split into a deposit and a balance&rdquo;</strong> when it&apos;s billed in two parts; each half keeps its own due date and its own paid tick. In the <strong>Payables</strong> list each commitment is one row — what it&apos;s for, its category, the total, and where it stands (<em>Scheduled</em>, <em>Part paid</em>, <em>Overdue</em> or <em>Paid</em>). Tap <strong>Payment details</strong> on the row to open the deposit and balance, with their due dates and their <strong>Mark paid</strong> buttons, plus that payable&apos;s notes.</p>
              <p><strong>Expense or payable? One button asks.</strong> <strong>Add</strong> on Expenses &amp; Payables opens one form, already set to whichever list you&apos;re looking at. An <strong>expense</strong> is money that has already left the team — you&apos;re recording what happened. A <strong>payable</strong> is money you&apos;ve promised but not paid — you&apos;re scheduling what&apos;s coming. The quick test: if it has a due date, it&apos;s a payable, and payables are what show on your Payment schedule. Picked the wrong one? Switch it at the top of the form and everything you&apos;ve typed comes with you.</p>
              <p><strong>Fixing a mistake.</strong> Tap the pencil on any row — or the row itself — to reopen it. Names, categories, notes and tags can always be changed. Once something is <strong>marked paid</strong> its amount locks, because that figure is already on the team&apos;s books; the form shows the amount with the reason and points you at the fix. <strong>Delete</strong> sits at the bottom of the same form. Deleting something already paid puts the money back and tells you the exact amount first, so a mistyped $1,300 is a correction rather than something you live with all season. If a family paid out of pocket, deleting also removes the credit the team owed them — the form says so before you confirm.</p>
      ...
      faqs: [
        ...
        },
        {
          ...
          question: 'Why does a budget line show “—” under Actual?',
          answer: (
            <>
              <p>Because an expense records a <strong>category</strong>, not which line inside it the money paid for. Budget vs. Actual reports your spending one level up: the <strong>category row carries the real Actual and variance</strong>, and the lines under it show a dash rather than a guess at how to divide it between them.</p>
              <p>There is one case where a line can be certain — when its category holds <strong>only that line</strong>. Then all of the category&apos;s spending is its spending, so expanding the line shows real figures against each payment period. A category holding several lines says so in a note under them.</p>
              <p>If you want one cost tracked on its own, give it a <strong>category of its own</strong>. That is the split the report can measure against.</p>
            </>
          ),
          answerText: 'Because an expense records a category, not which line inside it the money paid for. Budget vs. Actual reports your spending one level up: the category row carries the real Actual and variance, and the lines under it show a dash rather than a guess at how to divide it between them. There is one case where a line can be certain — when its category holds only that line. Then all of the category spending is its spending, so expanding the line shows real figures against each payment period. A category holding several lines says so in a note under them. If you want one cost tracked on its own, give it a category of its own. That is the split the report can measure against. The same rule applies in the Months view, where line rows show your plan and the category rows carry what was scheduled and paid.',
          keywords: ['dash under actual', 'em dash', 'budget line shows nothing', 'no actual on my line', 'actual is blank', 'line has no spending', 'why is actual empty', 'per line actual', 'per-line spending', 'line variance missing', 'spending by budget line', 'matched to the category not to a line', 'category has 3 budget lines'],
        },
        {
        ...
        },
        {
          ...
          answerText: 'Yes. Every expense and payable has a Tags box — start typing to find a label you have used or tap + Create for a new one (like Winter dome or Fundraiser). Only the tags you picked show as chips. On the Expenses list, a tag row up top lets you filter to one tag and see its total, every row shows its own tags, and you change them by opening that record with the pencil or by tapping the row. Tags come through on the export too. Budget vs. Actual gets the same filter — choose a tag to see just that spending, totalled across every category it touches. Money tags are their own list, separate from game tags, and need money edit access to add or change. A club or league can also share tags org-wide — those appear in blue and you can apply them, but only an org admin edits them. Use Manage tags on the Expenses page to rename, merge, or delete your own money tags.',

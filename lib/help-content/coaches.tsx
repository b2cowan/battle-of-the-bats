...
const coachesHelp: HelpPageContent = {
  ...
  sections: [
      ...
      content: (
        <>
          ...
          <ol>
            ...
            <li><strong>Score players yourself.</strong> On the <strong>Tryout day</strong> tab, tap <strong>Score players</strong> — you&apos;re straight into the field scorecard, signed in as you, with checked-in players listed first. Your scores count like anyone&apos;s and show on the scoreboard marked <strong>(you)</strong>.</li>
            <li><strong>Invite helpers to score too (optional).</strong> On the <strong>Evaluators</strong> card, add a helper by name to get them a private scoring link — <strong>no login, no app</strong>. It works for 48 hours (the link says until when) and you can turn it off any time — you&apos;ll be asked first. Lost or expired link? <strong>New link</strong> on their row issues a fresh one and <strong>keeps all their scores</strong>.</li>
            <li><strong>Watch the live scoreboard.</strong> The <strong>Live scoreboard</strong> ranks players by their weighted average across everyone scoring, and updates on its own as scores come in. A player who never checked in is marked, so a no-show never reads as a low scorer. If an evaluator&apos;s scores drift from the group, they&apos;re gently flagged &quot;runs hot/cold&quot; so you can weigh their input.</li>
            ...
            <li><strong>See who&apos;s been here before.</strong> If someone looks like a player from a past season, their row shows <strong>Possible returning player — verify</strong>. Open it, check the details side by side, and confirm it&apos;s the same child. Once you do, their row gains a <strong>memory strip</strong>: last season&apos;s tryout score beside this one, so the kid you cut last year who came back stronger is visible at the moment you decide. When both years used the same scoring scale you also get the change (<strong>▲ +0.7</strong>), and you can open <strong>Compare categories</strong> to see it skill by skill. If the two years used different scales, both are shown side by side with <strong>no</strong> arithmetic — a 7.1 out of 10 and a 3.6 out of 5 aren&apos;t the same measurement, and pretending otherwise would mislead you. Unverified matches show <strong>no scores at all</strong>, and none of this appears anywhere while names are still hidden.</li>
            <li><strong>Decide how families hear from you.</strong> Out of the box, decisions are <strong>only recorded</strong> — no emails go anywhere, and you deliver the news your way (most coaches call their picks). Flip <strong>Email families my decisions</strong> (the switch right above the buttons) and each choice emails the family for you: an <strong>Offer</strong> lands as a club-branded email with no-login <strong>Accept/Decline</strong> buttons good for <strong>7 days</strong>, <strong>Waitlist</strong> sends a &quot;you&apos;re on the waitlist&quot; note, and <strong>Not this season</strong> sends a warm &quot;not this time&quot; — that one asks you to confirm first, because an email can&apos;t be unsent. With the switch off, any offered player still has <strong>✉ Email this offer</strong> for sending just that family the self-serve link. Emailed offers show where the family stands — <em>awaiting</em>, <em>family accepted</em>, <em>declined</em>, or <em>expired</em> — a family with <strong>no email on file</strong> is flagged so you know to phone, and the note the family wrote at registration is one tap away on their row.</li>
            <li><strong>Accept them onto your roster.</strong> When a family accepts (or any time you&apos;re ready), tap <strong>Accept → add to roster</strong> — the family&apos;s response doesn&apos;t roster the player on its own; you always confirm. A quick drawer opens with their details already filled in from their registration (name, birthdate, guardian, and the family&apos;s note) — add an optional number, position, and jersey size. If your team already charges dues, their <strong>standard fee schedule is pre-filled</strong> and editable — that&apos;s whatever your roster is already paying, or, if you haven&apos;t set dues yet, your budget&apos;s per-player figure (net of any expected funding you&apos;ve budgeted). Leave it on to set them up with fees, or flip it off to add them now and set fees later. Confirm and they land on your roster in one step (the player and their fees are saved together — nothing half-finishes). Fees only <strong>record what&apos;s owed</strong> — no card is charged. The &quot;Welcome to the Team&quot; email follows the same email switch — off means you welcome them yourself.</li>
            <li><strong>Read your Tryout report.</strong> The <strong>Build team</strong> tab writes the story of your tryout as it happens: the path from signup to roster (with honest drop-off notes like &quot;3 never checked in&quot;), turnout compared to last season, where the incoming class is strong and thin by scorecard category, and a <strong>fairness receipt</strong> — a plain statement that players were scored by independent evaluators on one shared scorecard before names were revealed. If at least three verified returning candidates can be compared on the same scale, it also says how that group moved — <em>&quot;6 returning candidates improved +0.6 on average since their last tryout&quot;</em> — and below three it stays quiet rather than draw a conclusion from one or two players. The funnel row reads <strong>Offers extended</strong> because it counts every offer you ever made, including any you later took back; the Decide tab&apos;s tally counts who is offered <em>right now</em>, so the two can differ. It marks itself <strong>Final</strong> once every candidate has a decision. <strong>Export</strong> gives you a <strong>Board summary (PDF)</strong> — totals and roster names only, safe to hand to your club board or a curious parent — and, behind a deliberate confirmation, a <strong>Full detail</strong> PDF or Excel of every candidate with scores and decisions, for coaching staff only. Full detail can&apos;t be exported while names are still hidden.</li>
            <li><strong>Start development from tryouts.</strong> Once you&apos;ve accepted anyone, a card under the report offers to turn the tryout you just ran into the start of their season. <strong>Begin</strong> walks you through your new players one at a time: their <strong>tryout snapshot</strong> on screen, and — for anyone who scored below the middle of your scale — up to <strong>two suggested focus areas</strong> taken from their weakest categories, weakest first. Each suggestion is a picker against your team&apos;s existing focus words: match one, deliberately create a new one, or answer <strong>Don&apos;t add</strong>. <strong>Nothing is saved without you</strong>, and <strong>Skip this player</strong> works at every step. A player nobody scored simply says so and lets you set their focus by hand. Ten minutes here and your development hub starts the season populated instead of empty. A baseline is set <strong>once per season and never overwritten</strong>, so re-opening the walkthrough skips anyone already done.</li>
          </ol>
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p><strong>Premium Coaches Portal adds:</strong> installment schedules with due dates per player — build a <strong>Season Budget Plan</strong> and <strong>Generate installments</strong> creates every player&apos;s schedule in one click — automatic overdue reminder emails (the <strong>Automatic Dues Reminders</strong> toggle sits at the bottom of Player Dues), a line at the top of Player Dues telling you how many families still owe — with <strong>Remind all</strong> to nudge them in one go — and a season budget that tracks fees, expenses, and fundraiser credits.</p>
        </>
      ),
      faqs: [
        {
    ...
    },
    {
      ...
      content: (
        <>
          <p>When an organizer runs a chat for a tournament your team is in, that conversation shows up on the <strong>Chat</strong> tab of the FieldLogicHQ app — the same Chat tab as everything else, with every tournament you&rsquo;re in gathered in one <strong>inbox</strong>, grouped by event, and an <strong>unread badge</strong> on new messages. <strong>The Chat tab is the one place chat lives</strong> — there&rsquo;s no separate chat section inside your team. (On <strong>Premium Coaches Portal</strong>, Chat is also an item in your sidebar.) Open a conversation to read and reply in real time alongside the organizer and the other coaches in that tournament. On a <strong>computer</strong>, your conversation list stays <strong>beside</strong> the open conversation — like any desktop messaging app — so switching rooms is one click, and until you pick one the right side reads <em>Select a conversation</em>.</p>
          ...
          <p><strong>Keeping chat comfortable.</strong> Press and hold any message (or right-click on a computer) for a quick menu. From there you can <strong>Report to organizers</strong> if something&rsquo;s out of line — it goes privately to the tournament&rsquo;s organizers to review, and the sender isn&rsquo;t told who reported it — or <strong>Mute this room</strong> to silence its notifications and clear its unread badge when you need quiet. Muting is just for you: you can still open the room and post, and you can unmute the same way any time.</p>
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>Your <strong>Overview</strong> answers one question at a time:</p>
          <ul>
            <li><strong>One thing at the top</strong> — a single card for whatever matters most right now, and never two competing ones. On a <strong>game day</strong> it&apos;s the game: opponent, time, and where to be — the <strong>venue and the diamond</strong> (or rink, court or pitch), not just a number. Under that sits a row of <strong>chips</strong> for how ready you are: who&apos;s coming, whether the lineup is set, and your <strong>call time</strong> and <strong>uniform</strong> once you&apos;ve set them. <strong>Green means done, amber means still to do</strong>, and each chip opens the screen it&apos;s about. The button beside the game is always <strong>the first thing you haven&apos;t done</strong> — build the lineup, then take attendance — so it never asks for work you&apos;ve already finished. Once nothing is outstanding it stops asking: close to first pitch it becomes <strong>Open game day</strong>, and earlier in the day there&apos;s simply no button, because there&apos;s nothing left to do. (The score joins the card once you enter it.) Earlier in the week it&apos;s your <strong>next game</strong>. When the season has gone quiet it&apos;s a <strong>Season check</strong> asking whether you&apos;re done — with <strong>Add an event instead</strong> right beside it, so it never assumes. And in your first week it&apos;s simply your <strong>next setup step</strong>. If none of those apply, the card steps aside and your numbers lead.</li>
            <li><strong>Your team at a glance</strong> — six tiles, directly under that card rather than at the bottom of the page: your <strong>record</strong> (with recent form), <strong>roster</strong>, what&apos;s <strong>next up</strong> (or, when the card above is already about your next game, what&apos;s on <strong>this week</strong>, including player <strong>birthdays</strong>), <strong>tournaments</strong> (plus any <strong>entry fees due</strong>), and two money tiles — <strong>dues</strong> and <strong>budget</strong>. They keep the same places all season, so you learn where to look. Tiles also carry what needs attention: players <strong>missing a guardian email</strong>, a <strong>Lineup set / not set</strong> nudge, and who&apos;s <strong>overdue</strong> on dues.</li>
            <li><strong>Side panels on two other screens</strong> — on a wide screen only, a quiet panel sits beside the work and shows what would otherwise be off-screen: on the <strong>lineup builder</strong>, who&rsquo;s coming (with anyone who&rsquo;s <em>out</em> named, not just counted), so you don&rsquo;t scroll up and back down for every player you place; and on <strong>Player Dues</strong>, your <strong>collected</strong>, <strong>outstanding</strong> and <strong>next due</strong> totals beside the rows instead of scrolling away above them. They&rsquo;re for glancing at, so they carry no buttons, and a group disappears when there&rsquo;s nothing in it. The Overview has no such panel on purpose — its six tiles already answer those questions.</li>
            <li><strong>A quiet list at the bottom</strong> — anything that isn&apos;t today&apos;s work: your tournament (labelled <strong>Live now</strong>, <strong>Upcoming</strong> or <strong>Finished</strong>) with its <strong>⇄ Fan view</strong> door, and — once you&apos;ve wrapped a season — a <strong>Last season</strong> line (record, dues collected, spent) that opens <strong>Insights</strong>.</li>
          </ul>
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>In season, three cards read your books at a glance, each fact in exactly one place. <strong>Collections</strong> shows what&apos;s come in of what&apos;s expected on a progress bar, with a chip that flags anyone <strong>overdue</strong> or unpaid (the card edge turns red while anything is overdue). <strong>Cash on hand</strong> leads with received minus paid, with small <strong>In</strong> and <strong>Out</strong> bars you can compare at a glance — and says it plainly right on the card: these are <strong>cash received and cash paid, not what&apos;s still owed</strong>. An unpaid bill or instalment doesn&apos;t move them until it&apos;s marked paid — which is why they always agree with the Budget vs. Actual report — and the card links straight to <strong>what&apos;s outstanding</strong> on Player Dues. <strong>Budget</strong> shows your headroom against the plan with spending on a bar. Below them, <strong>Next 30 days</strong> lists everything coming due — player dues, team payables, and org payments — as <strong>one date-ordered list</strong> (switchable to 60 or 90 days): installments that share a date and amount collapse into a single line like <em>&ldquo;Installment #2 — 12 players&rdquo;</em>, and an overdue line carries a <strong>Remind</strong> shortcut that jumps into Player Dues — the screen where reminders are actually sent. The dashboard deliberately has no big action buttons — it reports; doing something is always one tap away on the screen that owns it.</p>
          <p>Under the guide card, <strong>Everything in Money</strong> lists all seven screens with a live figure beside each — grouped in the treasurer&apos;s order: <strong>Plan</strong> (Season Budget Plan) → <strong>Collect</strong> (Player Dues, Fundraisers) → <strong>Spend</strong> (Expenses &amp; Payables — plus Org Allocations and Payment Requests when a club or league runs your team) → <strong>Review</strong> (Budget vs. Actual). So working through the list is working through your season in the order it happens &mdash; on a phone or a narrow window it runs as one column top to bottom, and on a wide screen it folds into two side by side, Plan and Collect on the left, Spend and Review on the right, so the whole list fits without scrolling. In season the same list appears under the dashboard as <strong>More in Money</strong>, carrying whatever the three cards above it don&apos;t already show. Either way, every screen is also one tap away in the <strong>tab bar</strong> across the top — hopping between Money screens never means leaving the page. On a club-run team, <strong>Org Allocations</strong> and <strong>Payment Requests</strong> also link to each other — what the club has billed you on one, and how you pay it back or claim a reimbursement on the other.</p>
          <p><strong>All of it works on a phone.</strong> Lists of records — expenses, payables, allocation instalments, fundraiser results — read as one card per item with a label on every line, so nothing is cut off at the edge of the screen. <strong>Budget vs. Actual</strong> is the exception: comparing <em>Budgeted</em>, <em>Actual</em> and <em>Variance</em> side by side is the whole point of that report, so instead of stacking, the table itself <strong>swipes sideways</strong> — the line-item name stays put on the left while the money columns move, and a small <em>&ldquo;swipe the table&rdquo;</em> cue appears whenever there&apos;s more to see. The page itself never slides around; only the table does.</p>
          <p><strong>Starting from zero.</strong> An empty <strong>Season Budget Plan</strong> opens with a guided starter — five tap questions about your season (tournaments, travel, officials, training, uniforms) build a worksheet where you price only the lines you know; nothing is guessed for you, so a blank amount simply moves to a <strong>&ldquo;Not in your plan yet&rdquo;</strong> checklist instead. That checklist keeps sitting quietly on your budget page afterward — tap <strong>+</strong> on an item once you know its cost, or <strong>&times;</strong> to hide one your team doesn&apos;t pay for (remembered on that device). Not ready to commit? A clearly-labelled <strong>sample budget</strong> for a made-up team — with its own budget and Budget vs. Actual tabs — is one tap away from either empty page, or from the quiet <strong>&ldquo;See a sample budget&rdquo;</strong> link once you have one: its numbers are invented, so there&apos;s nothing in it to copy.</p>
          <p><strong>Budget your way.</strong> The top of the plan reads as one short calculation rather than a row of figures: your <strong>line items</strong>, then what players end up paying. Pick categories and items from the library, or create your own <strong>custom items and categories</strong> right in the picker (they&apos;re saved for reuse).</p>
          <p><strong>Planning to a number before you know every cost.</strong> Set an optional <strong>Estimated total</strong> and the difference between it and your line items appears as its own row — <em>&ldquo;not itemized yet&rdquo;</em> — shrinking on its own as you add real lines, so the total you set holds. Set one <em>below</em> what you&apos;ve already itemized and that row simply turns red and says you&apos;re over: nothing is blocked, and nothing you typed is quietly ignored. Whenever an estimate is set, it&apos;s the number per-player dues are worked out from. <strong>Clear</strong> takes it away again and your line items become the total.</p>
          <p><strong>Budgeting money coming in.</strong> A budget line can be a <strong>cost</strong> or <strong>expected funding</strong> — fundraising, a sponsor, a club grant. Expected funding is subtracted from what the season costs, so the plan tells you what players actually have to fund and dues are generated from that: budget $4,000 of fundraising against an $8,000 season and per player drops from $800 to $400. Enter what you expect the <strong>team</strong> to keep — if a fundraiser pays part of what a player raises back to that player, it already lowers their own dues and shouldn&apos;t be counted twice. In <strong>Budget vs. Actual</strong>, expected funding is measured against your team&apos;s share of what was really raised.</p>
          ...
          <p><strong>Spreading a cost across the season.</strong> Tick <strong>Split by period</strong> on any budget line and it asks one thing first: <em>how is this line split?</em> — by <strong>Months</strong>, by <strong>Quarters</strong>, by <strong>Specific dates</strong>, or by <strong>Just names</strong>. A tournament entry fee usually wants exact dates; an annual cost like ice or field time is far easier by month. Then add periods one at a time, each carrying its own picker. <strong>&ldquo;+ Add period&rdquo; moves the month along by itself</strong> — Feb, Mar, Apr — so a twelve-month budget is twelve taps and then <strong>Split evenly</strong>, with nothing to type; <strong>Fill the season</strong> does the same in one tap, and <strong>Clear all periods</strong> starts over. Amounts go in as dollars or percentages. Switching the split afterwards — months to quarters, say — <strong>clears the periods and starts over</strong>, because it&apos;s a fresh decision about how the line works rather than another way of viewing the same one. Nothing goes quietly: that, <strong>Fill the season</strong> and <strong>Clear all periods</strong> each leave an <strong>Undo</strong> beside them, which puts the periods — and the split you were using — straight back.</p>
          ...
          <p><strong>Your budget by month.</strong> <strong>Budget vs. Actual</strong> has two views, and it remembers which one you prefer. <strong>Categories</strong> is the report you already know. <strong>Months</strong> lays the same season out the way a spreadsheet does — your budget lines down the side, the season&apos;s months across the top, totals both ways. One toggle changes what every cell shows: <strong>Budget</strong> (what you planned for that month), <strong>Scheduled</strong> (what you&apos;ve actually committed to pay, by its due date), <strong>Actual</strong> (what&apos;s been paid), and <strong>Difference</strong> (your plan minus what you paid). Difference only fills in months that have already happened — a month still ahead shows a dash, because money nobody has spent yet isn&apos;t a saving. Budget and Scheduled stay separate on purpose, so a commitment never quietly becomes part of your estimate and nothing gets counted twice. Anything in your plan without a payment date sits in its own <strong>&ldquo;No date yet&rdquo;</strong> column rather than being spread across months you never chose.</p>
          <p><strong>Will we run short?</strong> Under the month grid, three rows answer it: <strong>money in</strong>, <strong>money out</strong>, and a <strong>running balance</strong>. If the balance dips below zero, a line underneath names the month and the amount in plain words. It projects using whichever view you&apos;re reading — Budget projects your plan, Scheduled projects your commitments — and says so, so it&apos;s always clear which one you&apos;re looking at. Money in counts <em>player dues</em>; fundraiser rebates already credit dues, so counting both would count the same dollar twice. Teams in their second season or later also get a <strong>last season</strong> column, plus a short list of lines you had last year and haven&apos;t planned this year — often the most useful part. <strong>Tapping a figure takes you to the tool that owns it:</strong> a budget cell opens that line&apos;s form with its payment dates ready to edit; an Actual or Scheduled figure opens a plain list of what makes it up. The grid is a way of getting to your forms, never a second place to edit.</p>
          <p><strong>Everything you owe, in one list.</strong> The <strong>Payment schedule</strong> tab on Expenses &amp; Payables shows every commitment by due date — payable deposits and balances, and on a club-run team your organization&apos;s allocations too. Filter by <strong>Unpaid</strong>, <strong>Paid</strong>, or <strong>All</strong>; overdue rows say how many days late they are, and you can mark your own payables paid right there. It&apos;s money going <em>out</em> only — player dues are money coming in and live on Player Dues, where the reminders that chase them are. The <strong>Next 30 days</strong> list on the Money Overview is the 30/60/90-day preview of the same commitments — interleaved with dues coming in — and links straight to it.</p>
          <p><strong>A payable is any commitment, not just a tournament.</strong> A dome block, an umpire invoice, a uniform order — anything with a due date belongs here. Record one amount due on one date, or open <strong>&ldquo;Split into a deposit and a balance&rdquo;</strong> when it&apos;s billed in two parts; each half keeps its own due date and its own paid tick. In the <strong>Payables</strong> list each commitment is one row — what it&apos;s for, its category, the total, and where it stands (<em>Scheduled</em>, <em>Part paid</em>, <em>Overdue</em> or <em>Paid</em>). Tap <strong>Payment details</strong> on the row to open the deposit and balance, with their due dates and their <strong>Mark paid</strong> buttons, plus that payable&apos;s notes and tags.</p>
          <p><strong>Getting data in and out: the two buttons at the top of Money.</strong> <strong>Import</strong> and <strong>Export</strong> sit at the top of Money and stay the same on every tab, so you never have to work out which screen hides them. <strong>Import</strong> offers <em>Budget lines</em> and <em>Expenses &amp; payables</em>, plus <strong>Recent imports</strong> — what&apos;s been brought in this season, when, and by whom. <strong>Export sits on each tab</strong>, in the row of controls above the numbers — so it always gives you <strong>what you&apos;re looking at</strong>. On Budget vs. Actual that means the month grid if you&apos;re reading by month, or the category table if you&apos;re not; on Expenses &amp; Payables it follows the sub-tab and any tag filter you&apos;ve set. Every tab has one except Overview, which reports rather than holding a list of its own.</p>
          <p><strong>Whichever you pick, you&apos;re asked which file type you want</strong> — the same way any program asks when you save or print. <strong>Excel</strong> for working with the numbers, <strong>CSV</strong> for plain text that opens anywhere, and — on Player dues and Budget vs. Actual — <strong>PDF</strong> for something printable you can hand to a parent. PDF appears only on plans that include it. Exporting doesn&apos;t need edit access: an assistant who can see the numbers can take them away.</p>
          <p><strong>On a phone, spreadsheets are put away.</strong> An exported file lands in a downloads folder you&apos;d never open there, and nobody imports a season&apos;s budget standing at a field. So on a phone <strong>Import disappears</strong>, and <strong>Export appears only where it can give you something you&apos;d actually use on a phone</strong> — a printable PDF of player dues or Budget vs. Actual. <strong>You can still import where it matters:</strong> an empty budget and an empty payables list both offer it right there, at any screen size, including the paste-from-a-message path built for phones.</p>
          <p><strong>Bringing a budget in from a spreadsheet.</strong> Pick which shape your sheet is — a <strong>month grid</strong> (a row per cost, a column per month), a <strong>simple list</strong> (one amount per line), or a <strong>payables schedule</strong> (what you owe and when) — then either <strong>paste</strong> your rows straight out of Excel or Google Sheets, or <strong>upload</strong> the file. Both end up in the same place.</p>
          ...
          <p><strong>What it won&apos;t do.</strong> It reads names and numbers, and nothing else — it will never guess what an unfamiliar category means or read an ambiguous date like <em>03/04/2026</em>. Anything it can&apos;t read confidently is handed back to you rather than filed somewhere plausible. Importing payables always <em>adds</em>; two payments to the same tournament are two real commitments, so it flags a look-alike instead of overwriting one. A spreadsheet row is always a <strong>cost</strong> — a sheet has no column for money coming in — so an imported row never matches or overwrites one of your <strong>expected funding</strong> lines, even when the names look the same. Add those on the plan itself. Importing needs <strong>money edit</strong> access.</p>
          ...
          <p><strong>Nothing here loses your work.</strong> The longer Money forms — a budget line, an expense, a payable, a payment request, fundraiser settings — ask before discarding if you close them with something typed in. <strong>Keep editing</strong> puts you back exactly where you were, with every field intact; <strong>Discard</strong> throws it away. A form you haven&apos;t touched just closes, with no question asked.</p>
      ...
      faqs: [
        ...
        },
        {
          ...
          popular: true,
          answer: (
            <p><strong>Set dues for all players</strong> on Player Dues and <strong>Generate installments</strong> on the Season Budget Plan open the same window — start from whichever screen you&apos;re already on. You choose the due dates and the amount per player, <strong>preview</strong> what every player on the roster would owe, then confirm; every active player gets the same schedule. It works from what players actually <em>fund</em> — your budget less any expected funding you&apos;ve entered, and your estimated total when you&apos;ve set one — so it needs a <strong>Season Budget Plan</strong> to start from. Haven&apos;t built one yet? The window tells you so and takes you straight there. Afterwards, adjust any single player from their own row with <strong>Edit schedule</strong>.</p>
          ),
          answerText: 'Set dues for all players on Player Dues and Generate installments on the Season Budget Plan open the same window — start from whichever screen you are already on. You choose the due dates and the amount per player, preview what every player on the roster would owe, then confirm, and every active player gets the same schedule. It works from what players actually fund — your budget less any expected funding you have entered, and your estimated total when you have set one — not the gross cost of the season, so it needs a Season Budget Plan to start from. If you have not built one yet the window says so and takes you straight to the plan; it also says so plainly when your expected funding already covers the whole season, or when your estimated total is zero, so you are never left guessing why there is nothing to charge. Afterwards you can adjust any single player from their own row with Edit schedule. The Automatic Dues Reminders toggle at the bottom of Player Dues controls the 30-day and 7-day reminder emails.',
          keywords: ['generate installments', 'set dues for all players', 'dues for all players', 'installment schedule', 'same schedule every player', 'bulk dues', 'set dues at once', 'charge everyone', 'no budget yet', 'build a budget first', 'why do i need a budget to set dues', 'nothing for players to fund'],
        },
        {
          id: 'faq-money-redo-dues',
          question: 'I set the dues wrong — can I redo them?',
          answer: (
            <p>Yes. Run <strong>Set dues for all players</strong> again with the right dates and amounts. When it finds a schedule already there it <strong>asks first</strong> — <em>&ldquo;this roster already has dues&rdquo;</em> — and only replaces it once you say so, with your figures still on screen if you&apos;d rather go back. One thing it never replaces: <strong>anyone who has already paid something is left exactly as they are</strong>, so a payment you&apos;ve recorded is never thrown away along with the schedule it was made against. Afterwards it tells you how many players were left alone — fix those individually from each player&apos;s row with <strong>Edit schedule</strong>.</p>
          ),
          answerText: 'Yes. Run Set dues for all players again with the right dates and amounts. When it finds a schedule already there it asks first — this roster already has dues — and only replaces it once you say so, with your figures still on screen if you would rather go back. One thing it never replaces: anyone who has already paid something is left exactly as they are, so a payment you have recorded is never thrown away along with the schedule it was made against. Afterwards it tells you how many players were left alone, and you fix those individually from each player’s row with Edit schedule. If any player could not be saved it names them, so you never have to audit the roster to find out who. This is also the answer to wrong due dates, a wrong per-player amount, or a budget that changed after you had already sent dues out.',
          keywords: ['redo dues', 'redo the dues', 'change dues', 'change the dues', 'wrong dates', 'wrong due date', 'wrong amount', 'regenerate dues', 'regenerate installments', 'run it again', 'do it again', 'already paid', 'replace dues', 'replace the schedule', 'start dues over', 'fix dues for everyone', 'budget changed after dues', 'dues do not match my budget'],
        },
        {
          ...
          answer: (
            <>
              <p>Yes — budget it and the dues follow. On the <strong>Season Budget Plan</strong>, add a line and choose <strong>Expected funding</strong> instead of <em>A cost</em>: fundraising you expect to run, a sponsor, a club grant. It shows in its own section at the foot of the plan, subtracted from what the season costs, and the summary tells you what&apos;s left as <strong>funded by players</strong>. Budget $4,000 of fundraising against an $8,000 season and per player drops from $800 to $400 — and <strong>Generate installments</strong> starts from the lower figure, so nobody has to do the subtraction by hand.</p>
          ...
          answerText: 'Yes — budget it and the dues follow. On the Season Budget Plan, add a line and choose Expected funding instead of A cost: fundraising you expect to run, a sponsor, a club grant. It shows in its own section at the foot of the plan, subtracted from what the season costs, and the summary tells you what is left as funded by players. Budget $4,000 of fundraising against an $8,000 season and per player drops from $800 to $400 — and Generate installments starts from the lower figure, so nobody has to do the subtraction by hand. Enter what you expect the TEAM to keep. If a campaign pays part of what a player raises back to that player, that already comes off their own dues as a credit — counting it here as well would lower the same dues twice. Later, Budget vs. Actual compares what you budgeted against your team’s share of what was really raised, so did our fundraising hit the number has an answer.',
          keywords: ['fundraising lower dues', 'reduce dues', 'lower what families pay', 'expected funding', 'budget fundraising', 'sponsorship', 'sponsor', 'grant', 'money coming in', 'funded by players', 'offset dues', 'subsidize dues', 'raise money instead of charging'],
        },
        {
          ...
          answer: (
            <>
              <p>The <strong>estimated total</strong> is optional: what you think the season costs before you&apos;ve itemized it. Set one and the gap between it and your real lines shows as its own row — <em>not itemized yet</em> — which shrinks on its own as you add lines, so the total you set holds.</p>
              <p>If your line items add up to <strong>more</strong> than your estimate, that row turns red and says how far over you are. Your total then shows the estimate, because the number you set is the number that counts — including for per-player dues. Trim lines, raise the estimate, or <strong>Clear</strong> it and your line items become the total again. <strong>Budget vs. Actual</strong> says the same thing, so the two pages never disagree.</p>
            </>
          ),
        ...
        },
        {
          ...
          answer: (
            <p>Tap any tab — <strong>Budget Plan</strong>, <strong>Player Dues</strong>, <strong>Fundraisers</strong>, <strong>Expenses &amp; Payables</strong>, and the rest — in the tab bar across the top of <strong>Money</strong>, or any link on the <strong>Overview</strong> tab: every card&apos;s footer links and every row in the More in Money list lead to the screen that owns the number. Either way you switch in place; the tab bar stays put, so getting from one Money screen to another is one tap, not a trip back to Money first.</p>
          ),
          answerText: 'Tap any tab — Budget Plan, Player Dues, Fundraisers, Expenses and Payables, and the rest — in the tab bar across the top of Money, or any link on the Overview tab: every card’s footer links and every row in the More in Money list lead to the screen that owns the number. Either way you switch in place; the tab bar stays put, so getting from one Money screen to another is one tap, not a trip back to Money first.',
      ...
      content: (
        <>
          ...
          <ul>
            ...
            <li><strong>Report doorways</strong> — question-titled tiles (<em>&ldquo;Where is playing time going?&rdquo;</em>, <em>&ldquo;Who shows up?&rdquo;</em>) that open the full report.</li>
          </ul>

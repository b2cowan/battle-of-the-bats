...
const coachesHelp: HelpPageContent = {
  ...
  sections: [
    ...
    },
    {
      ...
      summary: 'Money opens with your next step while you set up, then becomes a one-screen season dashboard — collections, cash on hand, budget, and the next 30 days of money. A Record button on every tab writes down money that moved without picking a tab first. All of it reads on a phone.',
        ...
        },
        {
          ...
          content: (
            <>
              ...
              <p><strong>Your season&apos;s dues, at the top of the screen.</strong> Player Dues opens with four figures: <strong>Dues</strong> (and how many players), <strong>Collected</strong> — cash, with any fundraising credits named underneath rather than added in — <strong>Balance owing</strong>, and <strong>Past due</strong> with the number of families behind. <strong>Past due only appears when there is some</strong>; a season where nobody is behind shows three figures instead of a zero under a red word. They sit above the list, so they read the same whichever view you are in and you don&apos;t scroll a roster to reach them. Nothing shows at all until at least one player has a dues schedule.</p>
              ...
              <p><strong>Collected counts what settled a bill</strong>, so it can never run past <strong>Dues</strong>. Where families have sent more than they were billed, <strong>Collected</strong> says so underneath — <em>&ldquo;$850.00 more sent than billed, $300.00 of it handed back&rdquo;</em> — which is why adding up the <strong>Paid</strong> column comes to more than the tile. The column answers <em>what did this family send</em>; the tile answers <em>how much of what we billed is settled</em>. Two different questions, so they are two different words.</p>
              <p><strong>The last row of the table adds the columns up</strong>, and it is there so you never have to take those sentences on faith. It reads across exactly like a family&apos;s row does — <strong>Dues − Fundraising − Other credits − Paid + Handed back = Balance</strong> — for the whole list at once, and the count on the left says whose money it covers. Narrow <strong>Showing</strong> and the total follows the list rather than the roster, so it always describes what you are looking at. Every gap between it and the figures at the top is explained by a figure already on the screen: <strong>Paid</strong> runs above <strong>Collected</strong> by the money families sent over their bills, and the total <strong>Balance</strong> sits below <strong>Balance owing</strong> by whatever is owed back to families in credit. The same row lands at the foot of the export.</p>

  ...
  sections: [
        ...
        },
        {
          ...
          content: (
            <>
              ...
              <p><strong>Then run it.</strong> Once the first block is written, <strong>Save as template…</strong> and <strong>Print the sheet</strong> appear above the sheet — a blank plan shows neither. On the day — three hours before the start to three hours after the end — a green <strong>Run practice</strong> joins them, first in the row: the same door the <strong>Practice plans</strong> card, your Overview and the practice&rsquo;s Schedule panel offer, and none of them for a practice three weeks out. While it runs, the block that should be on right now says <strong>now</strong> in its time gutter, with the clock — a tablet on the bench reads the sheet while your phone runs the field screen. See <em>Running a practice at the field</em> below.</p>
    ...
    },
    {
      ...
      keywords: ['run practice', 'run it', 'field screen', 'at the field', 'practice timer', 'timer', 'countdown', 'clock', 'rotate now', 'next block', 'my station', 'station view', 'running a station', 'who is here tonight', 'started late', 'running late', 'overrun', 'amber', 'read in the sun', 'one block at a time',
        ...
        // Stage 5 — the fact outside the window, the one list, the words for everyone.
        'planned for', 'the clock starts', 'no countdown', 'whole team on the field', 'whole team', 'sitting out', 'sits round out', 'nobody at a station', 'one row per station', 'who is coming to my station', 'just for tonight', 'run practice on the plan page', 'when is run practice offered'],
      searchText: 'run practice run it at the field field screen one block at a time phone in the sun gloves one hand arm length big timer countdown clock counts down remaining time left of overrun amber over by next block back button rotate now rotation due round 2 of 3 which group is at which station carousel no alarm no sound no beep no vibration no buzz no notification does not advance by itself never moves on its own manual tap my station station picker what am i doing what am i watching for coaching points setup just for tonight coming to you who is here tonight attendance planned for tue sep 15 5:01 p.m. the clock starts 2:21 p.m. outside the window no clock no counter never says ran one list keyed by station a row per station the group letter the station who runs it view open the row opens the station nobody two letters when two groups share d sits round 3 out amber head move the groups on coaches stay where they are each row shows the letter that arrives whole team under the words uat coach whole team chips for a few who runs this block folded collapsed running late started late behind schedule nothing is recorded no ticks read only planned not done assistant tee station schedule access',
      content: (
        <>
          <p><strong>Run practice</strong> opens the field screen — one block filling the phone, big enough to read at arm&rsquo;s length in the sun. The block, a countdown, the words, who&rsquo;s running it and who&rsquo;s in it (<strong>Whole team</strong>, or the few you named), and one quiet line at the foot telling you what&rsquo;s next. Two buttons: a big <strong>Next block</strong> and a smaller <strong>Back</strong>. There&rsquo;s nothing to swipe or drag, because gloves defeat all of that.</p>
          <p>It&rsquo;s offered <strong>on the day</strong> — from three hours before the start until three hours after the end — in four places that all follow the same rule: the <strong>Practice plans</strong> card, your Overview, the practice&rsquo;s panel on the <strong>Schedule</strong>, and the plan page itself, where it&rsquo;s first in the row above the sheet. Opening it mid-practice lands you on whichever block should be running right now, so you don&rsquo;t tap through from the start. Open it any other time and there&rsquo;s no countdown to mislead you: where the clock would be it says <strong>Planned for</strong> and the date and time — before a practice, when the clock starts — and you can still page through the blocks. It never says a practice <em>ran</em>; nothing here records that.</p>
        </>
      ),
      subtopics: [
        {
          ...
          content: (
            <>
              <p><strong>In a rotation the same screen shows one list, a row per station</strong> — the group letter that&rsquo;s there now on the left (two letters when two groups share a station; <em>nobody</em> when the round leaves one empty), the station&rsquo;s name, who&rsquo;s running it, and <strong>View</strong>. Tap the row to open that station; yours says <em>that&rsquo;s you</em>. A group sitting a round out is one line under the list. When the round runs over, the head goes amber (<em>Move the groups on · Coaches stay where they are</em>) and each row shows the letter that <strong>arrives</strong> next — read down the letters and everyone knows where to go. The button reads <strong>Rotate now</strong>, and once there&rsquo;s no round left it goes back to <strong>Next block</strong>. A rotation isn&rsquo;t a mode you have to get out of; it&rsquo;s just part of the same run.</p>
      ...
      faqs: [
        {
          ...
          answer: (
            <>
              ...
              <p>The screen moves only when you tap <strong>Next block</strong> or <strong>Rotate now</strong>. And the clock only runs <strong>on the day</strong> — three hours before the start to three hours after the end; open the field screen any other time and it says <strong>Planned for</strong> and the date instead of counting.</p>
            </>
          ),
        },
        {
        ...
        },
        {
          ...
          answer: (
            <>
              <p>Yes. On the field screen, tap your station&rsquo;s row and you get <strong>My station</strong>: the group with you right now and how long until they move, what you&rsquo;re doing, <strong>what you&rsquo;re watching for</strong>, the coaching points, the setup, any note left for tonight, and <strong>who&rsquo;s coming to you next</strong>.</p>

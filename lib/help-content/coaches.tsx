    ...
    },
    {
      ...
      faqs: [
        ...
        },
        {
          ...
          popular: true,
          answerText: 'Open any player from your Roster and find the Development section. Focus areas are the things this player is working on, in plain language ("first-step quickness off the bag") with one short note and a simple status you tap to change — Working on it, Achieved, or Parked. No grades or percentages, on purpose. Measurables are the tests you already run at practice — a 60-yd sprint, home-to-first, throw velocity. Set up each test once (name + unit) and it becomes a chip you pick when logging; a reading is just the value and a date (defaults to today). A small trend line appears once a player has two readings of the same test — one reading honestly says a trend will show after a second entry. If you ever change a test\x27s unit (mph to km/h), the line is drawn from readings in the current unit only and the row says "Units changed — earlier readings are listed but not drawn on this line"; every reading stays in the list with the unit it was recorded in, and two units are never joined into one line. Every number compares the player only to themselves — there is no team leaderboard. A focus area can carry an optional focus tag — one of your team\'s focus words, the same ones your drills use — so a practice plan can tell which areas it is about. Below, a Context list quotes the player’s depth-chart spots, this season’s field/bench innings, and attendance so the whole picture is on one screen. At the bottom, Print summary (PDF) builds a one-page handout — this season’s focus areas and dated readings with your organization’s header — made to hand a family at pickup; it never includes comparisons to other players and there is no share link. The head coach, and any assistant with the Development switch on, can add or edit development data (focus areas also need Internal notes); other assistants see focus areas only with notes access, and measurables with any of the team duties. To define, rename or retire a test (retiring keeps its logged history), follow the Metrics link at the top of the section — it opens the Metrics view of Skills & Goals, where every definition lives. If the player came through your tryout and you ran Start development from tryouts, a Tryout snapshot card sits above their focus areas showing where their season started — it is context only, never a measurable and never part of a trend, and it is coach-eyes-only.',
          keywords: ['development', 'player development', 'focus areas', 'focus area', 'idp', 'measurables', 'measurable', 'log a measurable', '60 yard', 'sprint time', 'test times', 'trend', 'sparkline', 'units changed', 'changed the unit', 'not drawn on this line', 'working on it', 'achieved', 'parked', 'test types', 'retire test', 'player progress', 'track progress', 'print summary', 'print', 'pdf', 'handout', 'parent summary', 'family', 'tryout snapshot', 'development baseline', 'where the season started'],
          answer: (
            <>
              ...
              <p><strong>Measurables</strong> are the tests you already run at practice — a 60-yd sprint, home-to-first, throw velocity. Set up each test once (name + unit) and it becomes a chip you pick when logging; a reading is just the value and a date (defaults to today). A small <strong>trend line</strong> appears once a player has <strong>two readings</strong> of the same test — one reading honestly says a trend will show after a second entry. If you ever change a test&apos;s unit (mph to km/h, say), the line is drawn from readings in the <strong>current unit only</strong> and the row says so — <em>Units changed — 2 earlier readings in mph are listed but not drawn on this line</em>. Every reading stays in the list with the unit it was recorded in; two units are never joined into one line. Every number compares the player <em>only to themselves</em> — there&apos;s no team leaderboard.</p>
              <p>Below, a <strong>Context</strong> list quotes the player&apos;s depth-chart spots, this season&apos;s field/bench innings, and attendance, so the whole picture is on one screen. At the bottom, <strong>Print summary (PDF)</strong> builds a one-page handout — this season&apos;s focus areas and dated readings under your organization&apos;s header — made to hand a family at pickup. It never includes comparisons to other players, and there&apos;s no share link. The <strong>head coach</strong>, and any assistant with the <strong>Development</strong> switch on, can add or edit development data (focus areas also need Internal notes); other assistants see focus areas only with <strong>notes</strong> access, and measurables with any of the team duties. To define, rename or retire a test (retiring keeps its logged history), follow the <strong>Metrics</strong> link at the top of the section — it opens the Metrics view of Skills &amp; Goals, where every definition lives. A focus area can carry an optional <strong>focus tag</strong> — one of your team&apos;s focus words, the same ones your drills use — so a practice plan can tell which areas it is about.</p>
        ...
        },
        {
          ...
          answerText: 'Open Skills & Goals in the Progress menu and tap + Start session in the page header (it was New session before September 2026). Pick which test you are running (or set one up on the spot), then work straight down your roster — tap a player, type their number, and it saves as you go, with an "N of M entered" count so you always know where you are. Leave a player blank to skip them (an absent player is never given a fake 0). Rows always stay in roster order — testing day is not a leaderboard. Every reading lands on that player\'s profile exactly as if you had logged it there, tagged with the session date. The session itself is saved ("Jul 17 — 14 players, 2 tests") so you can come back to it, and deleting a session never deletes the readings — they just lose the grouping. An old session shows everything saved in it: it opens on the first test it already has results for; a test you have since retired still appears as a chip marked "retired", read-only; a player who has since left the team keeps their row below the roster, marked "no longer on the roster", read-only; the "N of M entered" count is the current roster only. Skills & Goals has three views across the top: Sessions (this list), Players (everyone in roster order — pick Current focus, or one test to see its latest result and date per player; the Team board page used to do this) and Metrics (your tests and skills, where each is defined). If Start session is switched off, no measured test is defined yet. Two or three sessions a season is what makes each player\'s trend lines real.',
          keywords: ['evaluation session', 'testing day', 'test the whole team', 'whole roster', 'batch entry', 'development page', 'team board', 'new session', 'run tests', 'timing day', 'combine', 'your test list', 'retired test in a session', 'no longer on the roster', 'player left the team', 'old session', 'session opens on'],
          popular: true,
          answer: (
            <>
              <p>Open <strong>Skills &amp; Goals</strong> in the Progress menu and tap <strong>+ Start session</strong> in the page header. Pick which test you&apos;re running (or set one up on the spot), then work straight down your roster — tap a player, type their number, and it <strong>saves as you go</strong>, with an &ldquo;N of M entered&rdquo; count so you always know where you are. <strong>Leave a player blank to skip them</strong> — an absent player is never given a fake 0. Rows always stay in <strong>roster order</strong> — testing day isn&apos;t a leaderboard.</p>
              ...
              <p>An old session shows <strong>everything saved in it</strong>. It opens on the first test it already has results for. A test you have since retired still appears as a chip marked <strong>· retired</strong>, read-only; a player who has since left the team keeps their row below the roster, marked <strong>no longer on the roster</strong>, read-only. The &ldquo;N of M entered&rdquo; count is the current roster only, so a departed player&apos;s reading never makes it &ldquo;15 of 14&rdquo;.</p>
              <p>Skills &amp; Goals has three views across the top: <strong>Sessions</strong> (this list), <strong>Players</strong> (everyone in roster order — pick <em>Current focus</em>, or one test to see its latest result and date per player; the Team board page used to do this) and <strong>Metrics</strong> (your tests and skills, where each is defined). If <strong>Start session</strong> is switched off, no measured test is defined yet. Two or three sessions a season is what makes each player&apos;s trend lines real.</p>
            </>
          ),
        },
        {
    ...
    },
    {
      ...
      subtopics: [
          ...
          content: (
            <>
              ...
              <HelpDefs>
                ...
                <HelpDef term="Players">Every player in roster order. <strong>Show</strong> picks what the row says: <em>Current focus</em>, or one test with <em>that</em> test&rsquo;s latest result and the date it was recorded — never one &ldquo;last eval&rdquo; for everything. A player with nothing under that test reads <em>not recorded</em>. Tap a name to open their record on the right row; the back arrow brings you straight back here.</HelpDef>
          ...
          content: (
            <>
              ...
              <HelpDefs>
                ...
                <HelpDef term="Observed skill">Something you watch for. Give it a name and, if you like, <strong>descriptors</strong> — your own words for what you saw, one per line, in your order. Never a scale, never averaged. You can define a skill now; recording an observation against it comes in a later release.</HelpDef>
              </HelpDefs>
        ...
        },
        {
          ...
          content: (
            <>
              <p>Every coach with a team duty can read Skills &amp; Goals; focus areas show only with Internal notes. <strong>Writing</strong> follows one switch on the staff sheet: <strong>Development</strong> — <em>&ldquo;Define tests, run sessions and record results. With Internal notes, write goals too.&rdquo;</em></p>
              <HelpDefs>
                <HelpDef term="Head coach">Always has it.</HelpDef>
                <HelpDef term="Assistant with it on">Defines and retires metrics, starts sessions, records results — and, if they also hold Internal notes, adds and edits focus areas.</HelpDef>
      ...
      faqs: [
        {
          ...
          answer: (
            <>
              <p>Because there&rsquo;s no test for a session to <em>record</em>. A session runs your tests across the roster, so it needs at least one <strong>active measured test</strong> to exist — an observed skill isn&rsquo;t enough yet, since nothing can be recorded against a skill until a later release.</p>

    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>While you&apos;re setting up, the sections under the guide card follow the treasurer&apos;s workflow: <strong>Plan</strong> (Season Budget Plan) → <strong>Collect</strong> (Player Dues, Fundraisers) → <strong>Spend</strong> (Expenses &amp; Payables — plus Org Allocations and Payment Requests when a club or league runs your team) → <strong>Review</strong> (Budget vs. Actual). In season, the same places are one tap away in the <strong>tab bar</strong> — which sits across the top the whole time, so hopping between screens never means leaving the page — and in the <strong>More in Money</strong> list on the dashboard, where each screen shows its live figure. On a club-run team, <strong>Org Allocations</strong> and <strong>Payment Requests</strong> also link to each other — what the club has billed you on one, and how you pay it back or claim a reimbursement on the other.</p>

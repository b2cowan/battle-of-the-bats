    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p><strong>Three figures tell the whole plan.</strong> The card at the top of the plan shows <strong>Planned costs</strong>, <strong>Expected fundraising</strong>, and <strong>Player installments</strong>. The third is the season&apos;s story: before dues exist it&apos;s <strong>estimated</strong> for you (costs less fundraising, with the per-player figure beside it and Generate installments one tap away); the moment dues go out it becomes the <strong>official scheduled figure</strong>. Schedule more than the plan needs — most teams do — and the card notes it plainly: <em>&ldquo;includes a $200.00 buffer above the plan.&rdquo;</em> A planned buffer is confirmation, not a warning, so nothing turns red. The one caution is scheduling <em>less</em> than the plan needs, which shows in amber with the re-run door beside it. Pick categories and items from the library, or create your own <strong>custom items and categories</strong> right in the picker (they&apos;re saved for reuse).</p>
          <p><strong>Planning to a number before you know every cost.</strong> Set an optional <strong>Estimated total</strong> and it becomes the Planned costs figure — the number you set is the number that counts, including for player installments — with a caption underneath tracking what you&apos;ve itemized so far. Itemize <em>past</em> your estimate and that caption turns red and says how far over: nothing is blocked, and nothing you typed is quietly ignored. <strong>Clear</strong> (inside the editor) takes it away again and your line items become the total.</p>
          <p><strong>Budgeting money coming in.</strong> A budget line can be a <strong>cost</strong> or <strong>expected fundraising</strong> — a campaign, a sponsor, a club grant. Expected fundraising is subtracted from what the season costs, so the plan tells you what players actually have to fund and dues are generated from that: budget $4,000 of fundraising against an $8,000 season and per player drops from $800 to $400. Enter what you expect the <strong>team</strong> to keep — if a fundraiser pays part of what a player raises back to that player, it already lowers their own dues and shouldn&apos;t be counted twice. In <strong>Budget vs. Actual</strong>, expected fundraising is measured against your team&apos;s share of what was really raised.</p>
          ...
          <p><strong>Recording the money families send.</strong> Open a player on <strong>Player Dues</strong> and tap <strong>Record payment</strong> — how much, the day it arrived (it starts on today; the team&apos;s books use <em>this</em> date, so catching up on a month of e-transfers keeps each one in its real month), how it arrived (e-transfer, cash, cheque, other) and an optional note. <strong>Any amount works</strong>: a family paying $100 a month against $300 installments is recorded exactly as it happens — money fills the oldest installment first, a part-covered one reads <em>&ldquo;$200.00 of $300.00&rdquo;</em> instead of Unpaid, the table&apos;s <strong>Paid</strong> column includes every dollar received, and their status shows <strong>Partial</strong>. <strong>Mark Paid</strong> is still there as the one-tap shortcut — it records whatever is still owing on that installment (on a part-paid one it says <strong>Mark rest paid</strong>), so it can never charge a family twice. Every payment sits in the player&apos;s <strong>Payments</strong> list as its own receipt; remove one and its entry in the books is voided and the schedule rolls back — that&apos;s the undo. Pay <em>more</em> than what&apos;s left on the schedule and the extra is saved automatically as an <strong>Overpayment</strong> credit (marked <em>auto</em> in Credits — it goes away with its payment, not on its own). Reminder emails follow the same truth: they ask for <strong>what&apos;s left</strong> on an installment and thank the family for what&apos;s already arrived, and a family with any recorded payment is never told they&apos;ve &ldquo;paid nothing.&rdquo; The totals row under the dues table closes with <strong>Balance owing</strong> — what&apos;s left after payments <em>and</em> credits.</p>
          <p><strong>Your season&apos;s dues, totalled where you read them.</strong> The last row of the <strong>Player Dues</strong> table is the season&apos;s own: <strong>Assessed</strong>, <strong>Credits</strong>, <strong>Collected</strong>, <strong>Balance owing</strong> and the <strong>next due date</strong> — each figure sitting directly under the column it totals, so none of them needs a second label. If anyone is past their due date, the count sits under that date rather than in a corner of its own. The row appears only once at least one player has a dues schedule: a team that hasn&apos;t set dues yet gets no row instead of a line of zeros. On a phone, where the table reads as one card per player, it becomes the last card in the list.</p>
          ...
          <p><strong>What it won&apos;t do.</strong> It reads names and numbers, and nothing else — it will never guess what an unfamiliar category means or read an ambiguous date like <em>03/04/2026</em>. Anything it can&apos;t read confidently is handed back to you rather than filed somewhere plausible. Importing payables always <em>adds</em>; two payments to the same tournament are two real commitments, so it flags a look-alike instead of overwriting one. A spreadsheet row is always a <strong>cost</strong> — a sheet has no column for money coming in — so an imported row never matches or overwrites one of your <strong>expected fundraising</strong> lines, even when the names look the same. Add those on the plan itself. Importing needs <strong>money edit</strong> access.</p>

    ...
    },
    {
      ...
      faqs: [
          ...
          answer: (
            <>
              ...
              <HelpDefs>
                ...
                <HelpDef term="Observations">What you saw against a skill — a descriptor and/or a sentence, dated, who wrote it, and <em>Evidence for</em> the goal it supports. <strong>Record an observation</strong> lives here and on a goal.</HelpDef>
        ...
        },
        {
          ...
          popular: true,
          answerText: 'Open Skills & Goals in the Progress menu and tap + Start session in the page header (it was New session before September 2026). It asks for the session\'s scope first: the date, the practice it was taken at (optional — it fills the date in, never owns it), which metrics you are recording (your tests and your skills) and who is here — everyone active is ticked, untick anyone absent. Then the grid opens. Pick a metric across the top and work straight down your roster: a test gives one field per attempt, as many as its definition says; Enter moves to the next attempt, then the next player; the headline (best, average, last or attempts in range) updates as you type; a blank attempt was not run — never a zero. Every row shows its state — Saved, Saving, Not saved — retry, Not recorded or Not assessed — with Edit and Retry on the row; a value that failed to save stays on screen beside its error until you retry. Mark not assessed is a link on blank rows only — a neutral state with an optional reason, never a value. Editing a saved attempt keeps the original on the record ("corrected — was 8.51"). Every saved row says who entered it. A skill is a chip too: pick a descriptor and/or say what you saw, dated by the session — it needs the Development switch and Internal notes to record and Internal notes to read. The counts under the grid run against the scope — "4 recorded · 1 not assessed · 1 not recorded — of 6 in scope"; a session started before scopes existed counts against the active roster as before ("N of M entered"). Change scope on the session; a retired test or a player who has left the team can never be added to a scope. Review session reads the counts back for every metric, never fills anything in and never marks the session complete; Back to Sessions takes you out. Rows always stay in roster order — testing day is not a leaderboard. Every result lands on that player\'s record exactly as if you had recorded it there, tagged with the session date; deleting a session never deletes the results. An old session shows everything saved in it: a retired test still appears as a chip marked retired, read-only; a player who has since left keeps their row, marked no longer on the roster. Skills & Goals has four tabs across the top: Overview (where you land — the getting-started card on a new team, the season\'s dashboard once sessions exist), Sessions (this list), Players (everyone in roster order — pick Current focus, one test to see the headline of its latest session per player, or a skill for the latest observation; the Team board page used to do this) and Metrics (your tests and skills, where each is defined). If there is no Start session button, no test is defined yet — the Overview says so.',
          keywords: ['evaluation session', 'testing day', 'test the whole team', 'whole roster', 'batch entry', 'development page', 'team board', 'new session', 'start session', 'scope', 'change scope', 'who is here', 'what will you record', 'attempts', 'one field per attempt', 'enter moves', 'headline', 'best of', 'attempts in range', 'not recorded', 'not assessed', 'mark not assessed', 'not saved', 'retry', 'saved', 'saving', 'edit a reading', 'correction', 'corrected', 'entered by', 'review session', 'back to sessions', 'skill chip', 'observation in a session', 'run tests', 'timing day', 'combine', 'your test list', 'retired test in a session', 'no longer on the roster', 'player left the team', 'old session', 'session opens on', 'skills and goals', 'skills & goals', 'n of m entered', 'of 6 in scope'],
          answer: (
            <>
              <p>Open <strong>Skills &amp; Goals</strong> in the Progress menu and tap <strong>+ Start session</strong>. It asks for the session&rsquo;s <strong>scope</strong> first — the date, the practice it was taken at (optional; it fills the date in, never owns it), which metrics you&rsquo;re recording (tests <em>and</em> skills) and who is here (everyone active is ticked; untick anyone absent). Then the grid opens.</p>
              <ol>
                <li>Pick a metric across the top and work straight down your roster — it&rsquo;s always in roster order.</li>
                <li>A test gives <strong>one field per attempt</strong>, as many as its definition says. <strong>Enter</strong> moves to the next attempt, then the next player. The headline updates as you type. A blank attempt was not run — never a zero.</li>
                <li>Every row shows its state — <strong>Saved · Saving · Not saved — retry · Not recorded · Not assessed</strong> — with <strong>Edit</strong> and <strong>Retry</strong> on the row. A value that failed to save stays on screen until you retry. <strong>Mark not assessed</strong> is a link on blank rows only: a neutral state, never a value. Every saved row says who entered it.</li>
                <li>A <strong>skill</strong> is a chip too: pick a descriptor and/or say what you saw, dated by the session.</li>
                <li><strong>Review session</strong> reads the counts back for every metric — <em>4 recorded · 1 not assessed · 1 not recorded — of 6 in scope</em> — never fills anything in and never marks the session complete. <strong>Back to Sessions</strong> takes you out.</li>
              </ol>
              <p>Editing a saved attempt is a <strong>correction</strong> — the original stays on the record (<em>corrected — was 8.51</em>). <strong>Change scope</strong> lives on the session; a retired test or a player who has left can never be added to one. A session started before scopes existed counts against the active roster, as before.</p>
              <p>Every result lands on that player&rsquo;s record exactly as if you&rsquo;d recorded it there, tagged with the session date; deleting a session never deletes the results. An old session shows <strong>everything saved in it</strong> — a retired test as a chip marked <em>retired</em>, a player who has since left with their row marked <em>no longer on the roster</em>, both read-only.</p>
              <p>Skills &amp; Goals has three views across the top: <strong>Sessions</strong> (this list), <strong>Players</strong> (everyone in roster order — pick <em>Current focus</em>, one test to see the <em>headline</em> of its latest session per player, or a skill for the latest observation) and <strong>Metrics</strong> (your tests and skills, where each is defined). If there is no <strong>Start session</strong> button, no test is defined yet.</p>
            </>
          ),
        },
        {
    ...
    },
    {
        ...
        },
        {
          ...
          content: (
            <>
              ...
              <HelpDefs>
                <HelpDef term="Test">A number you record the same way every time. Give it a name, a unit (seconds, mph, feet), an <strong>aim</strong> — lower, higher, within a stated range (then From and To, in the unit), or record only — how many <strong>attempts</strong> one session takes (one to five) and which attempt is the <strong>headline</strong> (best in the aim&rsquo;s direction, the average, or the last; a range test offers <em>attempts in range</em> instead of best). <strong>Method</strong> — how you run the test — is optional: a note for whoever runs it next, so results are taken the same way.</HelpDef>
              ...
              <p>Under Unit and Aim a live line says what a result will read back as — <em>8.12 seconds · best attempt · lower is the aim</em> — and <strong>Preview</strong> shows the full example, including how the reports describe a change (<em>0.35 seconds lower</em>, never faster or better). Each Metrics row says the same in its second column.</p>
              ...
              <p><strong>Retire</strong> and <strong>Restore</strong> sit in the sheet&rsquo;s footer. Retired metrics stay on the list under <em>Retired (N)</em>, each row saying <em>retired</em> and what replaced it. Open one and it reads as a record — every field shown, only the name editable — with Restore beside Cancel. A metric that was replaced cannot be restored; its successor carries the name.</p>
            </>
          ),
        },
        {
          id: 'premium-development-record',
          title: 'Recording a session — the scope, every attempt, and what each row says',
          content: (
            <>
              <p><strong>+ Start session</strong> asks for the session&rsquo;s <strong>scope</strong> first: the date, the practice (optional — it fills the date in, never owns it), which metrics (tests <em>and</em> skills) and who is here — everyone active is ticked; untick anyone absent. Then the grid opens.</p>
              <HelpDefs>
                <HelpDef term="One field per attempt">As many as the test&rsquo;s definition says (<em>Attempts per session</em>, in Metrics). Saves when you leave a field; <strong>Enter</strong> moves to the next attempt, then the next player. The headline updates as you type. A blank attempt was not run — never a zero.</HelpDef>
                <HelpDef term="The row&rsquo;s state"><strong>Saved · Saving · Not saved — retry · Not recorded · Not assessed</strong>, with <strong>Edit</strong> and <strong>Retry</strong> on the row. A value that failed to save stays on screen until you retry. Every saved row says who entered it.</HelpDef>
                <HelpDef term="Not assessed">A link on blank rows only — you chose not to assess this player on this metric today. A neutral state, never a value; <strong>Undo</strong> returns the row to <em>Not recorded</em>.</HelpDef>
                <HelpDef term="A correction">Editing a saved attempt keeps the original on the record — the row reads <em>corrected — was 8.51</em>. A wrong attempt, test or date is removed and re-entered.</HelpDef>
                <HelpDef term="A skill chip">A skill sits among the chips: pick a descriptor and/or say what you saw, dated by the session. Recording one needs the Development switch <em>and</em> Internal notes.</HelpDef>
              </HelpDefs>
              <p>The line under the grid counts against the scope — <em>4 recorded · 1 not assessed · 1 not recorded — of 6 in scope</em>; a player outside the scope with a result is listed and flagged, never counted. A session started before scopes existed counts against the active roster, as before. <strong>Change scope</strong> lives on the session; a retired test or a departed player can never be added to one.</p>
              <p><strong>Review session</strong> reads the counts back for every metric, says who ran fewer attempts than the test asks for (nothing filled in), never invents a zero and never marks the session complete. <strong>Back to Sessions</strong> takes you out.</p>
          ...
          content: (
            <>
              ...
              <HelpDefs>
                ...
                <HelpDef term="Observations">A timeline per skill: the descriptor, what you saw, whether it was taken in a session, who wrote it, and <em>Evidence for</em> the goal it supports. <strong>Record an observation</strong> lives here and on a goal.</HelpDef>
        ...
        },
        {
          ...
          content: (
            <>
              ...
              <HelpNote variant="warning" title="Moving a session’s date moves every result in it">
                <p>So the session never disagrees with its own contents — you&rsquo;re asked first, and told exactly how many attempts will move.</p>
              </HelpNote>
      ...
      faqs: [
        ...
        },
        {
          ...
          answerText: 'Yes. Open the session and change the Date field. Because every result is stamped with the session\'s date as you type it, moving the session moves those results with it — otherwise the session would disagree with its own contents and the trend lines would plot on the wrong day. You are asked to confirm first and told exactly how many attempts will move; with nothing entered yet there is no prompt at all. You can also set "Taken at" to the practice or game the results came from, which fills the date in for you. Linking an event only fills the date in, it never owns it: if that practice is later rescheduled your session stays on the day the testing actually happened. Results you recorded one at a time from a player\'s profile are not part of any session and are never touched. Changing the date is head-coach only.',
          ...
          answer: (
            <>
              ...
              <p>Every result is stamped with the session&rsquo;s date as you type it, so <strong>moving the session moves those results with it</strong>. Otherwise the session would disagree with its own contents and your trend lines would plot on the wrong day. You&rsquo;re asked to confirm first and told <strong>exactly how many attempts will move</strong>; with nothing entered yet there&rsquo;s no prompt at all.</p>
              <p>You can also set <strong>Taken at</strong> to the practice or game the results came from, which fills the date in for you. That link only <em>fills in</em> the date — it never owns it, so a practice that&rsquo;s later rescheduled won&rsquo;t drag your session with it.</p>
              <p>Results you recorded one at a time from a player&rsquo;s profile aren&rsquo;t part of any session and are never touched. Changing the date is head-coach only.</p>
            </>
          ),
        },
        {

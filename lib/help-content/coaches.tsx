    ...
    },
    {
      ...
      content: (
        <>
          ...
          <ul>
            ...
            <li>A <strong>Development</strong> section on every player — plain-language focus areas with a simple status, and coach-logged <strong>measurables</strong> (like a 60-yd sprint time) with a trend line once a player has two readings — plus a <strong>Development</strong> page (Squad menu) with <strong>Evaluation Sessions</strong> for whole-roster testing days and a <strong>Team board</strong> overview.</li>
      ...
      faqs: [
        ...
        },
        {
          ...
          popular: true,
          answerText: 'Open any player from your Roster and find the Development section. Focus areas are the things this player is working on, in plain language ("first-step quickness off the bag") with one short note and a simple status you tap to change — Working on it, Achieved, or Parked. No grades or percentages, on purpose. Measurables are the tests you already run at practice — a 60-yd sprint, home-to-first, throw velocity. Set up each test once (name + unit) and it becomes a chip you pick when logging; a reading is just the value and a date (defaults to today). A small trend line appears once a player has two readings of the same test — one reading honestly says a trend will show after a second entry. Every number compares the player only to themselves — there is no team leaderboard. Below, a Context list quotes the player’s depth-chart spots, this season’s field/bench innings, and attendance so the whole picture is on one screen. Only the head coach can add or edit development data; assistants see focus areas only if you’ve granted notes access, and measurables with roster visibility. To rename or retire a test (retiring keeps its logged history), tap Test types at the top of the section.',
          keywords: ['development', 'player development', 'focus areas', 'focus area', 'idp', 'measurables', 'measurable', 'log a measurable', '60 yard', 'sprint time', 'test times', 'trend', 'sparkline', 'working on it', 'achieved', 'parked', 'test types', 'retire test', 'player progress', 'track progress'],
          answer: (
            <>
              ...
              <p>Below, a <strong>Context</strong> list quotes the player&apos;s depth-chart spots, this season&apos;s field/bench innings, and attendance, so the whole picture is on one screen. Only the <strong>head coach</strong> can add or edit development data; assistants see focus areas only with <strong>notes</strong> access, and measurables with roster visibility. To rename or retire a test (retiring keeps its logged history), tap <strong>Test types</strong> at the top of the section.</p>
            </>
          ),
        },
        {

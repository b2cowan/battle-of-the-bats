    ...
    },
    {
      ...
      faqs: [
          ...
          answer: (
            <>
              ...
              <HelpDefs>
                ...
                <HelpDef term="Results">Your measured tests. Each row leads with the latest session&rsquo;s <strong>headline</strong> — <em>7.48 seconds · best of 3 attempts · avg 7.553</em>, or <em>2 of 3 in range</em> for a range test — with its date and a small trend line once there are two results. Open a row and every session is listed with every attempt, its best and its average. <strong>Record a result</strong> adds a single dated reading with no session. If a test&rsquo;s unit ever changed, the line is drawn from the current unit only and the row says so.</HelpDef>
                <HelpDef term="Observations">What you saw against an observed skill — a descriptor and/or a sentence, dated, who wrote it, and <em>Evidence for</em> the goal it supports. <strong>Record an observation</strong> lives here and on a goal.</HelpDef>
        ...
        },
        {
          ...
          popular: true,
          answerText: 'Open Skills & Goals in the Progress menu and tap + Start session in the page header (it was New session before September 2026). It asks for the session\'s scope first: the date, the practice it was taken at (optional — it fills the date in, never owns it), which metrics you are recording (your tests and your observed skills) and who is here — everyone active is ticked, untick anyone absent. Then the grid opens. Pick a metric across the top and work straight down your roster: a test gives one field per attempt, as many as its definition says; Enter moves to the next attempt, then the next player; the headline (best, average, last or attempts in range) updates as you type; a blank attempt was not run — never a zero. Every row shows its state — Saved, Saving, Not saved — retry, Not recorded or Not assessed — with Edit and Retry on the row; a value that failed to save stays on screen beside its error until you retry. Mark not assessed is a link on blank rows only — a neutral state with an optional reason, never a value. Editing a saved reading keeps the original on the record ("corrected — was 8.51"). Every saved row says who entered it. An observed skill is a chip too: pick a descriptor and/or say what you saw, dated by the session — it needs the Development switch and Internal notes to record and Internal notes to read. The counts under the grid run against the scope — "4 recorded · 1 not assessed · 1 not recorded — of 6 in scope"; a session started before scopes existed counts against the active roster as before ("N of M entered"). Change scope on the session; a retired test or a player who has left the team can never be added to a scope. Review session reads the counts back for every metric, never fills anything in and never marks the session complete; Back to Sessions takes you out. Rows always stay in roster order — testing day is not a leaderboard. Every reading lands on that player\'s record exactly as if you had recorded it there, tagged with the session date; deleting a session never deletes the readings. An old session shows everything saved in it: a retired test still appears as a chip marked retired, read-only; a player who has since left keeps their row, marked no longer on the roster. Skills & Goals has four tabs across the top: Overview (where you land — the getting-started card on a new team, the season\'s dashboard once sessions exist), Sessions (this list), Players (everyone in roster order — pick Current focus, one test to see the headline of its latest session per player, or an observed skill for the latest observation; the Team board page used to do this) and Metrics (your tests and skills, where each is defined). If there is no Start session button, no measured test is defined yet — the Overview says so.',
          ...
          answer: (
            <>
              ...
              <ol>
                ...
                <li>An <strong>observed skill</strong> is a chip too: pick a descriptor and/or say what you saw, dated by the session.</li>
        ...
        },
        {
          ...
          answer: (
            <>
              ...
              <p>A previous season&apos;s focus areas and readings are <strong>read-only</strong> — you can read them in the archive, but nothing from this season can add to, edit or delete them. Anything that tries is refused with <em>&ldquo;This player belongs to a past season, which is read-only.&rdquo;</em></p>
    ...
    },
    {
      ...
      subtopics: [
        ...
        },
        {
          ...
          content: (
            <>
              <p>Open <strong>Metrics</strong> and tap <strong>+ Define a metric</strong>. The first choice changes the rest of the form:</p>
              <HelpDefs>
                <HelpDef term="Measured test">A number you record the same way every time. Give it a name, a unit (seconds, mph, feet), an <strong>aim</strong> — lower, higher, within a stated range (then From and To, in the unit), or record only — how many <strong>attempts</strong> one session takes (one to five), which attempt is the <strong>headline</strong> (best in the aim&rsquo;s direction, the average, or the last; a range test offers <em>attempts in range</em> instead of best), and the <strong>method</strong>: how you run it, written once so every result is comparable.</HelpDef>
                <HelpDef term="Observed skill">Something you watch for. Give it a name and, if you like, <strong>descriptors</strong> — your own words for what you saw, one per line, in your order. Never a scale, never averaged. An observation against it is recorded from the skill&rsquo;s chip on a session, or from a player&rsquo;s <strong>Observations</strong> view.</HelpDef>
              </HelpDefs>
              <p>The panel beside the form shows what a saved record will read back as — <em>&ldquo;8.05 seconds · best of three&rdquo;</em>, or <em>&ldquo;2 of 3 in range&rdquo;</em> — so you see the consequence of a unit or aim choice before you save. A test you set up earlier with only a name and a unit reads <em>record only · method not recorded</em> until you finish its definition; nothing is guessed for it.</p>
              <HelpNote variant="info" title="Changing a definition later">
                <p>A rename fixes a typo and keeps the series. Changing the <strong>unit</strong> or a written <strong>method</strong> on a test that already has results starts a <em>new</em> definition and retires this one — the page offers you the choice — because those results were taken the old way and the two must never be drawn as one line. Writing a method where none was recorded keeps the series.</p>
              </HelpNote>
              <p><strong>Retire</strong> and <strong>Restore</strong> live on the metric&rsquo;s page; retired definitions sit under a disclosure at the foot of Metrics, each saying what replaced it.</p>
            </>
          ),
        },
        {
          ...
          content: (
            <>
              <p><strong>+ Start session</strong> asks for the session&rsquo;s <strong>scope</strong> first: the date, the practice (optional — it fills the date in, never owns it), which metrics (tests <em>and</em> observed skills) and who is here — everyone active is ticked; untick anyone absent. Then the grid opens.</p>
              <HelpDefs>
                ...
                <HelpDef term="A correction">Editing a saved reading keeps the original on the record — the row reads <em>corrected — was 8.51</em>. A wrong attempt, test or date is removed and re-entered.</HelpDef>
                <HelpDef term="A skill chip">An observed skill sits among the chips: pick a descriptor and/or say what you saw, dated by the session. Recording one needs the Development switch <em>and</em> Internal notes.</HelpDef>
              </HelpDefs>
              <p>The line under the grid counts against the scope — <em>4 recorded · 1 not assessed · 1 not recorded — of 6 in scope</em>; a player outside the scope with a reading is listed and flagged, never counted. A session started before scopes existed counts against the active roster, as before. <strong>Change scope</strong> lives on the session; a retired test or a departed player can never be added to one.</p>
          ...
          content: (
            <>
              ...
              <HelpDefs>
                ...
                <HelpDef term="One result">One point and no line — a single reading is a point, not a change.</HelpDef>
                <HelpDef term="A changed unit">The earlier readings are listed beneath the table but never drawn on the same line.</HelpDef>
                <HelpDef term="Records behind the chart">Every session, every attempt, the best (or attempts in range), the average, who entered it and a link to the session — with <em>corrected — was 8.51</em> where a reading was corrected. <strong>Open the player&rsquo;s development →</strong> returns here afterwards.</HelpDef>
                <HelpDef term="An observed skill">The chart gives way to the <strong>observation timeline</strong> — each entry dated, with its descriptor, what you saw, who wrote it and the goal it is evidence for — followed by that goal&rsquo;s review history. No line ever joins descriptors.</HelpDef>
              </HelpDefs>
      ...
      faqs: [
        {
          ...
          answer: (
            <>
              <p>Because there&rsquo;s no test for a session to <em>record</em>. A session runs your tests across the roster, so it needs at least one <strong>active measured test</strong> to exist — an observed skill alone isn&rsquo;t enough, since a session&rsquo;s grid takes numbers (a skill joins the chips beside the tests).</p>
          ...
          answer: (
            <>
              ...
              <ul>
                ...
                <li>One result is one point with no line. A test whose unit changed lists its earlier readings but never draws them on the same line.</li>
                <li><strong>Records behind the chart</strong> lists every session and attempt, with a link to the session.</li>
              </ul>
              <p>Choose an observed skill and you get the <strong>observation timeline</strong> instead — no line ever joins descriptors.</p>
            </>
          ),
        },
        {

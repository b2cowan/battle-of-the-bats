...
export async function createRegistration(input: PublicRegistrationInput): Promise<LeagueRegistration> {
  const { data } = await supabaseAdmin
    ...
    .insert({
      ...
      guardian_email:       normalizeGuardianEmailRequired(input.guardianEmail),
...
export interface CoachingAssignment {
  ...
  programYearId: string;
  programYearName: string;
  /** Calendar year. Chunk F: the season switcher orders by this, and a team can legitimately
   *  hold BOTH a draft and an active year at once — without it, the client picked whichever the
   *  DB happened to return first while the server picked the newest, so the page header and the
   *  data behind it could name different seasons. */
...
export async function getCoachTeamMilestones(
  ...
  const [
    ...
    // milestone exists to avoid. Fails soft: an error here reads as "no lineup yet". Empty
...
/**
 * Wipe any leftover offer-link state on a registration. HYGIENE ONLY as of 2026-08-26: the family
 * self-serve Accept/Decline loop was retired with the tryout decision emails (owner ruling — a rep
 * offer is a custom letter the family signs, so the platform sends nothing on a coach's behalf).
 * Nothing mints these columns any more; this exists so a token left on an existing row by the old
 * loop is cleared on the row's next transition and can never resolve.
 */
 ...
 * route and the coach route both reuse it (auth is enforced by the caller).
 *
...
export async function createRepRosterPlayer(fields: {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .insert({
      ...
      guardian_email: normalizeGuardianEmail(fields.guardianEmail),
      guardian_phone: fields.guardianPhone ?? null,
...
export async function updateRepRosterPlayer(playerId: string, fields: {
  ...
  if (fields.guardianEmail !== undefined) patch.guardian_email = normalizeGuardianEmail(fields.guardianEmail);
...
function mapRepTeamLineup(r: any): RepTeamLineup {
  return {
    ...
    updatedBy: r.updated_by ?? null,
    status: (r.status ?? 'draft') as 'draft' | 'ready',
...
// Per-event honest readiness (within a program year): Not started / Draft / Ready / Needs review —
...
export async function getRepTeamLineupReadinessByEvent(
  ...
  const { data: entries, error: eErr } = await supabaseAdmin
    .from('rep_team_lineup_entries')
    .select('lineup_id, player_id, inning_positions')
    .in('lineup_id', lineups.map(l => l.id));
  if (eErr) throw eErr;

  const entriesByLineup = new Map<string, { playerId: string; inningPositions: Record<string, string> }[]>();
  for (const row of entries ?? []) {
    const arr = entriesByLineup.get(row.lineup_id as string) ?? [];
    arr.push({
      playerId: row.player_id as string,
      inningPositions: (row.inning_positions ?? {}) as Record<string, string>,
    });
...
// (the lineup route's PATCH handler) re-checks eligibility against freshly-read data immediately
// before calling this; this function does not re-validate, so it must never be reachable directly
// from a request body.
export async function markRepTeamLineupReady(lineupId: string, userId: string): Promise<RepTeamLineup> {
  const { data, error } = await supabaseAdmin
    .from('rep_team_lineups')
    .update({ status: 'ready', ready_at: new Date().toISOString(), ready_by: userId })
    .eq('id', lineupId)
    .select()
    .single();
  if (error) throw error;
...
export async function getDueReminderCandidates(
  teamId: string,
  /** `null` — no forward cutoff at all (the per-family "Remind this family" button, owner ruling
   *  2026-09-21: an explicit click reaches an installment however far off it is). A number bounds
   *  the team-wide sends (past due, or due within this many days) as before. */
  ...
  const cutoffStr = daysAhead === null ? null : addCalendarDays(todayStr, daysAhead);
  ...
  const candidates = allInstallments.filter(i => {
    ...
    if (cutoffStr !== null && i.due_date > cutoffStr) return false;

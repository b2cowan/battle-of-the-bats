 ...
 * of its own at `/practice/templates/{id}`, the circuit editor at `/practice/circuits/{id}` — the
 * list is a tab, the record is a page, the same split Skills & Goals makes between
 * `?section=metrics` and `/development/metrics/{id}`.
 *
 ...
 */
export type PracticePlansSection = 'practices' | 'templates' | 'circuits' | 'drills';
export const PRACTICE_PLANS_SECTIONS: ReadonlyArray<PracticePlansSection> = ['practices', 'templates', 'circuits', 'drills'];
...
/** The circuit editor's own address (stage 4, L9). */
export function circuitHref(base: string, circuitId: string): string {
  return `${base}/practice/circuits/${circuitId}`;
}

 ...
 * (board summary, family dues statement) — a drawn document must not reinvent the slot,
 * which is how the bracket ended up with its own squashed 20×10 version (its fix belongs
 * to the Posters, cards & brackets pass).
 */
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  function drawHeader(isFirstPage: boolean) {
    ...
    // Logo — the shared aspect-fit slot (D4).
...
export async function downloadTryoutBoardSummary(filename: string, opts: TryoutBoardSummaryOptions): Promise<void> {
  ...
  y = MARGIN;
  const drawnLogoW = drawLogoSlot(doc, settings, MARGIN, y);
  const headTextX = drawnLogoW > 0 ? MARGIN + drawnLogoW + 4 : MARGIN;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(14);
  doc.setTextColor(20, 20, 35);
  doc.text(line1, headTextX, y + 6);
  if (line2) {
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    doc.setTextColor(80, 80, 100);
    doc.text(line2, headTextX, y + 11);
  }
  y += Math.max(drawnLogoW > 0 ? LOGO_SLOT_H : 0, line2 ? 15 : 12);
  doc.setDrawColor(210, 210, 220);
  doc.setLineWidth(0.3);
  doc.line(MARGIN, y, pageWidth - MARGIN, y);
  y += 7;
...
export function buildFamilyDuesStatementsDoc(jsPDFClass: any, autoTable: any, opts: FamilyDuesStatementsOptions): any {
  ...
  families.forEach((family, idx) => {
    ...
    y += Math.max(drawnLogoW > 0 ? LOGO_SLOT_H : 0, settings.headerLine2 ? 15 : 12);
    doc.setDrawColor(210, 210, 220);
    doc.setLineWidth(0.3);
    doc.line(MARGIN, y, pageWidth - MARGIN, y);
    y += 6;
